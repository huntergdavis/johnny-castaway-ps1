...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.2387% over target / 99.7644% target speed** after the WALKSTUF1 low setup/`{113..129}` CD-pressure promotion, the same-speed `{355..371}` read-work row, the W1-high frame56/`{178..194}` CD-pressure promotion, and the VISITOR3-high 80 KiB clean-relief window promotion. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
