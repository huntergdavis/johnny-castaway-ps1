...
The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest four-yellow promotion gate is `scratch/ps1-perf-iterate/w1high-clip35-41-four-yellow-current-20260522/20260522-154945-2260110/summary.json`. Public rollup is `+0.2149%` over target / `99.7875%` target speed and raw signed rollup is about `-0.5020%` / `100.5178%`; bands are `123` green, `3` yellow, `0` orange, and `0` red. That is about `17.19` public over-target points removed and `12.69` public target-speed points added since the compact full-matrix baseline.

The newest WALKSTUF1-high headroom promotion extends the early offscreen clip
baseline by clipping active-loop entries `35`, `37`, `39`, and `41` while
preserving the `1535263` byte pack footprint, entry sizes, offsets, LBA
`24891`, sectors `750`, and `233472` byte PS-EXE bucket. The latest subset
drops selected logical payload `26856 -> 23537`, removing `688` cleanup pixels
and `4706` draw pixels. The four-yellow canary keeps W1-high exact-flat at
`1808/1469/1440`, overrun `29`, blocking/refill `42/12`, reads/due `41/7`,
while cumulative post-tail clip headroom drops runtime rows/spans/pixels
`16712/127281/714231 -> 16641/124649/688006`; BUILDING2 high and VISITOR3
