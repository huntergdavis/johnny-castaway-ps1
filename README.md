...
follow-up, the VISITOR3 high persistent setup-segment follow-up, the
VISITOR3 high frame-126/frame-125 re-anchor follow-ups, and the VISITOR3
high setup-prime expansion, the current public-capped rollup is `+0.3829%`
over target / `99.6284%` target speed
across all 126 timing-bearing rows; the raw signed CSV records
`-0.3856%` over target / `100.4122%` target speed for
internal optimization analysis.

The latest VISITOR3 follow-up, `visitor3-high-setup-prime320-v214`, expands
the high-only setup-prime residency cap from `288 KiB` to `320 KiB` on top of
the v207 high re-anchor stack, the v204 low-tide setup segment, and the v213
high setup-prime expansion. It improves active-loop timing
`1099/1032 -> 1089/1035`, cuts overrun `67 -> 54`, blocking `96 -> 83`,
loop reads `17 -> 15`, loop-read time `96 -> 83`, and due misses `17 -> 15`.
...
PS-EXE bucket. The rejected low-cap variants (`288/256/240 KiB`) either added
hidden refill or regressed visible timing, so the current runtime baseline is
v214 high plus v204 low.
...
87.1% target speed**, the public-capped view has removed about **17.02
percentage points** of over-target gap and added about **12.53 target-speed
points**.
...
`visitor3-high-setup-prime320-v214` plus `visitor3-low-segment281-305-v204`:
...
sectors `277..293` and `281..305` plus a high-only `320 KiB` setup-prime cap.
Loop timing is `1089/1035` high and `1102/1032` low while blocking drops to
`83/124`, loop-read time drops to `83/131`, and due misses are `15/22`; v214
high plus v204 low is the current
...
The current public-capped rollup is `+0.3829%` over target / `99.6284%` target
speed; the raw signed optimization matrix is `-0.3856%` / `100.4122%`. The next
...
`2075/2061`.
The public-capped timing-bearing average is now `+0.3829%` over target /
`99.6284%` target speed after the VISITOR3 high-only sparse frame-117
...
v213/v214 high setup-prime cap expansions.
The raw signed optimization matrix is `-0.3856%` / `100.4122%`.
The latest VISITOR3 pass is a high-tide setup-prime/CD-pressure promotion:
it moves high to `1089/1035`, cutting overrun `67 -> 54`, blocking `96 -> 83`,
loop-read time `96 -> 83`, loop reads `17 -> 15`, and due misses `17 -> 15`;
low remains the v204 profile at `1102/1032`, `blocking_vb=124`, and due
misses `22`. The latest rejected v183-v212 VISITOR3 probes close low precursor motion-copy frames `114..118`, a
...
`87.1%` target speed, the public-capped view has removed about `17.02`
percentage points of over-target gap and added about `12.53` points of target
speed.
