...
The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest W1-high promotion gate is `scratch/ps1-perf-iterate/w1high-direct185-191-under-yellow-canary-20260523/20260523-005607-1475417/summary.json`. Public rollup is `+0.2067%` over target / `99.7953%` target speed and raw signed rollup is about `-0.5102%` / `100.5257%`; bands are `123` green, `3` yellow, `0` orange, and `0` red. That is about `17.19` public over-target points removed and `12.70` public target-speed points added since the compact full-matrix baseline.

The newest WALKSTUF1-high direct-stage refinement extends the high-tide direct
stage from frames `189..191` to `185..191`, preserving pixels, pack LBA, and
the `233472` byte PS-EXE bucket. The under-yellow canary keeps VISITOR3
high/low exact-flat while W1-high improves `1816/1467/1442 -> 1816/1467/1443`,
overrun `25 -> 24`, with blocking/refill still `37/8` and due misses still
`6`. The prior `383..399` setup slice remains the reason this direct-stage
shape is now safe instead of paying the older blocking/due regression.
