...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.2767% over target / 99.7279% target speed**. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
The mainline shifted from "prove every scene" to **performance polish, stability, and content** at `v0.7.0-ps1`. Current post-release mainline win: BUILDING2 low now adds the slack-8 `218..229` retained-read row ahead of the accepted low groups, improving to `1344/1318`, overrun `26`, blocking/refill `61/0`, and reads/due `50/14`, with public rollup `+0.2767%` over target / `99.7279%` target speed and raw signed rollup `-0.4904%` / `100.5103%`. Earlier current mainline wins include VISITOR5 low's accepted high-tide `30..46` retained-read shape and the WALKSTUF1 shared `427..443` same-speed CD-work cleanup.

Recent releases:

- Current mainline after `v0.8.8-ps1` — VISITOR5 low uses the low-tide `30..46` retained-read group, improving from `1104/1092` to `1102/1097` and moving that row into green. WALKSTUF1 low also gained a same-speed shared `427..443` CD-work row, dropping loop reads `64 -> 62`. The latest promoted BUILDING2 low `218..229` slack-8 row improves `1349/1320 -> 1344/1318`, overrun `29 -> 26`, blocking `70 -> 61`, and reads/due `52/15 -> 50/14`; bands remain `117` green and `9` yellow.
