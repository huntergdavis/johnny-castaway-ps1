...
The mainline shifted from "prove every scene" to **performance polish, stability, and content** at `v0.7.0-ps1`. Current mainline builds on `v0.8.16-ps1` with the memory-region allocator promoted: BOOT allocations seal after startup, CACHE allocations use free-list/LRU reuse for long-lived resource data, and TRANSIENT scene allocations can be wiped between major scene loads instead of relying on a fragmented general heap. The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest five-yellow canary refresh is `scratch/ps1-perf-iterate/v3high-entry62-clean-only-five-yellow-current/20260521-134913-1781582/summary.json`. Public rollup is `+0.2465%` over target / `99.7571%` target speed and raw signed rollup is about `-0.4704%` / `100.4875%`; bands are `121` green, `5` yellow, `0` orange, and `0` red. That is about `17.15` public over-target points removed and `12.66` public target-speed points added since the compact full-matrix baseline. The newest VISITOR3-high entry `62` cleanup pass preserves file size, offsets, entry table sizes, LBA, and the PS-EXE bucket while shrinking entry restore bytes `2724 -> 596`, runtime restore bytes `56312 -> 54184`, and upload bytes `18038400 -> 18012160`; all five under-green canary rows stay exact-flat. The prior VISITOR3-low `88..104` read-group pass stacks on the accepted `16..32` and `72..88` rows and improves VISITOR3-low `1343/1070/1039 -> 1342/1069/1039`, cuts overrun `31 -> 30`, blocking `70 -> 68`, loop reads `15 -> 14`, and read time `93 -> 91`, with due misses flat at `11`. The prior VISITOR3-low `72..88` read-group pass stacks on the accepted `16..32` row and improves VISITOR3-low `1339/1066/1035 -> 1343/1070/1039`, keeps overrun at `31`, cuts blocking `73 -> 70`, loop reads `17 -> 15`, read time `96 -> 93`, and due misses `13 -> 11`. The prior VISITOR3-low `16..32` read-group pass built on the frame135 gap-D4 baseline and improved VISITOR3-low `1342/1070/1039 -> 1339/1066/1035`, overrun `31`, blocking `78 -> 73`, loop reads `18 -> 17`, read time `104 -> 96`, and due misses `14 -> 13`. The current under-green queue remains BUILDING2 high, VISITOR3 low, VISITOR3 high, WALKSTUF1 high, and WALKSTUF1 low.
...
- Current VISITOR3-high entry 62 cleanup-only headroom promotion — `VISITOR3.FG2` keeps file size, offsets, entry table sizes, LBA, and the PS-EXE bucket fixed while entry `62` / source frame `80` drops cleanup restore bytes `2724 -> 596`, cleanup spans `156 -> 88`, cleanup pixels `1362 -> 298`, runtime restore bytes `56312 -> 54184`, and upload bytes `18038400 -> 18012160`. The five-yellow canary stays exact-flat at the same public rollup.
