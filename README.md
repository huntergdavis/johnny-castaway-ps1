...
The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest under-yellow promotion gate is `scratch/ps1-perf-iterate/w1h-double-preload-3wait-canary-20260523-121349/20260523-121350-1998025/summary.json`. Public rollup is `+0.1956%` over target / `99.8062%` target speed and raw signed rollup is `-0.5213%` / `100.5366%`; bands are `124` green, `2` yellow, `0` orange, and `0` red. That is about `17.20` public over-target points removed and `12.71` public target-speed points added since the compact full-matrix baseline.

**The newest WALKSTUF1-high double-preload-3-wait promotion moves W1-high
...
- Current WALKSTUF1-high frame0 consume speed promotion - W1-high now presents
  ...
  bands remain `124` green / `2` yellow.
