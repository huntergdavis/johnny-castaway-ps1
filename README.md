...
- `main` after `v0.8.7-ps1` — BUILDING2 low stores frames `71` and `77` as previous-frame D4 deltas, improving to `1349/1320`, overrun `29`, blocking `70`, prefetch overrun `0`, reads/due `52/15`; VISITOR3 high now stores frame `137` as a 503-byte D4 delta, cutting blocking `52 -> 51` with loop/target flat at `1071/1040`; the latest WALKSTUF1 high current-control refresh records `1476/1434`, overrun `42`, blocking `81`, refill `23`. Public rollup remains `+0.3033%` over target / `99.7023%` target speed; WALKSTUF1 low, BUILDING2 high, VISITOR3 high, WALKSTUF1 high, and VISITOR3 low remain the top under-99 rows.
