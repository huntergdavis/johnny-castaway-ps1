...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.3336% over target / 99.6734% target speed**. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
- `v0.8.5-ps1` — full 126-row headless performance matrix baseline; public rollup at `+0.3336%` over target / `99.6734%` target speed.
