...
high setup-prime expansion, the current public-capped rollup is `+0.3930%`
over target / `99.6194%` target speed
across all 126 timing-bearing rows; the raw signed CSV records
`-0.3755%` over target / `100.4032%` target speed for
internal optimization analysis.

The latest VISITOR3 follow-up, `visitor3-high-setup-prime288-v213`, expands
the high-only setup-prime residency cap from `232 KiB` to `288 KiB` on top of
the v207 high re-anchor stack and the v204 low-tide setup segment. It trades
high setup time (`scene_vb 1414 -> 1423`) for active-loop relief:
...
`visitor3-high-setup-prime288-v213` plus `visitor3-low-segment281-305-v204`:
...
sectors `277..293` and `281..305` plus a high-only `288 KiB` setup-prime cap.
Loop timing is `1099/1032` high and `1102/1032` low while blocking drops to
`96/124`, loop-read time drops to `96/131`, and due misses are `17/22`; v213
...
The current public-capped rollup is `+0.3930%` over target / `99.6194%` target
speed; the raw signed optimization matrix is `-0.3755%` / `100.4032%`. The next
...
`2075/2061`.
The public-capped timing-bearing average is now `+0.3930%` over target /
`99.6194%` target speed after the VISITOR3 high-only sparse frame-117
...
v213 high setup-prime cap expansion.
The raw signed optimization matrix is `-0.3755%` / `100.4032%`.
The latest VISITOR3 pass is a high-tide setup-prime/CD-pressure promotion:
it moves high to `1099/1032`, cutting overrun `71 -> 67`, blocking `106 -> 96`,
loop-read time `106 -> 96`, loop reads `20 -> 17`, and due misses `20 -> 17`;
