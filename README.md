...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.2801% over target / 99.7249% target speed** after the WALKSTUF1 high setup-segment retarget. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
- Current allocator-era optimization checkpoint — VISITOR3 high improves from `1232/1033` to `1082/1042` and VISITOR3 low from `1231/1040` to `1074/1039` by retaining the tiny stage1 prefetch frame buffer under clean-memory relief, keeping bounded clean-relief stream windows, trimming the high-tide terminal read before resident setup-segment data, extending high setup segment2 to relative sectors `203..229`, relocating high frame `139` into that paid gap without decode, adding the high-tide third retained setup segment `228..262`, and adding the low-tide third retained setup segment `206..230`. BUILDING4 high then primes `264..288` during setup and moves into green, BUILDING2 high replaces its tail read group with `83..95`, WALKSTUF1 low replaces its split `197..243` + `410..434` residency with `238..342`, and WALKSTUF1 high keeps `198..244` while retargeting its second retained slice to `286..342`, cutting high blocking to `60` and reads to `48`. The current under-green queue is VISITOR3 high/low, BUILDING2 high, WALKSTUF1 low/high, BUILDING2 low, and BUILDING4 low.
