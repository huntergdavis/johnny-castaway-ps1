...
The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest under-yellow promotion gate is `scratch/ps1-perf-iterate/w1h-helper3wait-canary-20260523-113938/20260523-113938-1770485/summary.json`. Public rollup is `+0.1977%` over target / `99.8041%` target speed and raw signed rollup is `-0.5192%` / `100.5344%`; bands are `123` green, `3` yellow, `0` orange, and `0` red. That is about `17.20` public over-target points removed and `12.70` public target-speed points added since the compact full-matrix baseline.

The newest WALKSTUF1-high helper-3-wait retime extends the W1-high frame0
...
- Current WALKSTUF1-high frame0 consume speed promotion - W1-high now presents
  ...
  bands remain `123` green / `3` yellow.
