...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.2760% over target / 99.7290% target speed** after the WALKSTUF1 high setup-slice pressure promotion. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
The mainline shifted from "prove every scene" to **performance polish, stability, and content** at `v0.7.0-ps1`. Current mainline builds on `v0.8.16-ps1` with the new memory-region allocator promoted: BOOT allocations seal after startup, CACHE allocations use free-list/LRU reuse for long-lived resource data, and TRANSIENT scene allocations can be wiped between major scene loads instead of relying on a fragmented general heap. The allocator merge also adds pack-header metrics and CI gates requiring `MEM_REGION_RATIONALE` annotations at every `memAlloc` call site. The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest focused allocator-era checkpoint refreshes the under-green VISITOR3, WALKSTUF1, BUILDING2, and BUILDING4 rows. Public rollup is `+0.2760%` over target / `99.7290%` target speed and raw signed rollup is about `-0.4410%` / `100.4593%`; bands are `119` green, `7` yellow, `0` orange, and `0` red. That is about `17.12` public over-target points removed and `12.63` public target-speed points added since the compact full-matrix baseline. The newest pressure win extends WALKSTUF1 high's second retained setup slice from `286..342` to `286..344`, keeping overrun flat at `34` while reducing blocking `60 -> 59`, prefetch overrun `15 -> 13`, loop reads `48 -> 46`, and read time `228 -> 212`. The prior renderer win retunes the dirty-upload vertical band merge gap from `0` to `8`, improving BUILDING4 low from `2853/2816` to `2849/2816`, overrun `37 -> 33`, blocking `42 -> 38`, prefetch overrun `35 -> 31`, and read time `223 -> 222` while keeping loop reads/due `30/1` flat.
...
- Current allocator-era optimization checkpoint — VISITOR3 high improves from `1232/1033` to `1082/1042` and VISITOR3 low from `1231/1040` to `1074/1039` by retaining the tiny stage1 prefetch frame buffer under clean-memory relief, keeping bounded clean-relief stream windows, trimming the high-tide terminal read before resident setup-segment data, extending high setup segment2 to relative sectors `203..229`, relocating high frame `139` into that paid gap without decode, adding the high-tide third retained setup segment `228..262`, and adding the low-tide third retained setup segment `206..230`. BUILDING4 high then primes `264..288` during setup and moves into green, BUILDING2 high replaces its tail read group with `83..95` and then adds guarded `271..287` plus `315..327`, WALKSTUF1 low replaces its split `197..243` + `410..434` residency with `238..342`, WALKSTUF1 high keeps `198..244` while retargeting its second retained slice to `286..344`, BUILDING2 low primes `112..128` during setup to cut reads and due misses, and BUILDING4 low now uses an `8`-row dirty-upload band merge gap to reach `2849/2816`. The current under-green queue is VISITOR3 high/low, WALKSTUF1 low, BUILDING2 high, WALKSTUF1 high, BUILDING2 low, and BUILDING4 low.
