...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.3117% over target / 99.6956% target speed** after the BUILDING2 high read-group promotion. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
- Current allocator-era optimization checkpoint — VISITOR3 high improves from `1232/1033` to `1096/1040` and VISITOR3 low from `1231/1040` to `1088/1038` by retaining the tiny stage1 prefetch frame buffer under clean-memory relief, keeping bounded clean-relief stream windows, trimming the high-tide terminal read before resident setup-segment data, and extending high setup segment2 to relative sectors `203..229`. BUILDING4 high then primes `264..288` during setup and moves into green, and BUILDING2 high now replaces its tail read group with `83..95`, improving target/overrun to `1351/1313` and `38` over. The current under-green queue is VISITOR3 high/low, WALKSTUF1 high/low, BUILDING2 high/low, and BUILDING4 low.
