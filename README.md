...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.3184% over target / 99.6878% target speed**. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
- `main` after `v0.8.7-ps1` — BUILDING2 high now keeps retained read groups `60..72`, `206..230`, `226..242`, and `249..257` with a 24-sector grouped-read capacity, and BUILDING2 low adds the `238..250` retained read on top of the accepted `318..330` / `365..381` rows. Public rollup is now `+0.3184%` over target / `99.6878%` target speed; BUILDING2 high currently measures `1351/1311`, BUILDING2 low measures `1349/1319` with loop reads `53`, WALKSTUF1 high stays `1480/1432` with loop reads `65`, WALKSTUF1 low stays `1484/1431` while loop reads drop to `66`, and VISITOR5 high is `1107/1090`.
