...
The mainline shifted from "prove every scene" to **performance polish, stability, and content** at `v0.7.0-ps1`. Current mainline builds on `v0.8.16-ps1` with the memory-region allocator promoted: BOOT allocations seal after startup, CACHE allocations use free-list/LRU reuse for long-lived resource data, and TRANSIENT scene allocations can be wiped between major scene loads instead of relying on a fragmented general heap. The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest five-yellow canary refresh is `scratch/ps1-perf-iterate/local-lz-inline-five-yellow-current/20260521-180839-3241177/summary.json`. Public rollup is `+0.2465%` over target / `99.7571%` target speed and raw signed rollup is about `-0.4704%` / `100.4875%`; bands are `121` green, `5` yellow, `0` orange, and `0` red. That is about `17.15` public over-target points removed and `12.66` public target-speed points added since the compact full-matrix baseline. The newest code-headroom pass inlines the local-LZ decoder into `fgRuntimeLoadSceneFrame`, removing the standalone `0x1dc` decoder symbol, moving `foregroundPilotPlay` another `-124` bytes, and keeping all five under-green canary rows exact-flat with fixed pack LBAs and the same `233472` byte PS-EXE bucket. The prior D4 decoder inline pass removed the standalone `0x20c` decoder symbol and moved `foregroundPilotPlay` `-108` bytes under the same exact-flat five-yellow gate. The prior W1-high entries `183..191` previous-visible cleanup pass preserves file size, offsets, LBA, sectors, and the PS-EXE bucket while shrinking active payload `844162 -> 840654`, cleanup spans `2211 -> 885`, cleanup pixels `6670 -> 2265`, runtime restore bytes `509592 -> 500782`, and upload bytes `17182720 -> 17171200`; all five under-green canary rows stay exact-flat. The prior VISITOR3-low entry `109..112` fixed-layout clip preserves file size, offsets, LBA, and the PS-EXE bucket while shrinking selected active payload `8170 -> 6004` bytes. The prior W1-high entry `127..131` screen-clip pass preserves file size, offsets, LBA, and the PS-EXE bucket while shrinking the selected active payload `24097 -> 21512` bytes and removing `625` offscreen cleanup pixels plus `3511` offscreen draw pixels. The prior W1-high entry `53` screen-clip pass shrank that entry `3893 -> 2309` bytes and removed `8969` offscreen cleanup pixels plus `1945` offscreen draw pixels. The prior W1-low entry `90..99` canonicalization pass preserves file size, offsets, LBA, and the PS-EXE bucket while shrinking the active cluster `47579 -> 44511` bytes and total low-pack active payload `755808 -> 752740`. The current under-green queue remains BUILDING2 high, VISITOR3 low, VISITOR3 high, WALKSTUF1 high, and WALKSTUF1 low.
...
- Current local-LZ decoder inline code-headroom promotion — the standalone
...
- Current D4 decoder inline code-headroom promotion — the standalone
