...
compact-FGP3/v4 follow-up, the ACTIVITY9 high compact-FGP3/v4 follow-up, and
the WALKSTUF3 low compact-FGP3/v4 follow-up, the current public-capped rollup
is `+0.4412%` over target / `99.5774%` target
speed across all 126 timing-bearing rows; the raw signed CSV records
`-0.3273%` over target / `100.3612%` target speed for
...
`walkstuf3-low-compact-fgp3-v171`: it converts WALKSTUF3 low to padded compact
FGP3/v4 restore-minus-current data inside the original `994669` byte footprint.
Low improves `2321/2293 -> 2310/2295`, cuts overrun `28 -> 15`, blocking
`41 -> 26`, hidden refill `21 -> 17`, loop reads `72 -> 29`, loop-read time
`359 -> 150`, and due misses `5 -> 2`; pack LBA stays fixed at `26906`,
sectors stay `486`, and the `215040` byte PS-EXE bucket is unchanged. The
previous ACTIVITY9 high compact-FGP3/v4 pass converts high to padded compact
...
The current public-capped rollup is `+0.4412%` over target / `99.5774%` target
speed; the raw signed optimization matrix is `-0.3273%` / `100.3612%`. The next
true rows are VISITOR3 low/high, WALKSTUF1 high/low, BUILDING2 high/low,
VISITOR5 low, JOHNNY1 high/low, BUILDING4 low, BUILDING6 high/low, ACTIVITY9
...
`2310/2295`.
The public-capped timing-bearing average is now `+0.4412%` over target /
`99.5774%` target speed after the WALKSTUF3 low compact-FGP3/v4 promotion.
The raw signed optimization matrix is `-0.3273%` / `100.3612%`. The latest
...
misses `5 -> 2` while preserving the `994669` byte pack footprint, fixed LBA
`26906`, and the `215040` byte PS-EXE bucket. The prior ACTIVITY9 high pass
