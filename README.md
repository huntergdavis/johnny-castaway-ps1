...
The latest VISITOR3 follow-up, `visitor3-high-reanchor-f127-v202`, promotes a
high-only frame `127` re-anchor after the v201 both-tide frame `125` retry
proved low tide remains phase-sensitive. It is a work-reduction/CD-pressure
win, not a rollup-speed win: high remains `1101/1030` with overrun `71`, while
active payload drops `542729 -> 527206`, blocking moves `116 -> 114`, and
loop-read VBlanks move `118 -> 116`. Low remains exact-flat at `1108/1028`,
and the broad canaries keep fixed LBAs plus the `215040` byte PS-EXE bucket.
The current source baseline remains `visitor3-drop-unused-motion-dispatch-v197`,
which removed unused noncompact motion-marker dispatch for code-layout
headroom. The latest rejected v198-v201 probes close runtime/staged frame
`133` narrow upload and both-tide frame `125` re-anchor under the current
ownership model; future VISITOR3 work should stay in generated pack/sidecar
data, custom compression, or scheduler ownership rather than VSync scratch-copy
uploads.
...
`visitor3-high-reanchor-f127-v202`: it keeps the v181 VISITOR3 frames
...
`117` target-hull payload plus a high-only frame `127` re-anchor. Loop timing
is `1101/1030` high and `1108/1028` low while blocking drops to `114/139`,
loop-read time drops to `116/148`, and due misses are `21/25`; v197 remains
the current source-code headroom baseline, and v202 is the current high-pack
CD-pressure baseline. The
...
target-hull timing promotion plus the later v197 code-headroom and v202
high frame-127 CD-pressure passes.
The raw signed optimization matrix is `-0.3644%` / `100.3935%`.
The latest VISITOR3 pass is a high-only CD-pressure promotion: frame `127` is
re-anchored inside `VISITOR3.FG2`, preserving high loop timing at `1101/1030`
while cutting active payload `542729 -> 527206`, blocking `116 -> 114`, and
loop-read time `118 -> 116`; low remains `1108/1028`. The latest rejected
v183-v201 VISITOR3 probes close low precursor motion-copy frames `114..118`, a
...
and both-tide frame `125` re-anchor; future
