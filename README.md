...
The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest W1-high promotion gate is `scratch/ps1-perf-iterate/w1high-seg4-transient383-399-cap64-under-yellow-canary-20260523/20260523-001921-1229513/summary.json`. Public rollup is `+0.2073%` over target / `99.7948%` target speed and raw signed rollup is about `-0.5096%` / `100.5251%`; bands are `123` green, `3` yellow, `0` orange, and `0` red. That is about `17.19` public over-target points removed and `12.69` public target-speed points added since the compact full-matrix baseline.

The newest WALKSTUF1-high setup-residency refinement adds a high-tide
`383..399` setup slice in `MEM_REGION_TRANSIENT` and caps W1-high clean-rect
capture at `64 KiB`, preserving pixels and the `233472` byte PS-EXE bucket.
The under-yellow canary keeps all three yellow rows valid while W1-high
improves `1807/1468/1442 -> 1816/1467/1442`, overrun `26 -> 25`, blocking
`39 -> 37`, refill `9 -> 8`, and loop reads `40 -> 35`. The same current
canary measures VISITOR3 high at `1386/1065/1046`, blocking/refill `34/1`,
and VISITOR3 low at `1380/1063/1043`, blocking/refill `46/1`.
