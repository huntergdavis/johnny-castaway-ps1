...
| | |
|---|---|
| Current release | **`v1.0.0-ps1`** — **the 1.0.** All 63 scenes pixel-perfect with synced SFX, all 126 perf rows green, day-long zero-crash soaks — and the entire runtime hardened against **real launch-era hardware** (worn drive, burned CD-Rs, tonyhax chainload) through a twenty-burntest campaign: address-verified CD streaming, bounded waits with GPU self-heal, an I/O-never-halts policy, and drive-gentle read patterns throughout. The lessons are written down in [`docs/ps1/real-hardware-testing-lessons.md`](docs/ps1/real-hardware-testing-lessons.md). |
...
**The v0.9.3 release promotes SPU-backed async staging.** Unused SPU RAM is
now a cold cache for next-scene metadata/payload windows, walk PSBs, and the
walk clean buffer. The foreground pilot starts aligned non-blocking CD reads
before transition boundaries, polls them during slack, drains them before
blocking CD work, and DMA-copies parked data back from SPU RAM before CPU/GPU
use. The Original-order soak reached roughly 293 picks with no `JCBSOD`,
fatal, invalid-read, or `JCWALK` allocation-failure markers.

**The v0.9.2 longevity release fixed the two extended-soak failure modes.**
CD file searches drain the prior read/pause path before `CdSearchFile()`,
closing the rare DuckStation `CdlSetloc` zero-parameter failure during scene
transitions. VISITOR3-low also drops its clean-relief stream window while
VISITOR3-high keeps the reduced high-tide relief window, removing the 48 KB
allocation that made the final low-tide clean strip depend on CACHE/libc
fragmentation luck after many successful plays.

**🎉 The newest floor-lift multi-scene promotion applies the pre-loop
double-preload pattern to WALKSTUF1-low, BUILDING2-high, and BUILDING4
high/low.** Each gated by scene id or scene-name string check, reusing
the `fgRuntimePresentFirstFrameNoWait` / `fgRuntimePresentNextStagedAndAdvance`
helpers already in the binary. PS-EXE bucket stays `235520`.
- WALKSTUF1-low: `1461/1447 -> 1452/1448`, overrun `14 -> 4`, target
  speed `99.04% -> 99.72%` (+0.68pp)
- BUILDING2-high: `1330/1317 -> 1319/1316`, overrun `13 -> 3`, target
  speed `99.02% -> 99.77%` (+0.75pp)
- BUILDING4-high: `2843/2816 -> 2832/2816`, overrun `27 -> 16`, target
  speed `99.05% -> 99.43%` (+0.38pp)
- BUILDING4-low: `2847/2820 -> 2833/2822`, overrun `27 -> 11`, target
  speed `99.05% -> 99.61%` (+0.56pp)

All other rows stay exact-flat at their committed baselines. Pack LBAs
unchanged.

**The prior JOHNNY6 double-preload promotion lifts the floor by
moving johnny6 high/low (previously the lowest green at 99.011%) up
to 99.43%.** It adds johnny6-specific pre-loop frame0+1 preload via
the existing `fgRuntimePresentFirstFrameNoWait` and
`fgRuntimePresentNextStagedAndAdvance` helpers, gated by
`fgSceneEquals(sceneName, "johnny6")` to avoid adding a new enum.
johnny6 high improves `2830/2802 -> 2817/2801`, overrun `28 -> 16`,
target speed `99.011% -> 99.432%`; johnny6 low improves
`2830/2802 -> 2818/2802`, overrun `28 -> 16`, target speed
`99.011% -> 99.432%`. V3-high/low and W1-high stay exact-flat at
their committed baselines. PS-EXE bucket `235520` unchanged, pack
LBAs unchanged.

**The prior VISITOR3-high double-preload + phase-2 promotion moves
V3-high — the LAST yellow row — into green at 99.34% target speed.** V3-high
adds a pre-loop frame0+1 preload via the same helpers introduced in the
V3-low double-preload commit, and retunes V3-high pre-loop VSync wait
`4 -> 2` (the new sweet spot on the layout-shifted baseline). V3-high
improves `1386/1063/1046 -> 1374/1053/1046`, overrun `17 -> 7`, target
speed `98.40% -> 99.34%`. W1-high stays exact-flat at `1816/1458/1444`,
overrun `14`, target speed `99.04%`. V3-low stays exact-flat at
`1377/1052/1044`, overrun `8`, target speed `99.24%`. Pack LBA `22620`
and PS-EXE bucket `235520` unchanged.

**The prior VISITOR3-low double-preload + VISITOR3-high phase-4 retime
moves V3-low into green at 99.24% target speed.** It adds a VISITOR3-low
frame0 and frame1 pre-loop preload via two new size-optimized helpers
(`fgRuntimePresentFirstFrameNoWait` and `fgRuntimePresentNextStagedAndAdvance`)
and re-tunes V3-high pre-loop from `1` VSync to `4` VSyncs to recover
the layout shift on V3-high. V3-low improves `1386/1062/1045 -> 1377/1052/1044`,
overrun `17 -> 8`, target speed `98.40% -> 99.24%`. W1-high stays green
at `1815/1458/1444`, overrun `14`, target speed `99.04%` (was `99.18%`).
V3-high stays yellow at `1386/1063/1046`, overrun `17`, target speed
`98.40%` (was `98.49%`). This is an explicit layout-changing release: PS-EXE
bucket grows from `233472` to `235520` (1 sector), and pack LBAs shift
by 1 sector. Net `+1` green band.

**The prior WALKSTUF1-high double-preload-3-wait promotion moves W1-high
into green at 99.176% target speed.** It pre-presents both frame 0 (via
the existing helper) and frame 1 (via inline code) before the measured
loop starts, with 3 held VBlank waits before the sceneClockTick reset.
That shifts another 4 VBlanks of work from the measured loop into
pre-loop. W1-high improves `1815/1461/1445 -> 1815/1457/1445`, overrun
`16 -> 12`, target speed `98.905% -> 99.176%`. V3-high stays exact-flat
at `1381/1062/1046` and V3-low stays exact-flat at `1386/1062/1045`.
Pack LBA `24891` and PS-EXE bucket `233472` unchanged.

The prior WALKSTUF1-high helper-3-wait retime extends the W1-high frame0
helper from `2` to `3` held VBlank waits, the same phase-alignment mechanism
the VISITOR3-high phase-1 promotion used: 1 VBlank of pre-loop wait shifts
from the measured loop into the pre-loop helper. It improves W1-high
`1815/1462/1445 -> 1815/1461/1445`, overrun `17 -> 16`, target speed
`98.837% -> 98.905%`, while V3-high stays exact-flat at `1381/1062/1046` and
V3-low stays exact-flat at `1386/1062/1045`. Pack LBA `24891` and PS-EXE
bucket `233472` unchanged.

The prior VISITOR3-high pre-loop phase-1 retime shortens the V3-high pre-loop
VSync wait from `3` to `1` VBlank, the same phase-alignment mechanism the
accepted W1-high frame0-consume promotion used on a different scene. It
improves V3-high `1386/1065/1046 -> 1381/1062/1046`, overrun `19 -> 16`,
blocking `34 -> 32`, refill `1 -> 0`, target speed `98.215% -> 98.493%`, while
W1-high stays exact-flat at `1815/1462/1445` and V3-low stays exact-flat at
`1386/1062/1045`. Pack LBA `22619` and PS-EXE bucket `233472` unchanged.

The prior WALKSTUF1-high frame0 consume speed promotion presents and consumes
frame0 before the measured loop, then applies a four-VBlank high-tide phase
retime while preserving pixels, pack LBA, and the `233472` byte PS-EXE bucket.
It improves W1-high `1815/1466/1445 -> 1815/1462/1445`, overrun `21 -> 17`,
with blocking/refill unchanged at `35/5`, reads/due unchanged at `36/6`, and
VISITOR3 high/low exact-flat in the same under-yellow canary.

The prior WALKSTUF1-high previous-visible cleanup pass compacts late frames
`200..215` while preserving pixels, pack footprint, LBA, and timing. It keeps
W1-high exact-flat at `1815/1466/1445`, overrun `21`, blocking/refill `35/5`,
and reads/due `36/6`, while shrinking active payload `833386 -> 829510`,
cleanup spans/pixels/restore bytes `1445/14728/29456 -> 224/893/1786`, and
runtime dirty rows `28345 -> 28303`; VISITOR3 high/low stay exact-flat in the
same under-yellow canary.

The newest VISITOR3-low phase/segment promotion combines a one-VBlank low-tide
phase retime with a widened setup segment `38..79`. The focused proof and
under-yellow canary preserve pixels, pack LBA, and the `233472` byte PS-EXE
bucket while VISITOR3-low improves `1380/1063/1043 -> 1386/1062/1045`,
overrun `20 -> 17`, blocking/refill `46/1 -> 38/0`, loop reads `17 -> 12`,
read time `89 -> 72`, and due misses `8 -> 6`; VISITOR3-high and W1-high stay
exact-flat in the same canary.

The newest WALKSTUF1-high prep2 frame-gate promotion allows two-VBlank staged
frame prepare only for high-tide W1 displayed frames `128..191`, preserving
pixels, pack LBA, and the `233472` byte PS-EXE bucket. The under-yellow canary
keeps VISITOR3 high/low exact-flat while W1-high improves
`1816/1467/1443 -> 1815/1466/1445`, overrun `24 -> 21`,
blocking/refill `37/8 -> 35/5`, loop reads `37 -> 36`, and due misses stay
`6`. The prior `185..191` direct-stage rescue and `383..399` setup slice made
this narrower prepare gate safe instead of paying the earlier refill debt.

The prior WALKSTUF1-high fresh-owner retarget promotion pulls the high-tide
`372..388` sector window earlier for owner frames `183..188`, without changing
pixels, pack LBA, or the `233472` byte PS-EXE bucket. The canonical four-row
gate passes with BUILDING2 high and VISITOR3 high/low exact-flat while W1-high
improves `1809/1470/1442 -> 1807/1468/1442`, overrun `28 -> 26`,
blocking `40 -> 39`, reads `40 -> 39`, and target speed
`98.0952% -> 98.2289%`.

The prior WALKSTUF1-high screen-clip headroom promotion clips entry `134` /
source frame `242` in `WALKSTUF1.FG2`. It preserves the `1535263` byte pack
footprint, entry sizes, offsets, LBA `24891`, sectors `750`, and PS-EXE
...
`17175040 -> 17172480`; BUILDING2 high and VISITOR3 high/low stay exact-flat.

The prior WALKSTUF1-high CD-pressure promotion adds the `{108..124}` high-tide
read group on top of the accepted `62..66` clip and `{92..108}`/`{272..284}`
read stack. It preserves W1-high timing at `1808/1469/1441`, overrun `28`,
...
`214/684/17207040 -> 213/682/17175040`. BUILDING2 high and VISITOR3 high/low
stay exact-flat.

The prior WALKSTUF1-high speed promotion clips exact entries `62..66` in
`WALKSTUF1.FG2` and adds the now-safe `{92..108}` plus `{272..284}` read
groups. It preserves the `1535263` byte pack footprint, entry sizes, offsets,
LBA `24891`, sectors `750`, and `233472` byte PS-EXE bucket. The pack clip
drops selected logical payload `23002 -> 19866`, removing `3290` cleanup
pixels and `1904` draw pixels. The four-yellow canary improves W1-high from
`1808/1469/1440` to `1808/1469/1441`, overrun `29 -> 28`, blocking/refill
`42/12 -> 41/11`, reads/due `41/7 -> 37/6`, and target speed
`98.026% -> 98.094%`; BUILDING2 high and VISITOR3 high/low stay exact-flat.

The prior WALKSTUF1-high speed promotion trims fixed-offset entry tails for
...
- `v0.9.3-ps1` — SPU async staging and Original-order soak release. Unused
  ...
  now uses aligned non-blocking CD reads and drains them before any later
  blocking read or buffer mutation. The long visible Original-order soak
  reached roughly 293 picks with no `JCBSOD`, fatal, invalid-read, or `JCWALK`
  allocation-failure markers.
- `v0.9.2-ps1` — long-run CD and VISITOR3-low stability release. CD file
  lookups now go through `ps1_cdSearchFileQuiesced()` so async `CdlPause`
  completion cannot race later directory-walk `CdlSetloc` calls, and
  VISITOR3-low no longer keeps the clean-relief stream window that caused the
  multiday CACHE/libc fragmentation cliff. The targeted VISITOR3-low run
  confirmed `no-prefetch` and no `fg-stream-window`; the follow-up randomized
  soak was intentionally stopped for release after about 47.5 hours with no
  `JCBSOD`, `JCSKIP`, `CACHE exhausted`, or DuckStation `Incorrect parameters`
  markers.
- `v0.9.1-ps1` — longevity testing improvements. Raises the CACHE budget by
  32 KB, wraps long BSOD reasons, skips BOOT verification when the BOOT budget
  is zero, and was verified across a 6-hour 126-scene headless soak with 0
  BSODs.
...
- Current WALKSTUF1-high frame0 consume speed promotion - W1-high now presents
  and consumes frame0 before the measured loop, uses a four-VBlank high-tide
  phase offset, and compile-gates optional `printf-test` breadcrumbs to keep
  the PS-EXE in the fixed `233472` byte bucket. The under-yellow canary at
  `scratch/ps1-perf-iterate/w1high-frame0-consume-phase4-under-yellow-canary-20260523/20260523-065754-4032041/summary.json`
  improves W1-high `1815/1466/1445 -> 1815/1462/1445`, overrun `21 -> 17`,
  while blocking/refill stay `35/5`, reads/due stay `36/6`, and VISITOR3
  high/low stay exact-flat. Public rollup improves to `+0.2006%` over target /
  `99.8013%` target speed; raw signed rollup is `-0.5163%` / `100.5317%`;
  bands remain `126` green / `0` yellow.
...
**Raw doc source** — [`docs/ps1/`](docs/ps1/) on GitHub. Notable entry points: [`scene-status.md`](docs/ps1/scene-status.md) (per-scene ledger), [`current-status.md`](docs/ps1/current-status.md) (narrative), [`development-workflow.md`](docs/ps1/development-workflow.md) (per-scene loop), [`final-under-99-speed-handoff.md`](docs/ps1/final-under-99-speed-handoff.md) (handoff for the last three under-99 speed rows), [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) + [`performance-experiment-log.md`](docs/ps1/performance-experiment-log.md) (perf source of truth), [`release-notes-*.md`](docs/ps1/) (per-release notes), [`research/`](docs/ps1/research/) (dated design logs ↔ `/devlog/`), [`archaeology/`](docs/ps1/archaeology/) (timeline, retired tools, vision-artifacts).
