...
conversion, plus the follow-up VISITOR3 high 16-sector group `170..186` and
the VISITOR3-only `192 KiB` setup-prime resident cap. VISITOR3 high now trims
blocking `361 -> 355`, prefetch overrun `21 -> 14`, and loop reads `52 -> 45`;
VISITOR3 low trims blocking `365 -> 362`, prefetch overrun `23 -> 19`, and
loop reads `51 -> 50`. The timing-bearing average is now `+0.8556%` over
target / `99.4595%` target speed; since the compact full-matrix baseline was about
