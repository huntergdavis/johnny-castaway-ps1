...
The latest rejected v183 VISITOR3 probes close low precursor motion-copy frames
`114..118` and the C-side fastspan copy path: the pack rewrite saved bytes but
regressed low timing, and the runtime fast path crossed the PS-EXE bucket.
...
speed; the raw signed optimization matrix is `-0.3620%` / `100.3915%`. The next
...
remains on the v181 motion-copy row at `1108/1028`. The latest rejected v183
VISITOR3 probes close low precursor motion-copy frames `114..118` and a
runtime fastspan copy path; future VISITOR3 work is now focused on custom
data-shape/dictionary/precomposed-strip ideas rather than wider precursor
motion windows. The prior ACTIVITY9 low pass moves `2085/2058 ->
