...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.2481% over target / 99.7560% target speed**. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
The mainline shifted from "prove every scene" to **performance polish, stability, and content** at `v0.7.0-ps1`. Current mainline builds on `v0.8.14-ps1` with post-release perf baselines through `building2-high-setupseg86-242-v979`: W1-low stays on `walkstuf1-low-rg209-225-v935`, JOHNNY1 high/low remain green after the local-LZ payload swing, BUILDING2 high now primes relative sectors `86..242` during setup and improves to `1349/1315`, overrun `34`, blocking/refill `48/12`, reads/read time `25/117`, BUILDING4 low compresses entries `30`, `33`, and `270` in-place (`4650 -> 679`, `6656 -> 2761`, `8287 -> 8167`; active payload `807263 -> 799277`) and holds `2851/2815`, overrun `36`, blocking/refill `42/35`, and due `1`, and VISITOR3 high/low remain on the v629/v510 pack-shape baselines. Public rollup is `+0.2481%` over target / `99.7560%` target speed and raw signed rollup is about `-0.5190%` / `100.5383%`; bands are `119` green and `7` yellow. That is about `17.15` public over-target points removed and `12.66` public target-speed points added since the compact full-matrix baseline.
