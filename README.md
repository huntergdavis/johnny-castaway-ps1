...
The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest promotion gate is `scratch/ps1-perf-iterate/johnny6-double-preload-canary-20260523-142657/20260523-142657-2752035/summary.json`. Public rollup is `+0.1762%` over target / `99.8252%` target speed and raw signed rollup is `-0.5407%` / `100.5555%`; bands are **`126` green, `0` yellow, `0` orange, and `0` red — every routed/timing-bearing row is at or above 99% target speed**. That is about `17.22` public over-target points removed and `12.73` public target-speed points added since the compact full-matrix baseline.

**🎉 The newest JOHNNY6 double-preload promotion lifts the floor by
