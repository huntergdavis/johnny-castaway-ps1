...
high setup-prime expansion, the current public-capped rollup is `+0.3783%`
over target / `99.6326%` target speed
across all 126 timing-bearing rows; the raw signed CSV records
`-0.3903%` over target / `100.4164%` target speed for
internal optimization analysis.

The latest VISITOR3 follow-up, `visitor3-low-dual-segment150-174-slack4-v216`,
adds a second low-tide setup segment for sectors `150..174` and raises only
VISITOR3-low window-refill eligibility to `4` VBlanks. It improves low active
timing `1102/1032 -> 1098/1034`, cuts overrun `70 -> 64`, blocking `124 -> 112`,
loop reads `23 -> 20`, loop-read time `131 -> 119`, and due misses `22 -> 19`
...
tides keep fixed LBAs plus the `215040` byte PS-EXE bucket.
...
`visitor3-high-setup-prime320-v214` plus `visitor3-low-dual-segment150-174-slack4-v216`:
...
a guarded low-only second segment for sectors `150..174`.
Loop timing is `1089/1035` high and `1098/1034` low while blocking drops to
`83/112`, loop-read time drops to `83/119`, and due misses are `15/19`; v214
high plus v216 low is the current runtime/CD-pressure baseline. The
...
The current public-capped rollup is `+0.3783%` over target / `99.6326%` target
speed; the raw signed optimization matrix is `-0.3903%` / `100.4164%`. The next
...
`2075/2061`.
The public-capped timing-bearing average is now `+0.3783%` over target /
`99.6326%` target speed after the VISITOR3 high-only sparse frame-117
...
v213/v214 high setup-prime cap expansions, and the v216 guarded low second
setup segment.
The raw signed optimization matrix is `-0.3903%` / `100.4164%`.
The latest VISITOR3 pass is the v216 guarded low second-segment promotion:
it moves low to `1098/1034`, cutting overrun `70 -> 64`, blocking `124 -> 112`,
loop-read time `131 -> 119`, loop reads `23 -> 20`, and due misses `22 -> 19`;
