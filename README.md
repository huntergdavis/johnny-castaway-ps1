...
The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest W1-high promotion gate is `scratch/ps1-perf-iterate/w1high-direct-late-189-191-four-yellow-norequire-current-20260522/20260522-203120-3922960/summary.json`. Public rollup is `+0.2144%` over target / `99.7881%` target speed and raw signed rollup is about `-0.5024%` / `100.5184%`; bands are `123` green, `3` yellow, `0` orange, and `0` red. That is about `17.19` public over-target points removed and `12.69` public target-speed points added since the compact full-matrix baseline.

The newest WALKSTUF1-high direct-stage headroom promotion lets high-tide
frames `189..191` use the small-payload direct-stage path when held slack is
above the normal window minimum. The verified W1-high gate passes with
`1809/1470/1442`, overrun `28`, improving blocking/refill `41/11 -> 40/10`
and nudging target speed `98.0939% -> 98.0952%`; reads/read time move
`36/191 -> 40/206`, so this is a bounded CD-headroom tradeoff rather than the
final 99% breakthrough.
