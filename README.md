...
The mainline shifted from "prove every scene" to **performance polish, stability, and content** at `v0.7.0-ps1`. Current post-release mainline win: VISITOR5 low now mirrors the accepted high-tide `30..46` retained-read shape, improving to `1102/1097`, overrun `5`, blocking/refill `5`, reads/due `18/0`, with public rollup `+0.2786%` over target / `99.7262%` target speed and raw signed rollup `-0.4886%` / `100.5086%`. Latest mainline CD-work cleanup adds WALKSTUF1 shared `427..443`, keeping low timing flat while cutting loop reads `64 -> 62`; the latest logs close the remaining WALKSTUF1 scalar/scheduler branches, compact-origin rebase, preserve-offset draw-gap coalescing, and low-window retuning lanes, plus the BUILDING2 low physical gap-compaction lane, as phase-negative, inert, or zero-savings.

Recent releases:

- Current mainline after `v0.8.8-ps1` — VISITOR5 low now uses the low-tide `30..46` retained-read group, improving from `1104/1092` to `1102/1097` and moving that row into green. WALKSTUF1 low also gained a same-speed shared `427..443` CD-work row, dropping loop reads `64 -> 62`; subsequent WALKSTUF1 and BUILDING2 low closures kept the rollup unchanged and pushed those rows toward generated ownership, semantic row/strip pack compression, or targeted pack-side work. Bands remain `117` green and `9` yellow.
