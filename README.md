...
The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest five-yellow canary refresh is `scratch/ps1-perf-iterate/b2high-phase1-five-yellow-noreq-current/20260521-233813-912248/summary.json`. Public rollup is `+0.2340%` over target / `99.7691%` target speed and raw signed rollup is about `-0.4829%` / `100.4994%`; bands are `121` green, `5` yellow, `0` orange, and `0` red. That is about `17.17` public over-target points removed and `12.67` public target-speed points added since the compact full-matrix baseline.

The newest speed promotion adds a one-VBlank BUILDING2-high phase offset before the measured loop. The five-yellow canary keeps all pack LBAs and the `233472` byte PS-EXE bucket fixed while BUILDING2-high improves from `1583/1341/1313` to `1583/1340/1314`, cutting loop overrun `28 -> 26`, blocking `47 -> 45`, and hidden refill overrun `14 -> 12`; VISITOR3 high/low and WALKSTUF1 high/low stay exact-flat. The current under-green queue is VISITOR3 low, WALKSTUF1 high, VISITOR3 high, BUILDING2 high, and WALKSTUF1 low.
