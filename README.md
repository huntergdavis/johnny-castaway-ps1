...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.2561% over target / 99.7477% target speed** after the VISITOR3 high `56 KiB` tight-refill cap promotion. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
The mainline shifted from "prove every scene" to **performance polish, stability, and content** at `v0.7.0-ps1`. Current mainline builds on `v0.8.16-ps1` with the new memory-region allocator promoted: BOOT allocations seal after startup, CACHE allocations use free-list/LRU reuse for long-lived resource data, and TRANSIENT scene allocations can be wiped between major scene loads instead of relying on a fragmented general heap. The allocator merge also adds pack-header metrics and CI gates requiring `MEM_REGION_RATIONALE` annotations at every `memAlloc` call site. The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest focused allocator-era checkpoint refreshes the under-green VISITOR3, WALKSTUF1, BUILDING2, and BUILDING4 rows. Public rollup is `+0.2561%` over target / `99.7477%` target speed and raw signed rollup is about `-0.4608%` / `100.4780%`; bands are `119` green, `7` yellow, `0` orange, and `0` red. That is about `17.14` public over-target points removed and `12.65` public target-speed points added since the compact full-matrix baseline. The newest speed win lowers VISITOR3 high's tight-refill cap to `56 KiB`, improving `1079/1043 -> 1075/1043`, overrun `36 -> 32`, blocking `49 -> 46`, and hidden refill `5 -> 3` while canaries stayed exact-flat. The prior same-overrun win adds WALKSTUF1 high's `{149,165}` retained read-pressure row, keeping `1475/1441` and overrun `34` flat while cutting blocking `59 -> 57`, reads `46 -> 44`, and read time `212 -> 199`.
