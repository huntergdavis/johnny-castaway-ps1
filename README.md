...
The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest four-yellow canary refresh is `scratch/ps1-perf-iterate/v3low-seg4-add55-79-cache-phase0-four-yellow-current-20260522/20260522-083527-3978849/summary.json`. Public rollup is `+0.2233%` over target / `99.7793%` target speed and raw signed rollup is about `-0.4936%` / `100.5097%`; bands are `122` green, `4` yellow, `0` orange, and `0` red. That is about `17.18` public over-target points removed and `12.68` public target-speed points added since the compact full-matrix baseline.
