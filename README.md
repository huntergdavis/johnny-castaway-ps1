...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.3685% over target / 99.6457% target speed** after the VISITOR3 stage1-under-clean-relief promotion. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
- Current allocator-era optimization checkpoint — VISITOR3 high improves from `1232/1033` to `1135/1035` and VISITOR3 low from `1231/1040` to `1110/1040` by retaining only the tiny stage1 prefetch frame buffer under clean-memory relief. The prior targeted dual setup segments moved WALKSTUF1 high from `1509/1425` to `1489/1430` and WALKSTUF1 low from `1507/1426` to `1477/1434`, while BUILDING2 high uses allocator-safe CACHE setup slices at `3..35` and `202..242`.
