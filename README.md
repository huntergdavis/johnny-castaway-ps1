...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.3010% over target / 99.7044% target speed**. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
- `main` after `v0.8.7-ps1` — BUILDING2 low stores frames `71` and `77` as previous-frame D4 deltas, improving to `1349/1320`, overrun `29`, blocking `70`, prefetch overrun `0`, reads/due `52/15`; VISITOR3 high now stores frames `132` and `137` as D4 deltas and preloads frame `132` through a one-sector high setup segment, improving to `1067/1039`, overrun `28`, blocking `45`, reads/due `8/8`, hidden refill `0`; VISITOR3 low now stores frame `132` as a second D4 delta after frame `129`, keeping `1068/1041`, overrun `27`, reads/due `9/9`, and cutting blocking/read time `51 -> 50`; the latest WALKSTUF1 high current-control refresh records `1476/1434`, overrun `42`, blocking `81`, refill `23`. Public rollup is `+0.3010%` over target / `99.7044%` target speed; WALKSTUF1 low, BUILDING2 high, WALKSTUF1 high, VISITOR3 high, and VISITOR3 low remain the top under-99 rows.
