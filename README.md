...
The mainline shifted from "prove every scene" to **performance polish, stability, and content** at `v0.7.0-ps1`. Current mainline builds on `v0.8.14-ps1` with post-release perf baselines through `building4-low-local-lz-entry30-v953`: W1-low stays on `walkstuf1-low-rg209-225-v935`, JOHNNY1 high/low remain green after the local-LZ payload swing, BUILDING2 high remains on `building2-high-frame100-inplace-v926`, BUILDING4 low now compresses entry `30` / source frame `54` in-place (`4650 -> 679` bytes, active payload `807263 -> 803292`) while staying exact-flat to current control at `3129/2853/2815`, and VISITOR3 high/low remain on the v629/v510 pack-shape baselines. Public rollup is `+0.2523%` over target / `99.7519%` target speed and raw signed rollup is about `-0.5148%` / `100.5342%`; bands are `119` green and `7` yellow. That is about `17.15` public over-target points removed and `12.65` public target-speed points added since the compact full-matrix baseline.
