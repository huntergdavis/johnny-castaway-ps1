...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.3196% over target / 99.6867% target speed**. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
- `main` after `v0.8.7-ps1` — BUILDING2 high now keeps retained read groups `60..72`, `226..242`, and `249..257` on top of the BUILDING6 scene-local slack4 guard, JOHNNY6 compact-FGP3 metadata plus restore-minus-current promotion, WALKSTUF1 high `344..360`, `422..434`, and the shared dual-tail `443..455` / `444..456` rows, VISITOR3 low tail pack compaction, the WALKSTUF1 low staged-prepare fallback, and a current-layout VISITOR5 high refresh. Public rollup is now `+0.3196%` over target / `99.6867%` target speed; BUILDING2 high currently measures `1352/1311`, WALKSTUF1 high stays `1480/1432` with loop reads `65`, WALKSTUF1 low stays `1484/1431` while loop reads drop to `66`, and VISITOR5 high is `1107/1090`.
