...
The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest promotion gate is `scratch/ps1-perf-iterate/consolidated-helper-canary-20260523-161249/20260523-161249-3354286/summary.json`. Public rollup is `+0.1351%` over target / `99.8649%` target speed and raw signed rollup is `-0.5979%` / `100.5979%`; bands are **`126` green, `0` yellow, `0` orange, and `0` red — every routed/timing-bearing row is at or above 99% target speed**. That is about `17.26` public over-target points removed and `12.76` public target-speed points added since the compact full-matrix baseline.

**🎉 The newest consolidated double-preload promotion folds 11 scenes'
pre-loop double-preload logic into one shared helper and adds 4 new
scenes (fishing3, building6, activity9, suzy2) to the pattern.** A new
`fgRuntimeSceneNeedsDoublePreload()` helper replaces 5 pairs of
per-scene `if`-blocks with 2 call sites. ELF shrinks by `1 KB`
(`1034448 -> 1033400`), PS-EXE bucket stays `235520`, pack LBAs
unchanged.
- FISHING3-high: `1964/1947 -> 1962/1951`, overrun `17 -> 11`, target
  speed `99.13% -> 99.44%` (+0.31pp)
- BUILDING6-high: `2475/2454 -> 2466/2453`, overrun `21 -> 13`, target
  speed `99.15% -> 99.47%` (+0.33pp)
- ACTIVITY9-high: `2079/2062 -> 2059/2061`, overrun `17 -> 0`, target
  speed `99.18% -> 100.10%` (+0.92pp)
- ACTIVITY9-low: `2077/2061 -> 2055/2060`, overrun `16 -> 0`, target
  speed `99.23% -> 100.24%` (+1.01pp)
- SUZY2-high: `2652/2633 -> 2640/2632`, overrun `19 -> 8`, target
  speed `99.28% -> 99.70%` (+0.42pp)

All 8 previously-promoted scenes stay exact-flat at their committed
baselines. Net `+2.99pp` total target speed.

**The prior floor-lift multi-scene promotion applies the pre-loop
