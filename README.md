...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.2431% over target / 99.7603% target speed** after the WALKSTUF1 low frame132 payload trim promotion. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
The mainline shifted from "prove every scene" to **performance polish, stability, and content** at `v0.7.0-ps1`. Current mainline builds on `v0.8.16-ps1` with the new memory-region allocator promoted: BOOT allocations seal after startup, CACHE allocations use free-list/LRU reuse for long-lived resource data, and TRANSIENT scene allocations can be wiped between major scene loads instead of relying on a fragmented general heap. The allocator merge also adds pack-header metrics and CI gates requiring `MEM_REGION_RATIONALE` annotations at every `memAlloc` call site. The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest focused allocator-era checkpoint refreshes the under-green VISITOR3, WALKSTUF1, BUILDING2, and BUILDING4 rows. Public rollup is `+0.2431%` over target / `99.7603%` target speed and raw signed rollup is about `-0.4738%` / `100.4906%`; bands are `121` green, `5` yellow, `0` orange, and `0` red. That is about `17.22` public over-target points removed and `12.72` public target-speed points added since the compact full-matrix baseline. The newest speed win trims WALKSTUF1 low frame132's draw-tail payload without changing pack size/LBA, improving `1473/1447 -> 1470/1445`, overrun `26 -> 25`, blocking `41 -> 35`, loop reads `36 -> 31`, due `5 -> 4`, and target speed `98.235% -> 98.299%`. The prior WALKSTUF1 low win keeps the accepted `238..344` CACHE setup segment and adds a separate TRANSIENT `344..350` edge, improving target `1444 -> 1447`, overrun `29 -> 26`, blocking/refill `43/11 -> 41/7`, and target speed `98.031% -> 98.235%` while loop time stays `1473`.
