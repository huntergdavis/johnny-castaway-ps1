...
The mainline shifted from "prove every scene" to **performance polish, stability, and content** at `v0.7.0-ps1`. Current mainline builds on `v0.8.16-ps1` with the memory-region allocator promoted: BOOT allocations seal after startup, CACHE allocations use free-list/LRU reuse for long-lived resource data, and TRANSIENT scene allocations can be wiped between major scene loads instead of relying on a fragmented general heap. The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest five-yellow canary refresh is `scratch/ps1-perf-iterate/visitor3-prev-visible-minus62-promote-five-yellow/20260521-004059-1519648/summary.json`. Public rollup is `+0.2505%` over target / `99.7533%` target speed and raw signed rollup is about `-0.4664%` / `100.4837%`; bands are `121` green, `5` yellow, `0` orange, and `0` red. That is about `17.15` public over-target points removed and `12.65` public target-speed points added since the compact full-matrix baseline. The newest VISITOR3-high previous-visible cleanup-headroom pass changes `106` high-pack entries while excluding regressing entry `62`, reducing active payload `461631 -> 437785`, cleanup restore bytes `542088 -> 36092`, runtime restore bytes `471382 -> 56312`, and upload bytes `18785280 -> 18038400` with fixed pack footprint/LBA and flat `1082/1045` timing. The prior BUILDING2-high previous-visible cleanup pass reduces active high-pack payload `663590 -> 574094`, cleanup restore bytes `439186 -> 80522`, runtime restore bytes `438988 -> 116648`, and upload bytes `26753280 -> 24341120`, improving B2-high from `1357/1307` to `1343/1311` (`97.617%` target speed) and overrun `50 -> 32`. The current under-green queue remains BUILDING2 high, VISITOR3 low, VISITOR3 high, WALKSTUF1 high, and WALKSTUF1 low.
...
- Current VISITOR3-high cleanup-headroom promotion — `VISITOR3.FG2` now uses previous-visible cleanup compaction for every safe changed entry except entry `62`, which is logged as phase-negative. Active high-pack payload drops `461631 -> 437785`, cleanup restore bytes drop `542088 -> 36092`, runtime restore bytes drop `471382 -> 56312`, and upload bytes drop `18785280 -> 18038400`; VISITOR3-high stays `1082/1045`, overrun `37`, with fixed pack footprint/LBA, while the five-yellow canary stays clean.
- Current BUILDING2-high cleanup-speed promotion — `BUILDING2.FG2` now uses broad previous-visible cleanup compaction. Active high-pack payload drops `663590 -> 574094`, cleanup restore bytes drop `439186 -> 80522`, runtime restore bytes drop `438988 -> 116648`, and upload bytes drop `26753280 -> 24341120`; B2-high improves to `1343/1311` / `97.617%` target speed, overrun `32`, with fixed pack footprint/LBA.
