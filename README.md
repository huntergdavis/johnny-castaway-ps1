...
The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest W1-high promotion gate is `scratch/ps1-perf-iterate/w1high-layout-owner372-384-four-yellow-norequire-current-20260522/20260522-232432-869167/summary.json`. Public rollup is `+0.2133%` over target / `99.7891%` target speed and raw signed rollup is about `-0.5037%` / `100.5195%`; bands are `123` green, `3` yellow, `0` orange, and `0` red. That is about `17.19` public over-target points removed and `12.69` public target-speed points added since the compact full-matrix baseline.

The newest WALKSTUF1-high late-layout/owner refinement physically places
frames `183..199` contiguously in `WALKSTUF1.FG2` and narrows the high-tide
fresh-owner window to owner frames `183..187` / sectors `372..384`. The
four-row canary keeps BUILDING2 high and VISITOR3 high/low exact-flat and keeps
W1-high speed flat at `1807/1468/1442`, overrun `26`, blocking `39`, reads
`40`, and due `6`, while hidden refill improves `10 -> 9`. Direct-stage
`183..191` rescue remains closed for now: it reaches `1806/1467/1443`, overrun
`24`, and refill `8`, but regresses blocking/due to `42/7`.
