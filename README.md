...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.2492% over target / 99.7548% target speed**. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
The mainline shifted from "prove every scene" to **performance polish, stability, and content** at `v0.7.0-ps1`. Current mainline at `v0.8.14-ps1` promotes the JOHNNY1 local-LZ payload swing: both high and low compress the two full-frame entries inside the existing `448370` byte packs, keeping pack LBA/sectors and the `217088` byte PS-EXE bucket fixed while moving both rows from `1973/1945` to `1948/1945`, overrun `28 -> 3`, blocking/refill `25 -> 5`, loop read time `58 -> 37`, and target speed `98.56% -> 99.85%`. WALKSTUF1 low remains on `walkstuf1-low-frame106-inplace-v910`, BUILDING2 high remains on `building2-high-frame100-inplace-v926`, BUILDING4 low remains on `building4-low-frame425-inplace-v928`, and VISITOR3 high/low remain on the v629/v510 pack-shape baselines. Public rollup is `+0.2492%` over target / `99.7548%` target speed and raw signed rollup is `-0.5179%` / `100.5371%`; bands are now `119` green and `7` yellow. That is about `17.15` public over-target points removed and `12.65` public target-speed points added since the compact full-matrix baseline.
