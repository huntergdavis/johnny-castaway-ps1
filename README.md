...
Latest accepted headless performance work restored stage1/window prefetch for
`building4`, `building6`, and `johnny6` under clean-snapshot pressure, and
refreshed stale `activity1` rows against the current validated pack. The
timing-bearing average is now `+1.0270%` over target / `99.3047%` target
speed; since the compact full-matrix baseline was about `+17.4%` over target /
`87.1%` target speed, the headless methodology has removed about `16.4`
percentage points of over-target gap and added about `12.2` points of target
speed.
