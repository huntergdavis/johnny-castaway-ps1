...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.2610% over target / 99.7431% target speed** after the VISITOR3 high frame56/57 raw-gap plus tight-refill promotion. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
The mainline shifted from "prove every scene" to **performance polish, stability, and content** at `v0.7.0-ps1`. Current mainline builds on `v0.8.16-ps1` with the new memory-region allocator promoted: BOOT allocations seal after startup, CACHE allocations use free-list/LRU reuse for long-lived resource data, and TRANSIENT scene allocations can be wiped between major scene loads instead of relying on a fragmented general heap. The allocator merge also adds pack-header metrics and CI gates requiring `MEM_REGION_RATIONALE` annotations at every `memAlloc` call site. The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest focused allocator-era checkpoint refreshes the under-green VISITOR3, WALKSTUF1, BUILDING2, and BUILDING4 rows. Public rollup is `+0.2610%` over target / `99.7431%` target speed and raw signed rollup is about `-0.4560%` / `100.4734%`; bands are `119` green, `7` yellow, `0` orange, and `0` red. That is about `17.14` public over-target points removed and `12.64` public target-speed points added since the compact full-matrix baseline. The newest speed win moves VISITOR3 high frames `56` and `57` raw into the retained `228..262` gap and caps tight-slack speculative refill windows at `64 KiB`, improving high from `1082/1042` to `1079/1043`, overrun `40 -> 36`, blocking `50 -> 49`, loop reads `6 -> 4`, and read time `62 -> 57` while keeping refill/due flat at `5/3`. The prior speed win adds WALKSTUF1 low's `{91,107}` first-boundary read group after the low-only clean-rect/setup-edge promotion.
