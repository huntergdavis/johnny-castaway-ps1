...
| | |
|---|---|
| Current release | **`v0.8.12-ps1`** — WALKSTUF1 low frame77/frame130 payload trims |
...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.2697% over target / 99.7347% target speed**. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
The mainline shifted from "prove every scene" to **performance polish, stability, and content** at `v0.7.0-ps1`. Current mainline after `v0.8.12-ps1` keeps the v0.8.11 lazy CD/FG stream rollback and advances WALKSTUF1 low to `walkstuf1-low-frame106-inplace-v910`: the low-tide-only retained read group `394..410` plus fixed-sector frame87 shrink now measure `1769/1477/1432`, overrun `45`, loop reads/read time `58/259`, while blocking/refill/due stay bounded at `65/20/11`; the v849/v852/v853/v855/v859/v860/v861/v862/v863/v864/v865/v866/v867/v868/v869/v870/v871/v872/v873/v874/v875/v876/v908/v909/v910 in-place frame shrinks now cut active payload `879801 -> 790208` without moving pack offsets. BUILDING2 low now trims dead draw-tail payload in the existing pack footprint, improving to `1339/1317`, overrun `22`, blocking/read time `53`/`150`, reads `37`, and due `12` while preserving pack size/LBA and the PS-EXE bucket. BUILDING4 low clips offscreen PAL4 draw spans directly in the pack, improving to `2853/2816`, overrun `37`, blocking/read time `40`/`215`, and refill `34`; v913 now also shrinks frames `291`, `34`, `31`, `290`, `32`, `35`, `36`, `37`, `288`, `287`, `39`, `286`, `292`, `426`, `41`, `293`, `284`, and `283` in-place without moving offsets, keeping timing exact-flat while cutting active payload `855284 -> 808412`. BUILDING2 high carries same-speed offscreen work-volume clips for frames `89..92`, `94..104`, and `168..177`, keeping `1351/1311` flat while dropping runtime rows/spans/pixels to `18030/105645/446246`. WALKSTUF1 high/low also gained same-speed work-volume clips: high frames `194..210` plus frames `55`, `138`, `51`, `49`, `47`, `45`, `43`, `56`, `57`, `136`, `135`, and `139` drop runtime rows/spans/pixels to `16859/129919/731016` while staying exact-flat at `1476/1434` with loop reads/read time `63/276`; v839/v841/v842/v843/v844/v882/v884 then shrink high frames `51`, `49`, `47`, `45`, `43`, `138`, and `135` in place with preserved offsets, cutting active payload `882007 -> 860009` and staying exact-flat. Low carries isolated offscreen clips through v726 plus  no-shift frame `51`/`49`/`47`/`61`/`62`/`58`/`45`/`37`/`35`/`43`/`41`/`57`/`33`/`67`/`68`/`69`/`32`/`133`/`5`/`141`/`70`/`30`/`6`/`71`/`72`/`142`/`73`/`131`/`74`/`19`/`28`/`138`/`145`/`75`/`76`/`77`/`130`/`135`/`1`/`88`/`90`/`3`/`53`/`136`/`79`/`81`/`129`/`139`/`87`/`89`/`98`/`27`/`101`/`93`/`94`/`97`/`99`/`100`/`134`/`91`/`92`/`95`/`140`/`108`/`109`/`107`/`110`/`111`/`106` shrinks, with v859 converting a fixed-sector shrink into a one-VBlank overrun win and v860/v861/v862/v863/v864/v865/v866/v867/v868/v869/v870/v871/v872/v873/v874/v875/v876/v908/v909/v910 preserving that timing exact-flat; v760 restores bounded CD fast-polling after the release merge while preserving the long-soak timeout fallback. A fresh under-99 refresh also corrected the stale JOHNNY1 low row to `1973/1945`, matching high tide. Public rollup is `+0.2697%` over target / `99.7347%` target speed and raw signed rollup is `-0.4975%` / `100.5171%`. Earlier current release wins include VISITOR3 high's `277..293` tail-pack repack, VISITOR5 low's accepted high-tide `30..46` retained-read shape, and the WALKSTUF1 shared `427..443` same-speed CD-work cleanup.

Post-`v0.8.12-ps1` mainline also promotes BUILDING2 high preserve-offset payload trims through `building2-high-frame169-inplace-v896`: entry `172` / source frame `231` shrinks `1831 -> 851` bytes, entry `171` / source frame `228` shrinks `1980 -> 1025` bytes, entry `96` / source frame `119` shrinks `8781 -> 7944` bytes, entry `170` / source frame `226` shrinks `1683 -> 1186` bytes, entry `97` / source frame `121` shrinks `8718 -> 8258` bytes, entry `98` / source frame `123` shrinks `8876 -> 8637` bytes, entry `174` / source frame `239` shrinks `1625 -> 1460` bytes, entry `99` / source frame `126` shrinks `8843 -> 8728` bytes, entry `168` / source frame `219` shrinks `1372 -> 1266` bytes, and entry `169` / source frame `223` shrinks `1820 -> 1495` bytes. Active payload drops `674798 -> 670119`, and BUILDING2 high remains exact-flat at `1602/1351/1311`, overrun `40`, blocking/refill `54/18`, reads/read time `58/257`, and due `7`.
