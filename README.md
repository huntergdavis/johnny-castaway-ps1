...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.3253% over target / 99.6811% target speed**. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
- `v0.8.5-ps1` — full 126-row headless performance matrix baseline; current public rollup at `+0.3253%` over target / `99.6811%` target speed after the VISITOR3 low v292 no-op alias promotion.
