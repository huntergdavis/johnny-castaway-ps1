...
The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest four-yellow promotion gate is `scratch/ps1-perf-iterate/b2high-compact-four-yellow-current-20260522/20260522-115040-907307/summary.json`. Public rollup is `+0.2155%` over target / `99.7870%` target speed and raw signed rollup is about `-0.5014%` / `100.5173%`; bands are `123` green, `3` yellow, `0` orange, and `0` red. That is about `17.18` public over-target points removed and `12.69` public target-speed points added since the compact full-matrix baseline.

The newest BUILDING2-high speed promotion physically compacts
