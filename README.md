...
Post-`v0.6.12-ps1`, `VISITOR 4` and `VISITOR 5` are validated.
VISITOR4 is the coconut/plane gag in the current scene mapping, correcting
an old red-boat catalogue label. Both scenes use regenerated high/low packs
from the generic normal/far-left/far-right multi-view stitch; VISITOR5 also
uses scene-specific hold redistribution so the coconut impact and downed-plane
motion are readable without changing total scene duration.
