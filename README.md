...
| | |
|---|---|
| Current release | **`v0.8.14-ps1`** — JOHNNY1 local-LZ green promotion |
...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.2325% over target / 99.7709% target speed**. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
The mainline shifted from "prove every scene" to **performance polish, stability, and content** at `v0.7.0-ps1`. Current mainline builds on `v0.8.14-ps1` with post-release perf baselines through `walkstuf1-low-setupseg238-388-v989`: W1-low now primes relative sectors `238..388` during setup and improves to `1469/1447`, overrun `22`, blocking/refill `34/12`, reads/read time `28/145`, and due `4`; JOHNNY1 high/low remain green after the local-LZ payload swing; BUILDING2 high primes relative sectors `86..242` during setup and holds `1349/1315`, overrun `34`, blocking/refill `48/12`, reads/read time `25/117`; BUILDING4 low compresses entries `30`, `33`, and `270` in-place (`4650 -> 679`, `6656 -> 2761`, `8287 -> 8167`; active payload `807263 -> 799277`) and holds `2851/2815`, overrun `36`, blocking/refill `42/35`, and due `1`; VISITOR3 high/low remain on the v629/v510 pack-shape baselines. Public rollup is `+0.2325%` over target / `99.7709%` target speed and raw signed rollup is about `-0.5347%` / `100.5532%`; bands are `119` green and `7` yellow. That is about `17.17` public over-target points removed and `12.67` public target-speed points added since the compact full-matrix baseline.
