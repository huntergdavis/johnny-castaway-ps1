...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.3326% over target / 99.6742% target speed**. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
- `main` after `v0.8.7-ps1` — WALKSTUF1 high `213..229` read-group extension plus high-tide slack4 guard, with same-layout canary refreshes. Public rollup is `+0.3326%` over target / `99.6742%` target speed.
