...
The mainline shifted from "prove every scene" to **performance polish, stability, and content** at `v0.7.0-ps1`. Current post-release mainline win: BUILDING2 low now trims dead draw-tail payload in the existing pack footprint, improving to `1339/1317`, overrun `22`, blocking/read time `53`/`150`, reads `37`, and due `12` while preserving pack size/LBA and the PS-EXE bucket. BUILDING4 low clips offscreen PAL4 draw spans directly in the pack, improving to `2853/2816`, overrun `37`, blocking/read time `40`/`215`, and refill `34`; v746 now also shrinks frame `291` in-place without moving offsets, keeping timing exact-flat while cutting active payload `855284 -> 849109`. BUILDING2 high carries same-speed offscreen work-volume clips for frames `89..92`, `94..104`, and `168..177`, keeping `1351/1311` flat while dropping runtime rows/spans/pixels to `18030/105645/446246`. WALKSTUF1 high/low also gained same-speed work-volume clips: high frames `194..210` plus frames `55`, `138`, `51`, and `49` drop runtime rows/spans/pixels to `17052/132773/755038` while staying exact-flat at `1476/1434` with loop reads/read time `63/275`, and low carries isolated offscreen clips through v726 plus v747/v749/v750/v751/v753/v755/v756/v757/v759 no-shift frame `51`/`49`/`47`/`61`/`62`/`58`/`45`/`37`/`35` shrinks, keeping `1478/1431` flat while cutting active payload `879801 -> 844668`; v759 now reports low loop reads/read time `60/273`. A fresh under-99 refresh also corrected the stale JOHNNY1 low row to `1973/1945`, matching high tide. Public rollup is `+0.2708%` over target / `99.7337%` target speed and raw signed rollup is `-0.4963%` / `100.5160%`. Earlier current mainline wins include VISITOR3 high's `277..293` tail-pack repack, VISITOR5 low's accepted high-tide `30..46` retained-read shape, and the WALKSTUF1 shared `427..443` same-speed CD-work cleanup.

Recent releases:

- Current mainline after `v0.8.8-ps1` — VISITOR5 low uses the low-tide `30..46` retained-read group, improving from `1104/1092` to `1102/1097` and moving that row into green. WALKSTUF1 low gained the same-speed shared `427..443` CD-work row, then high/low late-tail and isolated low offscreen work-volume clips through v726, followed by v705 low late-tail physical compaction that cuts loop reads/read time `62/281 -> 60/274` on the clean current baseline while staying exact-flat at `1478/1431`; v747/v749/v750/v751/v753/v755/v756/v757/v759 now cut low frame51/frame49/frame47/frame61/frame62/frame58/frame45/frame37/frame35 active payload in-place while staying exact-flat. W1-high frames `55`, `138`, `51`, and `49` now clip safe offscreen draw spans and drop the high runtime work to `17052/132773/755038` while staying exact-flat at `1476/1434`. BUILDING2 low `218..229` slack8 first improves `1349/1320 -> 1344/1318`, and v739 draw-tail trimming now improves it to `1339/1317`, overrun `22`, blocking `53`, reads/read time `37/150`, and due `12`. BUILDING2 high now has exact-flat offscreen work-volume clips for frames `89..92`, `94..104`, and `168..177`. VISITOR3 high tail repack improves `1065/1039 -> 1063/1040`, overrun `26 -> 23`, blocking `41 -> 35`, and reads/due `7/7 -> 6/6`; BUILDING4 low offscreen pack clipping improves `2856/2816 -> 2853/2816`, overrun `40 -> 37`, blocking `44 -> 40`, and refill `37 -> 34`, then v746 cuts frame291 active payload in place while staying exact-flat. Bands remain `117` green and `9` yellow.
