...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; refreshed public-capped average is **+0.2480% over target / 99.7557% target speed** after the W1-low frame 87..99 fixed-layout cleanup-slack headroom pass, the W1-high cleanup-only fixed-layout screen-clip headroom pass, the VISITOR3-low fixed-layout previous-visible cleanup headroom pass, the W1-high exact-entry preserve-entry-size screen-clip headroom subset, the BUILDING2-high no-decode trim-draw-tail speed promotion and follow-up work-headroom subset, the W1-high cleanup-only preserve-entry-size screen-clip headroom promotion, the VISITOR3-low preserve-entry-size screen-clip headroom promotion, the W1-low preserve-entry-size screen-clip headroom promotion, the BUILDING2-high preserve-entry-size screen-clip promotion, the VISITOR3-high offscreen cleanup screen-clip headroom pass, the VISITOR3-high previous-visible cleanup-headroom promotion with entry `62` excluded, the BUILDING2-high previous-visible cleanup promotion, the W1-low setup/`{113..129}` + `{355..371}` CD-pressure promotions, fresh-owner `160..176`, previous-visible cleanup compaction, and entry65/entry39/entry55/entry56/entry59/entry63/entry66/entry85 payload trims, the W1-high frame56/`{178..194}` + `{423..439}` + `{404..416}` + `{395..411}` + retargeted `{411..423}` CD-pressure promotions plus prepare-first scheduler ownership, the VISITOR3-high 80 KiB clean-relief window plus setup-edge `40..47` speed and `42..49` CD-pressure retained-segment promotions, and the B2-high entries `92`/`94`/`95` trim plus `{185..197}` + `{158..174}` CD-pressure promotions and setup aliases for duplicate entries `141`/`142` and `38`. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
The mainline shifted from "prove every scene" to **performance polish, stability, and content** at `v0.7.0-ps1`. Current mainline builds on `v0.8.16-ps1` with the memory-region allocator promoted: BOOT allocations seal after startup, CACHE allocations use free-list/LRU reuse for long-lived resource data, and TRANSIENT scene allocations can be wiped between major scene loads instead of relying on a fragmented general heap. The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest five-yellow canary refresh is `scratch/ps1-perf-iterate/w1low-slack-frames87-99-five-yellow-current/20260521-085703-140832/summary.json`. Public rollup is `+0.2480%` over target / `99.7557%` target speed and raw signed rollup is about `-0.4687%` / `100.4860%`; bands are `121` green, `5` yellow, `0` orange, and `0` red. That is about `17.15` public over-target points removed and `12.66` public target-speed points added since the compact full-matrix baseline. The newest W1-low frame 87..99 fixed-layout cleanup-slack pass preserves file size, offsets, entry sizes, LBA, and the PS-EXE bucket while reducing selected active payload `755808 -> 751288`, cleanup spans `3322 -> 1605`, cleanup pixels `12700 -> 7209`, cleanup restore bytes `25400 -> 14418`, runtime restore bytes `439090 -> 430052`, upload bytes `17854720 -> 17838080`, dirty rows `27898 -> 27872`, and loop read time `146 -> 145`; all five under-green rows stay exact-flat. The prior W1-high cleanup-only fixed-layout screen-clip pass preserves file size, offsets, entry sizes, LBA, and the PS-EXE bucket while clipping exact entries `14`, `23`, `43`, `45`, `47`, `49`, `51`, `54`, `56`, `57`, `67`, `137`, and `147`; subset logical active payload drops `29339 -> 27643`, `18160` cleanup pixels are removed with no draw-span removal, runtime restore bytes drop `525020 -> 510842`, max restore bytes drop `12684 -> 7712`, upload bytes drop `17185920 -> 17182720`, and all five under-green rows stay exact-flat. The prior VISITOR3-low fixed-layout previous-visible cleanup pass preserves file size, offsets, entry sizes, LBA, and the PS-EXE bucket while changing `94` low-pack entries; logical active payload drops `425729 -> 405835`, cleanup restore bytes drop `456786 -> 53456`, runtime restore bytes drop `467962 -> 64632`, max restore bytes drop `293544 -> 2962`, upload bytes drop `18113920 -> 17494400`, and all five under-green rows stay exact-flat. The prior W1-high exact-entry preserve-entry-size screen-clip subset clips entries `58..61` while preserving file size, entry sizes, and LBA; subset logical active payload drops `17164 -> 9896`, `4995` cleanup pixels and `6052` draw pixels are removed, and all five under-green rows stay exact-flat. The prior BUILDING2-high no-decode trim-draw-tail headroom subset trims entries `74` and `78..82` while preserving file size and LBA; active payload drops `548293 -> 539990` and all five under-green rows stay exact-flat. The prior BUILDING2-high trim-draw-tail speed subset trims entries `67`, `69`, `70`, `71`, and `72`; active payload dropped `574094 -> 548293`, and B2-high improved `1343/1312 -> 1341/1313`, overrun `31 -> 28`, blocking `50 -> 47`, refill `17 -> 14`, and target speed `97.692% -> 97.912%`. The prior W1-high preserve-entry-size screen-clip subset clips cleanup-only entries `204..211` while keeping entry `dataSize`, file size, and LBA fixed; W1-high stays `1472/1441`, overrun `31`, blocking `43`, and refill `13`, while `12368` cleanup pixels and `720` logical payload bytes are removed. The prior VISITOR3-low preserve-entry-size screen-clip pass clips `10` entries while keeping entry `dataSize`, file size, and LBA fixed; VISITOR3-low stays `1071/1039`, overrun `32`, and blocking `80`, while logical active payload drops `425729 -> 423647`. The prior W1-low preserve-entry-size screen-clip pass clips `63` entries while keeping entry `dataSize`, file size, and LBA fixed; W1-low stays `1470/1446`, overrun `24`, and blocking `32`, while hidden refill improves `4 -> 3` and logical active payload drops `755808 -> 712808`. The prior BUILDING2-high preserve-entry-size screen-clip pass clips `25` entries while keeping entry `dataSize`, file size, and LBA fixed; B2-high improves `1343/1311 -> 1343/1312`, overrun `32 -> 31`, blocking `51 -> 50`, refill overrun `18 -> 17`, and target speed `97.617% -> 97.692%`. The prior VISITOR3-high offscreen cleanup screen-clip pass changes entries `101` and `116`, reducing active payload `437785 -> 436469` and removing `7393` offscreen cleanup pixels while the canonical five-yellow gate stays exact-flat. The prior VISITOR3-high previous-visible cleanup-headroom pass changed `106` high-pack entries while excluding regressing entry `62`, reducing active payload `461631 -> 437785`, cleanup restore bytes `542088 -> 36092`, runtime restore bytes `471382 -> 56312`, and upload bytes `18785280 -> 18038400` with fixed pack footprint/LBA. The prior BUILDING2-high previous-visible cleanup pass reduces active high-pack payload `663590 -> 574094`, cleanup restore bytes `439186 -> 80522`, runtime restore bytes `438988 -> 116648`, and upload bytes `26753280 -> 24341120`, improving B2-high from `1357/1307` to `1343/1311` (`97.617%` target speed) and overrun `50 -> 32`. The current under-green queue remains BUILDING2 high, VISITOR3 low, VISITOR3 high, WALKSTUF1 high, and WALKSTUF1 low.
