...
re-anchor, and the VISITOR3 low frame-118 resident-copy follow-up, the current
public-capped rollup is `+0.3720%`
over target / `99.6381%` target speed
across all 126 timing-bearing rows; the raw signed CSV records
`-0.3965%` over target / `100.4219%` target speed for
internal optimization analysis.

The latest VISITOR3 follow-up, `visitor3-low-f118-resident-copy-v234`, copies
the low-tide frame-118 payload unchanged into the already accepted `150..174`
low setup segment. It preserves frame cadence/render semantics while improving
low active timing `1095/1035 -> 1091/1035`, cutting overrun `60 -> 56`,
blocking `108 -> 103`, loop reads `19 -> 18`, loop-read time `115 -> 110`,
and due misses `18 -> 17` while keeping hidden refill at `0`. High remains the
...
`visitor3-high-setup-prime320-v214` plus `visitor3-low-f118-resident-copy-v234`:
...
low frame-125/frame-126 re-anchor payloads and a frame-118 resident copy into
that segment. Loop timing is `1089/1035` high and `1091/1035` low while
blocking drops to `83/103`, loop-read time drops to `83/110`, and due misses
are `15/17`; v214 high plus v234 low is the current runtime/CD-pressure
baseline. The
...
The current public-capped rollup is `+0.3720%` over target / `99.6381%` target
speed; the raw signed optimization matrix is `-0.3965%` / `100.4219%`. The next
...
`2075/2061`.
The public-capped timing-bearing average is now `+0.3720%` over target /
`99.6381%` target speed after the VISITOR3 high-only sparse frame-117
...
v234 low frame-118 resident-copy follow-up.
The raw signed optimization matrix is `-0.3965%` / `100.4219%`.
The latest VISITOR3 pass is the v234 low frame-118 resident-copy promotion:
it moves low to `1091/1035`, cutting overrun `60 -> 56`, blocking `108 -> 103`,
loop-read time `115 -> 110`, loop reads `19 -> 18`, and due misses `18 -> 17`;
