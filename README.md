...
The mainline shifted from "prove every scene" to **performance polish, stability, and content** at `v0.7.0-ps1`. Current post-release mainline win: BUILDING4 low now clips offscreen PAL4 draw spans directly in the pack, improving to `2853/2816`, overrun `37`, blocking/read time `40`/`215`, and refill `34` while preserving pack size/LBA and the PS-EXE bucket. WALKSTUF1 high/low also gained same-speed late-tail work-volume clips: high frames `194..210` drop `47671` draw pixels, `5152` spans, and `315` draw rows, while low carries isolated offscreen clips through v689, keeping `1478/1431` flat and dropping runtime rows/spans/pixels to `16663/120738/676450`. Public rollup is `+0.2736%` over target / `99.7310%` target speed and raw signed rollup is `-0.4936%` / `100.5133%`. Earlier current mainline wins include VISITOR3 high's `277..293` tail-pack repack, BUILDING2 low's slack-8 `218..229` row, VISITOR5 low's accepted high-tide `30..46` retained-read shape, and the WALKSTUF1 shared `427..443` same-speed CD-work cleanup.

Recent releases:

- Current mainline after `v0.8.8-ps1` — VISITOR5 low uses the low-tide `30..46` retained-read group, improving from `1104/1092` to `1102/1097` and moving that row into green. WALKSTUF1 low gained the same-speed shared `427..443` CD-work row, then high/low late-tail and isolated low offscreen work-volume clips through v689, dropping draw pixels/spans/rows while staying exact-flat at `1476/1434` and `1478/1431`. BUILDING2 low `218..229` slack8 improves `1349/1320 -> 1344/1318`; VISITOR3 high tail repack improves `1065/1039 -> 1063/1040`, overrun `26 -> 23`, blocking `41 -> 35`, and reads/due `7/7 -> 6/6`; the latest BUILDING4 low offscreen pack clip improves `2856/2816 -> 2853/2816`, overrun `40 -> 37`, blocking `44 -> 40`, and refill `37 -> 34`. Bands remain `117` green and `9` yellow.
