...
The mainline shifted from "prove every scene" to **performance polish, stability, and content** at `v0.7.0-ps1`. Current mainline builds on `v0.8.16-ps1` with the memory-region allocator promoted: BOOT allocations seal after startup, CACHE allocations use free-list/LRU reuse for long-lived resource data, and TRANSIENT scene allocations can be wiped between major scene loads instead of relying on a fragmented general heap. The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest five-yellow canary refresh is `scratch/ps1-perf-iterate/v3low-rg16-32-five-yellow-current/20260521-112951-1002166/summary.json`. Public rollup is `+0.2474%` over target / `99.7563%` target speed and raw signed rollup is about `-0.4695%` / `100.4867%`; bands are `121` green, `5` yellow, `0` orange, and `0` red. That is about `17.15` public over-target points removed and `12.66` public target-speed points added since the compact full-matrix baseline. The newest VISITOR3-low `16..32` read-group pass builds on the frame135 gap-D4 baseline and improves VISITOR3-low `1342/1070/1039 -> 1339/1066/1035`, keeps overrun at `31`, cuts blocking `78 -> 73`, loop reads `18 -> 17`, read time `104 -> 96`, and due misses `14 -> 13`; the other four under-green canary rows stay exact-flat. The prior VISITOR3-low frame135 gap-D4 pass preserved file size, LBA, sectors, and the PS-EXE bucket while moving a `14246` byte previous-frame delta into the existing early D4 gap, improving VISITOR3-low `1071/1039 -> 1070/1039`, overrun `32 -> 31`, blocking `80 -> 78`, and loop read time `106 -> 104`. The prior W1-low frame 87..99 fixed-layout cleanup-slack pass preserves file size, offsets, entry sizes, LBA, and the PS-EXE bucket while reducing selected active payload `755808 -> 751288`, cleanup spans `3322 -> 1605`, cleanup pixels `12700 -> 7209`, cleanup restore bytes `25400 -> 14418`, runtime restore bytes `439090 -> 430052`, upload bytes `17854720 -> 17838080`, dirty rows `27898 -> 27872`, and loop read time `146 -> 145`; all five under-green rows stay exact-flat. The prior W1-high cleanup-only fixed-layout screen-clip pass preserves file size, offsets, entry sizes, LBA, and the PS-EXE bucket while clipping exact entries `14`, `23`, `43`, `45`, `47`, `49`, `51`, `54`, `56`, `57`, `67`, `137`, and `147`; subset logical active payload drops `29339 -> 27643`, `18160` cleanup pixels are removed with no draw-span removal, runtime restore bytes drop `525020 -> 510842`, max restore bytes drop `12684 -> 7712`, upload bytes drop `17185920 -> 17182720`, and all five under-green rows stay exact-flat. The prior VISITOR3-low fixed-layout previous-visible cleanup pass preserves file size, offsets, entry sizes, LBA, and the PS-EXE bucket while changing `94` low-pack entries; logical active payload drops `425729 -> 405835`, cleanup restore bytes drop `456786 -> 53456`, runtime restore bytes drop `467962 -> 64632`, max restore bytes drop `293544 -> 2962`, upload bytes drop `18113920 -> 17494400`, and all five under-green rows stay exact-flat. The current under-green queue remains BUILDING2 high, VISITOR3 low, VISITOR3 high, WALKSTUF1 high, and WALKSTUF1 low.
