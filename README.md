...
The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest five-yellow canary refresh is `scratch/ps1-perf-iterate/v3high-seg48-55-exact-clean-phase3-five-yellow-current/20260521-203921-4092186/summary.json`. Public rollup is `+0.2352%` over target / `99.7679%` target speed and raw signed rollup is about `-0.4817%` / `100.4982%`; bands are `121` green, `5` yellow, `0` orange, and `0` red. That is about `17.16` public over-target points removed and `12.67` public target-speed points added since the compact full-matrix baseline.

The newest speed promotion retargets VISITOR3-high setup segment3 to relative sectors `48..55`, uses exact six-rect high-tide clean-background capture, and adds a three-VBlank V3-high phase ballast before the measured loop. The five-yellow canary keeps all pack LBAs and the `233472` byte PS-EXE bucket fixed while VISITOR3-high improves from `1372/1082/1045` to `1358/1067/1045`, cutting loop overrun `37 -> 22`, blocking `34 -> 32`, and scene time by `14` VBlanks with no prefetch overrun. The current under-green queue is VISITOR3 low, WALKSTUF1 high, BUILDING2 high, VISITOR3 high, and WALKSTUF1 low.
...
- Current VISITOR3-high retained-segment/exact-clean speed promotion - segment3
