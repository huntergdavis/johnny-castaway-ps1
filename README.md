...
bucket. After the MARY3 guarded prefetch-preserve promotion, the current
public-capped rollup is `+0.5526%` over target / `99.4709%` target speed
across all 126 timing-bearing rows; the raw signed CSV records `-0.2159%`
over target / `100.2547%` target speed for internal optimization analysis.
...
87.1% target speed**, the public-capped view has removed about **16.85
...
control drift rather than candidate-caused. The newer MARY3 guarded
...
drop `255 -> 13`. The current public-capped rollup is `+0.5526%` over target /
`99.4709%` target speed; the raw signed optimization matrix is `-0.2159%` /
`100.2547%`. The next true rows are VISITOR3 low/high, BUILDING2 low,
WALKSTUF1 high/low, BUILDING6, and remaining generated scheduler/data-shape
work.
...
The public-capped timing-bearing average is now `+0.5526%` over target /
`99.4709%` target speed after the MARY3 guarded prefetch-preserve promotion.
The raw signed optimization matrix is `-0.2159%` / `100.2547%`. The MARY3 pass
