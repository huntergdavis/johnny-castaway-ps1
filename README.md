...
- `main` after `v0.8.7-ps1` — BUILDING2 high now keeps retained read groups `60..72`, `226..242`, and `249..257` on top of the BUILDING6 scene-local slack4 guard, JOHNNY6 compact-FGP3 metadata plus restore-minus-current promotion, WALKSTUF1 high `344..360` plus `422..434`, VISITOR3 low tail pack compaction, and the WALKSTUF1 low staged-prepare fallback. Public rollup is now `+0.3141%` over target / `99.6920%` target speed; BUILDING2 high is `1351/1313`, and WALKSTUF1 high stays `1480/1432` while loop reads drop to `66` and loop-read time to `286`.
