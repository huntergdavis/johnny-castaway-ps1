...
The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest four-yellow canary refresh is `scratch/ps1-perf-iterate/v3low-seg4-add55-79-cache-phase0-four-yellow-current-20260522/20260522-083527-3978849/summary.json`, with a B2-high focused headroom refresh at `scratch/ps1-perf-iterate/b2high-trim-safe-tail-current-20260522/20260522-112706-773172/summary.json`. Public rollup is `+0.2233%` over target / `99.7793%` target speed and raw signed rollup is about `-0.4936%` / `100.5097%`; bands are `122` green, `4` yellow, `0` orange, and `0` red. That is about `17.18` public over-target points removed and `12.68` public target-speed points added since the compact full-matrix baseline.

The newest BUILDING2-high payload-headroom promotion trims safe draw tails in
entries `101..104`, `141`, `175..177`, and `333`, excluding the known
phase-negative `76`/`77` pocket. `BUILDING2.FG2` keeps file size, offsets, LBA
`6189`, sectors `637`, and the `233472` byte PS-EXE bucket fixed while active
payload drops `527960 -> 527884` and max payload drops `8752 -> 8733`. B2-high
stays exact-flat at `1583/1340/1314`, overrun `26`, blocking/refill `45/12`,
loop reads/read time `42/186`, and due `7`, so the speed rollup is unchanged.
