...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.2744% over target / 99.7301% target speed**. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
The mainline shifted from "prove every scene" to **performance polish, stability, and content** at `v0.7.0-ps1`. Current post-release mainline win: VISITOR3 high now reuses the proven low-tail compact cleanup payloads and repacks frames `141/140/142/143/144` plus sound events inside the existing `277..293` setup segment, improving to `1063/1040`, overrun `23`, blocking/read time `35`, and reads/due `6/6`, with public rollup `+0.2744%` over target / `99.7301%` target speed and raw signed rollup `-0.4927%` / `100.5125%`. Earlier current mainline wins include BUILDING2 low's slack-8 `218..229` row, VISITOR5 low's accepted high-tide `30..46` retained-read shape, and the WALKSTUF1 shared `427..443` same-speed CD-work cleanup.

Recent releases:

- Current mainline after `v0.8.8-ps1` — VISITOR5 low uses the low-tide `30..46` retained-read group, improving from `1104/1092` to `1102/1097` and moving that row into green. WALKSTUF1 low also gained a same-speed shared `427..443` CD-work row, dropping loop reads `64 -> 62`. BUILDING2 low `218..229` slack8 improves `1349/1320 -> 1344/1318`; the latest VISITOR3 high tail repack improves `1065/1039 -> 1063/1040`, overrun `26 -> 23`, blocking `41 -> 35`, and reads/due `7/7 -> 6/6`. Bands remain `117` green and `9` yellow.
