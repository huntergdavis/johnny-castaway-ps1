...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.2387% over target / 99.7644% target speed** after the W1-low setup/`{113..129}` + `{355..371}` CD-pressure promotions plus entry65/entry39/entry55/entry56/entry59/entry63/entry66/entry85 payload trims, the W1-high frame56/`{178..194}` + `{423..439}` + `{404..416}` + `{395..411}` CD-pressure promotions plus prepare-first scheduler ownership, the VISITOR3-high 80 KiB clean-relief window promotion, and the B2-high entries `92`/`94`/`95` trim plus `{185..197}` CD-pressure promotion and setup alias for duplicate entries `141`/`142`. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
