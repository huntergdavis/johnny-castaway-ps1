...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.3111% over target / 99.6949% target speed**. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
- `main` after `v0.8.7-ps1` — VISITOR5 low now uses the padded compact FGP3/v4 restore-minus-current pack plus a compact-only `23..47` retained read, moving to `1104/1092`, overrun `12`, blocking/refill `11`, loop reads `19`, and loop-read VBlanks `91`. BUILDING2 high keeps the 24-sector grouped-read capacity, BUILDING2 low keeps the `238..250` retained read, and public rollup is now `+0.3111%` over target / `99.6949%` target speed; WALKSTUF1 high/low and VISITOR3 low remain the top under-99 rows.
