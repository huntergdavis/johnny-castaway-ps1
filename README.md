...
The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest under-yellow promotion gate is `scratch/ps1-perf-iterate/v3low-dp-v3h-phase4-canary-20260523-131154/20260523-131154-2336560/summary.json`. Public rollup is `+0.1906%` over target / `99.8111%` target speed and raw signed rollup is `-0.5263%` / `100.5414%`; bands are `125` green, `1` yellow, `0` orange, and `0` red. That is about `17.20` public over-target points removed and `12.71` public target-speed points added since the compact full-matrix baseline.

**The newest VISITOR3-low double-preload + VISITOR3-high phase-4 retime
...
- Current WALKSTUF1-high frame0 consume speed promotion - W1-high now presents
  ...
  bands remain `125` green / `1` yellow.
