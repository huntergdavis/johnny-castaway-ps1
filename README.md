...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.2690% over target / 99.7354% target speed** after the VISITOR3 low frame138 raw-gap promotion. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
The mainline shifted from "prove every scene" to **performance polish, stability, and content** at `v0.7.0-ps1`. Current mainline builds on `v0.8.16-ps1` with the new memory-region allocator promoted: BOOT allocations seal after startup, CACHE allocations use free-list/LRU reuse for long-lived resource data, and TRANSIENT scene allocations can be wiped between major scene loads instead of relying on a fragmented general heap. The allocator merge also adds pack-header metrics and CI gates requiring `MEM_REGION_RATIONALE` annotations at every `memAlloc` call site. The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest focused allocator-era checkpoint refreshes the under-green VISITOR3, WALKSTUF1, BUILDING2, and BUILDING4 rows. Public rollup is `+0.2690%` over target / `99.7354%` target speed and raw signed rollup is about `-0.4479%` / `100.4658%`; bands are `119` green, `7` yellow, `0` orange, and `0` red. That is about `17.13` public over-target points removed and `12.64` public target-speed points added since the compact full-matrix baseline. The newest speed win moves VISITOR3 low frame `138`'s raw payload into the paid `206..232` setup gap and extends the third low setup segment by two sectors, improving low from `1074/1039` to `1065/1039`, overrun `35 -> 26`, blocking `85 -> 75`, reads `19 -> 18`, and due `15 -> 14` while keeping refill at `0`. The prior pressure win extends WALKSTUF1 high's second retained setup slice from `286..342` to `286..344`, keeping overrun flat at `34` while reducing blocking `60 -> 59`, prefetch overrun `15 -> 13`, loop reads `48 -> 46`, and read time `228 -> 212`.
