...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.3273% over target / 99.6792% target speed**. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
- `main` after `v0.8.7-ps1` — VISITOR3 low frame128/frame129 resident-slot swap on top of the WALKSTUF1 high `213..229` read-group/slack4 baseline. Public rollup is `+0.3273%` over target / `99.6792%` target speed.
