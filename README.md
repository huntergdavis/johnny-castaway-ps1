...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.2425% over target / 99.7608% target speed** after the WALKSTUF1 low setup/`{113..129}` CD-pressure promotion, the same-speed `{355..371}` read-work row, and the exact-flat WALKSTUF1 high entry136/entry57 payload trims. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
The mainline shifted from "prove every scene" to **performance polish, stability, and content** at `v0.7.0-ps1`. Current mainline builds on `v0.8.16-ps1` with the new memory-region allocator promoted: BOOT allocations seal after startup, CACHE allocations use free-list/LRU reuse for long-lived resource data, and TRANSIENT scene allocations can be wiped between major scene loads instead of relying on a fragmented general heap. The allocator merge also adds pack-header metrics and CI gates requiring `MEM_REGION_RATIONALE` annotations at every `memAlloc` call site. The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest focused allocator-era checkpoint refreshes the under-green VISITOR3, WALKSTUF1, BUILDING2, and BUILDING4 rows. Public rollup is `+0.2425%` over target / `99.7608%` target speed and raw signed rollup is about `-0.4744%` / `100.4911%`; bands are `121` green, `5` yellow, `0` orange, and `0` red. That is about `17.22` public over-target points removed and `12.72` public target-speed points added since the compact full-matrix baseline. The newest W1-low CD-pressure wins shift the main setup residency to `244..350`, keep a split `179..185` setup edge, add `{113..129}`, then add `{355..371}` as a same-speed work-volume row, holding target speed at `98.367%` while improving scene `1812 -> 1809`, blocking/refill `34/6 -> 33/5`, and loop reads/read time `30/159 -> 24/147`; the five-yellow canary stays flat elsewhere. The newest W1-high work-volume win trims WALKSTUF1 high entry57/source70 in place (`3409 -> 320` bytes, `3 -> 2` sectors) on top of the prior entry136/source244 trim (`3762 -> 2596` bytes, `3 -> 2` sectors), with fixed pack LBA/PS-EXE bucket and exact-flat five-yellow timing. The prior speed win adds the WALKSTUF1 low `{378..390}` retained read group after splitting a broader refill-debt batch, improving `1470/1445 -> 1470/1446`, overrun `25 -> 24`, blocking/refill `35/7 -> 34/6`, loop reads/read time `31/163 -> 30/159`, and target speed `98.299% -> 98.367%`; due stays `4`. The prior WALKSTUF1 low win trims frame132's draw-tail payload without changing pack size/LBA, improving `1473/1447 -> 1470/1445`, overrun `26 -> 25`, blocking `41 -> 35`, loop reads `36 -> 31`, due `5 -> 4`, and target speed `98.235% -> 98.299%`.

The v0.8.13 checkpoint also extended BUILDING2 high preserve-offset payload trims through `building2-high-frame100-inplace-v926`: entry `172` / source frame `231` shrinks `1831 -> 851` bytes, entry `171` / source frame `228` shrinks `1980 -> 1025` bytes, entry `96` / source frame `119` shrinks `8781 -> 7944` bytes, entry `170` / source frame `226` shrinks `1683 -> 1186` bytes, entry `97` / source frame `121` shrinks `8718 -> 8258` bytes, entry `98` / source frame `123` shrinks `8876 -> 8637` bytes, entry `174` / source frame `239` shrinks `1625 -> 1460` bytes, entry `99` / source frame `126` shrinks `8843 -> 8728` bytes, entry `168` / source frame `219` shrinks `1372 -> 1266` bytes, entry `169` / source frame `223` shrinks `1820 -> 1495` bytes, entry `173` / source frame `235` shrinks `1765 -> 1134` bytes, and entry `100` / source frame `129` shrinks `8701 -> 8621` bytes. Active payload drops `674798 -> 669408`; the current B2-high speed baseline layers the allocator-era setup slices with the `83..95`, guarded `271..287`, and `315..327` scheduler rows.

Recent releases:

- Current allocator-era optimization checkpoint — VISITOR3 high improves from `1232/1033` to `1075/1044` and VISITOR3 low from `1231/1040` to `1065/1039` by retaining the tiny stage1 prefetch frame buffer under clean-memory relief, keeping bounded clean-relief stream windows, trimming the high-tide terminal read before resident setup-segment data, extending high setup segment2 to relative sectors `203..229`, relocating high frame `139` into that paid gap without decode, adding the high-tide third retained setup segment `228..262`, moving high frames `56` and `57` raw into that retained gap with a `56 KiB` tight-refill cap, and capping high clean strips at `64 KiB`. Low adds the third retained setup segment `206..230` and moves frame `138` raw into the paid `206..232` gap. BUILDING4 high primes `264..288` during setup and moves into green, BUILDING2 high replaces its tail read group with `83..95` and then adds guarded `271..287` plus `315..327`, WALKSTUF1 low replaces its split `197..243` + `410..434` residency with `238..344` after a low-only 48 KiB clean-rect cap, adds `{91,107}`, pays a split TRANSIENT `344..350` setup edge, trims frame132, adds `{378,390}`, and now retargets setup to `244..350` plus split `179..185` with `{113,129}` to lower CD debt while staying `1470/1446`, WALKSTUF1 high keeps `198..244`, retargets its second retained slice to `286..344`, adds `{149,165}`, encodes frame `92` as D4, and now trims entries `136` and `57` exact-flat to keep `1471/1440` with less payload work, BUILDING2 low now primes `112..128` and `226..262` during setup with a low-only `80 KiB` clean strip cap plus `{141,153}` to reach green at `1327/1318`, and BUILDING4 low now combines the `8`-row dirty-upload band merge gap with a `24 KiB` stream window to reach green at `2847/2820`. The current under-green queue is VISITOR3 high, BUILDING2 high, VISITOR3 low, WALKSTUF1 high, and WALKSTUF1 low.
