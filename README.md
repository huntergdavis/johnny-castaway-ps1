...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.2702% over target / 99.7342% target speed**. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
The mainline shifted from "prove every scene" to **performance polish, stability, and content** at `v0.7.0-ps1`. Current mainline after `v0.8.12-ps1` keeps the v0.8.11 lazy CD/FG stream rollback and advances WALKSTUF1 low to `walkstuf1-low-rg394-410-v817`: the low-tide-only retained read group `394..410` improves the focused gate to `1769/1477/1431`, overrun `46`, loop reads/read time `58/266`, while blocking/refill/due stay `64/20/11`; the earlier v802 in-place frame shrinks still cut active payload `879801 -> 798079` without moving pack offsets. BUILDING2 low now trims dead draw-tail payload in the existing pack footprint, improving to `1339/1317`, overrun `22`, blocking/read time `53`/`150`, reads `37`, and due `12` while preserving pack size/LBA and the PS-EXE bucket. BUILDING4 low clips offscreen PAL4 draw spans directly in the pack, improving to `2853/2816`, overrun `37`, blocking/read time `40`/`215`, and refill `34`; v827 now also shrinks frames `291`, `34`, `31`, `290`, `32`, `35`, `36`, `37`, `288`, `287`, `39`, and `286` in-place without moving offsets, keeping timing exact-flat while cutting active payload `855284 -> 810226`. BUILDING2 high carries same-speed offscreen work-volume clips for frames `89..92`, `94..104`, and `168..177`, keeping `1351/1311` flat while dropping runtime rows/spans/pixels to `18030/105645/446246`. WALKSTUF1 high/low also gained same-speed work-volume clips: high frames `194..210` plus frames `55`, `138`, `51`, `49`, and `47` drop runtime rows/spans/pixels to `17039/132173/749787` while staying exact-flat at `1476/1434` with loop reads/read time `63/276`, and low carries isolated offscreen clips through v726 plus v747/v749/v750/v751/v753/v755/v756/v757/v759/v762/v763/v766/v767/v769/v770/v771/v772/v773/v774/v775/v776/v777/v779/v780/v781/v782/v783/v784/v785/v786/v787/v788/v789/v790/v791/v794/v795/v797/v798/v800/v801/v802 no-shift frame `51`/`49`/`47`/`61`/`62`/`58`/`45`/`37`/`35`/`43`/`41`/`57`/`33`/`67`/`68`/`69`/`32`/`133`/`5`/`141`/`70`/`30`/`6`/`71`/`72`/`142`/`73`/`131`/`74`/`19`/`28`/`138`/`145`/`75`/`76`/`77`/`130`/`135`/`1`/`88`/`90`/`3` shrinks, keeping the prior row exact-flat; v760 restores bounded CD fast-polling after the release merge while preserving the long-soak timeout fallback. A fresh under-99 refresh also corrected the stale JOHNNY1 low row to `1973/1945`, matching high tide. Public rollup is `+0.2702%` over target / `99.7342%` target speed and raw signed rollup is `-0.4969%` / `100.5165%`. Earlier current release wins include VISITOR3 high's `277..293` tail-pack repack, VISITOR5 low's accepted high-tide `30..46` retained-read shape, and the WALKSTUF1 shared `427..443` same-speed CD-work cleanup.
