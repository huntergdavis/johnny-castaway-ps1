...
The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest promotion gate is `scratch/ps1-perf-iterate/v3high-double-preload-phase2-canary-20260523-132533/20260523-132533-2417812/summary.json`. Public rollup is `+0.1830%` over target / `99.8185%` target speed and raw signed rollup is `-0.5339%` / `100.5488%`; bands are **`126` green, `0` yellow, `0` orange, and `0` red — every routed/timing-bearing row is at or above 99% target speed**. That is about `17.22` public over-target points removed and `12.72` public target-speed points added since the compact full-matrix baseline.

**🎉 The newest VISITOR3-high double-preload + phase-2 promotion moves
