...
The mainline shifted from "prove every scene" to **performance polish, stability, and content** at `v0.7.0-ps1`. Current mainline builds on `v0.8.16-ps1` with the memory-region allocator promoted: BOOT allocations seal after startup, CACHE allocations use free-list/LRU reuse for long-lived resource data, and TRANSIENT scene allocations can be wiped between major scene loads instead of relying on a fragmented general heap. The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest five-yellow canary refresh is `scratch/ps1-perf-iterate/w1high-entry53-clip-five-yellow-current/20260521-143049-2014915/summary.json`. Public rollup is `+0.2465%` over target / `99.7571%` target speed and raw signed rollup is about `-0.4704%` / `100.4875%`; bands are `121` green, `5` yellow, `0` orange, and `0` red. That is about `17.15` public over-target points removed and `12.66` public target-speed points added since the compact full-matrix baseline. The newest W1-high entry `53` screen-clip pass preserves file size, offsets, LBA, and the PS-EXE bucket while shrinking that entry `3893 -> 2309` bytes and removing `8969` offscreen cleanup pixels plus `1945` offscreen draw pixels; all five under-green canary rows stay exact-flat. The prior W1-low entry `90..99` canonicalization pass preserves file size, offsets, LBA, and the PS-EXE bucket while shrinking the active cluster `47579 -> 44511` bytes and total low-pack active payload `755808 -> 752740`. The prior VISITOR3-high entry `62` cleanup pass preserves file size, offsets, entry table sizes, LBA, and the PS-EXE bucket while shrinking entry restore bytes `2724 -> 596`, runtime restore bytes `56312 -> 54184`, and upload bytes `18038400 -> 18012160`. The current under-green queue remains BUILDING2 high, VISITOR3 low, VISITOR3 high, WALKSTUF1 high, and WALKSTUF1 low.
...
- Current W1-high entry `53` fixed-layout screen-clip promotion — `WALKSTUF1.FG2` keeps file size, offsets, LBA, and the PS-EXE bucket fixed while entry `53` / source frame `63` shrinks `3893 -> 2309` bytes, removing `8969` offscreen cleanup pixels and `1945` offscreen draw pixels. The five-yellow canary stays exact-flat at the same public rollup.
