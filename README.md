...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.2994% over target / 99.7069% target speed** after the VISITOR3 high frame139 raw-gap promotion. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
The mainline shifted from "prove every scene" to **performance polish, stability, and content** at `v0.7.0-ps1`. Current mainline builds on `v0.8.16-ps1` with the new memory-region allocator promoted: BOOT allocations seal after startup, CACHE allocations use free-list/LRU reuse for long-lived resource data, and TRANSIENT scene allocations can be wiped between major scene loads instead of relying on a fragmented general heap. The allocator merge also adds pack-header metrics and CI gates requiring `MEM_REGION_RATIONALE` annotations at every `memAlloc` call site. The latest full matrix remains `126 / 126` routed and timing-bearing with 0 BSODs in the R34 allocator validation run; the latest focused allocator-era checkpoint refreshes the under-green VISITOR3, WALKSTUF1, BUILDING2, and BUILDING4 rows. Public rollup is `+0.2994%` over target / `99.7069%` target speed and raw signed rollup is about `-0.4175%` / `100.4373%`; bands are `119` green, `6` yellow, `1` orange, and `0` red. That is about `17.10` public over-target points removed and `12.61` public target-speed points added since the compact full-matrix baseline. The newest speed win moves VISITOR3 high frame `139`'s raw `15679` byte payload into the already retained `203..229` setup-segment gap at offset `417308`, removing one active read without D4 decode cost. VISITOR3 high improves target/overrun `1040/56 -> 1041/55`, blocking `71 -> 67`, and loop reads/due `8/5 -> 7/4`; it remains the single orange row at `94.982%`.
...
- Current allocator-era optimization checkpoint — VISITOR3 high improves from `1232/1033` to `1096/1041` and VISITOR3 low from `1231/1040` to `1074/1039` by retaining the tiny stage1 prefetch frame buffer under clean-memory relief, keeping bounded clean-relief stream windows, trimming the high-tide terminal read before resident setup-segment data, extending high setup segment2 to relative sectors `203..229`, relocating high frame `139` into that paid gap without decode, and adding the low-tide third retained setup segment `206..230`. BUILDING4 high then primes `264..288` during setup and moves into green, BUILDING2 high replaces its tail read group with `83..95`, VISITOR3 high cuts blocking to `67`, and VISITOR3 low cuts blocking to `85` while staying yellow. The current under-green queue is VISITOR3 high/low, WALKSTUF1 high/low, BUILDING2 high/low, and BUILDING4 low.
