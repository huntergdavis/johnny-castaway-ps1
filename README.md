...
high setup-prime expansion, and the VISITOR3 low frame-125/frame-126 resident
re-anchor, the current public-capped rollup is `+0.3751%`
over target / `99.6354%` target speed
across all 126 timing-bearing rows; the raw signed CSV records
`-0.3934%` over target / `100.4192%` target speed for
...
already accepted `150..174` low setup segment. It improves low active timing
`1098/1034 -> 1095/1035`, cuts overrun `64 -> 60`, blocking `112 -> 108`,
loop reads `20 -> 19`, loop-read time `119 -> 115`, and due misses `19 -> 18`
while keeping hidden refill at `0`. High remains the v214 profile at
`1089/1035`, blocking `83`, loop-read time `83`, and due misses `15`. Both
tides keep fixed LBAs plus the `215040` byte PS-EXE bucket, and the low pack
keeps its fixed footprint and sound-table offset.
...
`visitor3-high-setup-prime320-v214` plus `visitor3-low-f125126-resident-v227`:
...
low frame-125/frame-126 re-anchor payloads into that resident segment.
Loop timing is `1089/1035` high and `1095/1035` low while blocking drops to
`83/108`, loop-read time drops to `83/115`, and due misses are `15/18`; v214
high plus v227 low is the current runtime/CD-pressure baseline. The
...
The current public-capped rollup is `+0.3751%` over target / `99.6354%` target
speed; the raw signed optimization matrix is `-0.3934%` / `100.4192%`. The next
...
`2075/2061`.
The public-capped timing-bearing average is now `+0.3751%` over target /
`99.6354%` target speed after the VISITOR3 high-only sparse frame-117
...
setup segment, and the v227 low frame-125/frame-126 resident re-anchor.
The raw signed optimization matrix is `-0.3934%` / `100.4192%`.
The latest VISITOR3 pass is the v227 low resident re-anchor promotion:
it moves low to `1095/1035`, cutting overrun `64 -> 60`, blocking `112 -> 108`,
loop-read time `119 -> 115`, loop reads `20 -> 19`, and due misses `19 -> 18`;
