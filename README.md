...
| Headless perf | **126 / 126** scene/tide rows are routed and timing-bearing; public-capped average is **+0.2520% over target / 99.7522% target speed**. Live battle card at [/perf/](https://hunterdavis.com/johnny-castaway-ps1/perf/) · CSV at [`performance-scene-matrix.csv`](docs/ps1/performance-scene-matrix.csv) |
...
The mainline shifted from "prove every scene" to **performance polish, stability, and content** at `v0.7.0-ps1`. Current mainline builds on `v0.8.14-ps1` with `walkstuf1-low-rg209-225-v935`: W1-low stays flat at `1773/1481`, improves target `1428 -> 1431`, overrun `53 -> 50`, blocking `78 -> 72`, reads/read time `61/279 -> 58/267`, and due `11 -> 10` against the fresh current control. JOHNNY1 high/low remain green after the local-LZ payload swing, BUILDING2 high remains on `building2-high-frame100-inplace-v926`, BUILDING4 low remains on `building4-low-frame425-inplace-v928`, and VISITOR3 high/low remain on the v629/v510 pack-shape baselines. Public rollup is `+0.2520%` over target / `99.7522%` target speed and raw signed rollup is about `-0.5152%` / `100.5345%`; bands are `119` green and `7` yellow. That is about `17.15` public over-target points removed and `12.65` public target-speed points added since the compact full-matrix baseline.
