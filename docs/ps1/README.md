...
| Headless perf battle card | **126 / 126** variants routed; **120 / 126** timing-bearing; **+0.4% over target / 99.8% target speed** |
...
metadata reader inline baseline: `0.4096%` over target / `99.7860%` target
speed across 120 timing-bearing rows.
...
current timing-bearing rows average `+0.4096%` over target / `99.7860%`
...
compact draw metadata, and compact decoder inline promotions.
