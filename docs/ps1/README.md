...
| Headless perf battle card | **126 / 126** variants routed; **126 / 126** timing-bearing; **+0.1983% public over target / 99.8035% public target speed** |
...
loading, and the current battle card is `+0.1983%` over target /
`99.8035%` target speed across all 126 timing-bearing rows. The raw signed
optimization matrix is `-0.5186%` / `100.5339%`; bands are `123`
