...
| Headless perf battle card | **126 / 126** variants routed; **126 / 126** timing-bearing; **+0.2028% public over target / 99.7992% public target speed** |
...
loading, and the current battle card is `+0.2028%` over target /
`99.7992%` target speed across all 126 timing-bearing rows. The raw signed
optimization matrix is about `-0.5141%` / `100.5295%`; bands are `123`
