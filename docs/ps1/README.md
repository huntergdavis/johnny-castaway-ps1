...
| Headless perf battle card | **126 / 126** variants routed; **126 / 126** timing-bearing; **+0.1906% public over target / 99.8111% public target speed** |
...
loading, and the current battle card is `+0.1906%` over target /
`99.8111%` target speed across all 126 timing-bearing rows. The raw signed
optimization matrix is `-0.5263%` / `100.5414%`; bands are `123`
