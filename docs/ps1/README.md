...
| Headless perf battle card | **126 / 126** variants routed; **126 / 126** timing-bearing; **+0.1762% public over target / 99.8252% public target speed** |
...
loading, and the current battle card is `+0.1762%` over target /
`99.8252%` target speed across all 126 timing-bearing rows. The raw signed
optimization matrix is `-0.5407%` / `100.5555%`; bands are `123`
