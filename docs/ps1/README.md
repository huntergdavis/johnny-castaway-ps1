...
| Headless perf battle card | **126 / 126** variants routed; **126 / 126** timing-bearing; **+0.1956% public over target / 99.8062% public target speed** |
...
loading, and the current battle card is `+0.1956%` over target /
`99.8062%` target speed across all 126 timing-bearing rows. The raw signed
optimization matrix is `-0.5213%` / `100.5366%`; bands are `123`
