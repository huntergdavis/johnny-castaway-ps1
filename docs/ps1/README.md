...
`STAND 4`, `STAND 5`, `STAND 6`, and `STAND 7` on the normal
high-tide/night route. `STAND 5`, `STAND 6`, and `STAND 7` use the STAND
no-stitch export fast path with a full-frame single-position
foreground-only overlay so static frame-0 Johnny pixels are not dropped. The scene-loader path now skips the stale
