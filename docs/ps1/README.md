...
| Headless perf battle card | **126 / 126** variants routed; **126 / 126** timing-bearing; **+0.1977% public over target / 99.8041% public target speed** |
...
loading, and the current battle card is `+0.1977%` over target /
`99.8041%` target speed across all 126 timing-bearing rows. The raw signed
optimization matrix is `-0.5192%` / `100.5344%`; bands are `123`
