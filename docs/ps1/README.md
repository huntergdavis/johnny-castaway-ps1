...
| Headless perf battle card | **126 / 126** variants routed; **126 / 126** timing-bearing; **+0.2006% public over target / 99.8013% public target speed** |
...
loading, and the current battle card is `+0.2006%` over target /
`99.8013%` target speed across all 126 timing-bearing rows. The raw signed
optimization matrix is `-0.5163%` / `100.5317%`; bands are `123`
