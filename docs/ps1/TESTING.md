...
`walkstuf1-low-frame81-inplace-v852` shrinks low entry `81` / source frame
`143` in place after the v849 frame79 trim, preserving all payload
offsets while cutting that entry `4919 -> 4259` bytes.
Low stays exact-flat at `1769`, active loop/target `1477/1431`,
blocking/refill `64/20`, loop reads/read time `58/266`, and due `11`, while
runtime rows/spans/pixels stay at the v726 row
`16257/114798/633876`; active payload now drops directly
`794755 -> 794095`, and the no-shift lane drops `879801 -> 794095`.
