...
`walkstuf1-low-frame89-inplace-v860` shrinks low entry `89` / source frame
`162` in place after the speed-bearing v859 frame87 trim, preserving all
payload offsets while cutting that entry `3792 -> 3547` bytes. Low repeats at
scene `1769`, active loop `1477`, target `1432`, blocking/refill `65/20`,
loop reads/read time `58/259`, and due `11`, while runtime rows/spans/pixels
stay at the v726 row `16257/114798/633876`; active payload now drops directly
`792902 -> 792657`, and the no-shift lane drops `879801 -> 792657`.
