...
`building2-high-frame171-inplace-v879` trims entry `171` / source frame `228`
after v877 trimmed entry `172` / source frame `231`, preserving all payload
offsets, pack LBA/sectors, and the PS-EXE bucket. Entries `172` and `171`
shrink `1831 -> 851` and `1980 -> 1025` bytes, and active payload drops
`674798 -> 672863`; timing/CD stay exact-flat at scene `1602`, loop/target
`1351/1311`, overrun `40`, blocking/refill `54/18`, loop reads/read time
`58/257`, and due `7`.
...
`building2-high-frame171-inplace-v879` trims entry `171` / source frame `228`
after v877 trimmed entry `172` and v703 added frame `89` on top of v702/v701/v700
frames `90`, `91`, and `92`, v698 safe `94..104`, and v664 late `168..177`.
...
active payload drops `674798 -> 672863` with fixed pack LBA/sectors and
PS-EXE bucket.
