...
**Last updated:** 2026-05-03 (scene ledger at 17/63; `MARY 3` was
validated after a far-right full-frame foreground recapture, host
capture stale-surface invalidation, and low-memory clean-snapshot
relief).
...
`MARY 2`, and `MARY 3` have been validated
...
| Scene playback (fgpilot, `foreground_pilot.c`) | Primary render path; 17/63 scenes fully validated |
...
Milestone releases:
- Current branch after `v0.6.8-ps1` — promotes `MARY 3` after
  far-right full-frame keyed foreground-only recapture restored the
  left-of-island action, host capture stopped replaying stale
...
| **17 / 63** | **2026-05-03** | **Current scene ledger after `MARY 3` promotion; far-right foreground-only recapture, stale host-surface invalidation, and low-memory clean-snapshot relief make the scene stable and readable** | **this doc, `scene-status.md`** |
