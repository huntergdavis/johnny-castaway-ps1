...
**Last updated:** 2026-05-04 (scene ledger at 26/63; `STAND 5` was
validated after the STAND no-stitch export fast path kept a full-frame
foreground-only overlay for static frame-0 Johnny pixels).
...
`STAND 1`, `STAND 2`, `STAND 3`, `STAND 4`, and `STAND 5` have been validated
...
`STAND 2`, `STAND 3`, `STAND 4`, and `STAND 5` are validated on the
normal high-tide/night route. `STAND 5` uses the STAND no-stitch export
fast path, but that fast path still keeps a full-frame single-position
foreground-only overlay so static first-frame Johnny pixels are not
suppressed as background.
...
| Scene playback (fgpilot, `foreground_pilot.c`) | Primary render path; 26/63 scenes fully validated |
...
- Current main after `v0.6.11-ps1` — promotes `STAND 2`, `STAND 3`,
  `STAND 4`, and `STAND 5` after visual signoff on the normal
  high-tide/night route. `STAND 5` also promotes the STAND no-stitch
  export fast path with a full-frame foreground-only overlay, after the
  pure base-diff shortcut faded Johnny's legs. The scene-loader direct
...
| **26 / 63** | **2026-05-04** | **Current scene ledger after `STAND 5` promotion; high/low packs were regenerated through the STAND no-stitch fast path with a full-frame foreground-only overlay, and normal high-tide/night playback passed human visual signoff** | **this doc, `scene-status.md`** |
