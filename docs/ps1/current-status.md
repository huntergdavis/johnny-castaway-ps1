...
**Last updated:** 2026-05-04 (scene ledger at 27/63; `STAND 6` was
validated on the normal high-tide/night route through the same STAND
no-stitch fast-path export `STAND 5` proved out).
...
`STAND 1`, `STAND 2`, `STAND 3`, `STAND 4`, `STAND 5`, and `STAND 6` have been validated
...
`STAND 2`, `STAND 3`, `STAND 4`, `STAND 5`, and `STAND 6` are validated
on the normal high-tide/night route. `STAND 5` and `STAND 6` use the
STAND no-stitch export fast path, but that fast path still keeps a
full-frame single-position foreground-only overlay so static first-frame
Johnny pixels are not suppressed as background.
...
| Scene playback (fgpilot, `foreground_pilot.c`) | Primary render path; 27/63 scenes fully validated |
...
- Current main after `v0.6.11-ps1` — promotes `STAND 2`, `STAND 3`,
  `STAND 4`, `STAND 5`, and `STAND 6` after visual signoff on the normal
  high-tide/night route. `STAND 5` and `STAND 6` ride the STAND
  no-stitch export fast path with a full-frame foreground-only overlay
  the pure base-diff shortcut needed after it faded Johnny's legs on
  `STAND 5`. The scene-loader direct launch path now skips the stale
  story walk before the frog/full-wipe transition, and high-pressure
  clean snapshots release optional caches before allocation.
...
| **27 / 63** | **2026-05-04** | **Current scene ledger after `STAND 6` promotion; high/low packs were regenerated through the same STAND no-stitch fast-path export `STAND 5` proved out, and normal high-tide/night playback passed human visual signoff** | **this doc, `scene-status.md`** |
