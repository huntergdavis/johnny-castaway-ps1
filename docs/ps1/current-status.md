...
baseline, WALKSTUF1 high setup-prime retune, and BUILDING2 high
restore-minus-current cleanup; all 63 scenes remain validated, and the
headless battle card is under target at `-0.0595%` over / `100.1296%` target
speed).
...
setup priming. The later VISITOR3 high/low offscreen draw clips and BUILDING2
high restore-minus-current pass move the current matrix to `-0.0595%` over
target / `100.1296%` target speed. See
...
`blocking_vb=191`. BUILDING2 high/low are `1353/1311` and `1385/1303`,
