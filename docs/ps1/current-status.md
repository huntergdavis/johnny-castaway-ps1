...
**Last updated:** 2026-05-05 (scene ledger at 61/63; `ACTIVITY 7` was
...
on the right side of the island from base-diff against the
static-Johnny base on the bathes-and-seagull-steals-clothes loop.
Earlier this push: `ACTIVITY 4` was validated on the existing on-disc
packs (no rework). `ACTIVITY 5` was validated on the climb/look/dive
gag using a JOHNNY 2-style split (upper bubble lane on full base-diff,
lower third on keyed overlay) plus +30 vblanks of hold on source frame 46.
`ACTIVITY 6` was validated after re-exporting high/low packs through the
no-stitch fast path with frame-wide keyed overlay. Earlier still: `BUILDING 1`-`BUILDING 7`
...
animations. `ACTIVITY 4`, `ACTIVITY 5`, `ACTIVITY 6`, `ACTIVITY 7`, `ACTIVITY 8`, `ACTIVITY 10`, `ACTIVITY 11`,
...
| Scene playback (fgpilot, `foreground_pilot.c`) | Primary render path; 61/63 scenes fully validated |
