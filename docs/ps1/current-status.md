...
**Last updated:** 2026-05-04 (scene ledger at 37/63; `SUZY 2` was
validated after keeping its scene-local `MRAFT.BMP` raft in the
foreground overlay and adding SFX mixer headroom for overlapping raft
samples).
...
`STAND 1`-`STAND 12`, `STAND 15`, `STAND 16`, `SUZY 1`, and `SUZY 2` have been validated
...
| Scene playback (fgpilot, `foreground_pilot.c`) | Primary render path; 37/63 scenes fully validated |
...
- Current main after `v0.6.11-ps1` — promotes `STAND 2`-`STAND 12`,
  `STAND 15`, `STAND 16`, `SUZY 1`, and `SUZY 2` after visual signoff. The STAND
  ...
  overlapping raft samples play without clipping.
...
| **37 / 63** | **2026-05-04** | **Current scene ledger after `SUZY 2` promotion; high/low packs were regenerated with a static-base foreground overlay for `MRAFT.BMP`, and SFX playback now leaves mixer headroom for overlapping raft samples** | **this doc, `scene-status.md`** |
