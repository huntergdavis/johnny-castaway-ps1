...
the VISITOR3 low third setup-segment promotion;
all 63 scenes remain validated, all 126 high/low rows are timing-bearing, and
the public headless battle card is `+0.3002%` over target / `99.7062%` target
speed).
...
latest VISITOR3 low third setup-segment promotion, the public battle card is
now `+0.3002%` over target / `99.7062%` target speed; the raw signed
optimization rollup is about `-0.4167%` / `100.4366%`.
...
segment for sectors `206..230`.
VISITOR3 high is now
`1096/1040` with `blocking_vb=71`; low is
...
high/low are now
`1475/1433` and `1479/1435`, WALKSTUF3 high/low are `2310/2290` and
...
- Current `main` after `v0.8.8-ps1` — promotes the BUILDING2 high `206..230`
  ...
  The public battle card is `+0.3002%` / `99.7062%` with `119` green, `6` yellow, and `1` orange row;
  ...
  `1096/1040` with blocking/read time `71`,
