...
Each holiday gets **5 variants** for owner review. v1–v4 are
hand-authored in the batch files; v5 is auto-generated by
`holidays_art_lib.as_night` post-processing v1.
...
- **Variant 4 — playful**: exaggerated/comedic — sweat drops, tilted
  poses, expressive details that didn't fit v1.
- **Variant 5 — night (auto)**: procedural recolor of v1. Background
  becomes DEEPBLUE, sun-yellow → SKY moonglow, sunset orange →
  PURPLE twilight, plus a moon and stars sprinkled into the former
  sky region. Already-night v1s route through a "dawn" path instead
  (PURPLE → ORANGE → YELLOW gradient with a sun) so the alternate
  always reads as a meaningfully different scene.
