...
- New `src/ps1_perf.c/h` module landed with a level-gated logging API:
...
- **Custom font path (shipped)**: embedded 8x8 ASCII glyph table
  (`pmFontBits[96][8]` in `src/pause_menu.c`), pre-doubled to 16x16 at
...
- Files: `src/pause_menu.c/h`. Design: [pause-menu-design.md](pause-menu-design.md).
...
- Files: `src/spi.c/h`, integrated into `src/events_ps1.c`. The pause
  menu, day-night override, and any future input feature all sit on top
  of this driver.

## 6. Memcard support (SHIPPED)

- `src/memcard.c/h` saves user pause-menu settings (sound mute,
