...
promotion, and the v204 low persistent setup segment:
...
persistent setup segment for sectors `281..305`. The v202 pass keeps
...
`scratch/ps1-perf-iterate/visitor3-low-segment281-305-v204-broad-norequire/20260509-044056-764102/summary.json`.
...
| 5 | Next VISITOR3 pass: build scheduler-owned/generated append timing for additional balanced 16/24-sector candidates without more hot hand tables, or move to generated motion/residual/precomposed pack data. | A read-group win must lower loop or blocking without increasing visible refill; scheduler counters explain why it fired. The current VISITOR3 baseline is high `1101/1030`, blocking `114`, loop-read time `116`, and low `1102/1032`, blocking `124`, loop-read time `131`; v204 proves one bounded low setup segment can promote, but setup debt must be justified by active-loop payoff. Remaining VISITOR3 local read-table probes should be treated as exhausted unless generated metadata can add them size-neutrally. |
