...
| VISITOR3 raw stream windows and standalone groups | Do not retry scalar window sizes; fresh-baseline high/low sweeps failed. VISITOR3 high `72..84` proves selective grouping can still pay, but the post-recovery generated groups high `163..175` and low `158..170` both reported `group_hits=0`, with low regressing visible timing. The later high `170..186` larger-group probe is accepted against same-source canaries, but the `144..156` follow-up regressed `loop_vb 1455 -> 1459` and `blocking_vb 361 -> 367` despite saving one nominal read. Do not remove or disable the accepted high groups under the current matrix: the correct-baseline disable probe regressed `loop_vb 1455 -> 1456`, and the branch-removal probe exposed FISHING1 phase sensitivity. Continue only with scene-local/generated metadata that proves scheduler timing, direct16/selective preprocessing, or prepared visual ownership; current-fit append ownership alone is not enough. |
