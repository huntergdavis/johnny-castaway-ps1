...
follow-up:
`+0.3558%` public average over target / `99.6529%` public target speed across
all `126` timing-bearing rows. The raw signed optimization matrix is
`-0.4126%` / `100.4367%`. Since the compact full-matrix baseline was about
`17.4%` over target / `87.1%` target speed, the headless methodology has
removed about `17.04` public over-target points and added about `12.55`
...
low to `1091/1035`, the v237 resident copy moves low to `1088/1035`, and the
v248 no-op residual compaction moves low to `1086/1035`,
cutting overrun `56 -> 51`, blocking `103 -> 93`, loop reads `18 -> 17`,
loop-read time `110 -> 100`, and due misses `17 -> 16`, with the current
scene total at `scene_vb=1412`.
...
`blocking_vb=59`, `loop_read_vb=59`, and due misses `11`. The later v248 pass
owns the current low profile:
`scene_vb=1412`, `loop_vb=1086`, `target_vb=1035`, `overrun_vb=51`,
`blocking_vb=93`, `loop_read_vb=100`, and due misses `16`. Hidden refill stays
