...
profiles. The current all-scene battle card is now `0.6400%` over target /
`99.6118%` target speed across `120` timing-bearing rows.
...
This checkpoint later moved to `0.6400%` over target / `99.6118%` target
speed after the ACTIVITY9 low cleanup-metadata compaction; the current
...
remaining absolute gaps are now VISITOR3, WALKSTUF1, BUILDING4, BUILDING6,
JOHNNY2, ACTIVITY9, residual BUILDING2 pressure, and
...
residual conversion for both validated packs. High tide improves the
same-commit current active loop `1833 -> 1801`, lowers blocking `401 -> 369`,
and keeps the pack LBA plus PS-EXE bucket fixed; low tide improves `1833 ->
1800` and lowers blocking `399 -> 377`. The matrix row also replaces stale
v0.6.4 timing, so the public rollup moves backward even though the current
pack is faster than its same-commit FGP2 control.
...
| 5 | Next VISITOR3 pass: build scheduler-owned/generated append timing for additional balanced 16/24-sector candidates without more hot hand tables. | A read-group win must lower loop or blocking without increasing visible refill; scheduler counters explain why it fired. The current cleanup-compact canary is `loop_vb=1406/1405`, `blocking_vb=296/301`, and `loop_read_vb=333/341`; the last detail pass still showed secondary compose/advance pressure, but CD ownership remains first priority. |
...
| JOHNNY2 pal4 padded FGP3 | Done; keep. Both current JOHNNY2 validated packs shrink as FGP3 temporal residuals and fit when padded back to the original `288637` byte CD footprint. Against the same-commit FGP2 control, high improves `loop_vb 1833 -> 1801`, `overrun_vb 82 -> 50`, `blocking_vb 401 -> 369`, and scene time `2089 -> 2080`; low improves `loop_vb 1833 -> 1800`, `overrun_vb 82 -> 49`, `blocking_vb 399 -> 377`, and scene time `2091 -> 2080`. The row refresh replaces stale v0.6.4 matrix timing; after the later MARY2 prefetch-relief promotion, the current exact matrix rollup is `0.8228%` over target / `99.4872%` target speed. |
