...
Current all-scene rollup after the BUILDING2 high restore-minus-current cleanup:
`-0.0595%` average over target / `100.1296%` target speed across `120`
...
removed about `17.46` over-target points and added about `13.03`
target-speed points.
...
`loop_reads=57`. VISITOR3 high/low, BUILDING4 high/low, ACTIVITY9 low, and
FISHING1 high controls stayed exact-flat.
...
accepted profiles. This checkpoint later moved to `-0.0595%` over target /
`100.1296%` target speed after the VISITOR3 low read-table prune, WALKSTUF1
...
cleanup moves it to `-0.0595%` over target / `100.1296%` target speed.
