...
Current all-scene rollup after the BUILDING4 restore-minus-current pack pass:
`-0.0193%` average over target / `100.0943%` target speed across `120`
...
removed about `17.42` over-target points and added about `12.99`
target-speed points.

Latest promoted BUILDING4 restore-minus-current baseline: subtract current
...
accepted profiles. The next true rows are WALKSTUF1, VISITOR3, BUILDING2,
BUILDING6, and selective upload-ready preprocessing.
