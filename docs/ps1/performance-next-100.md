...
| 23 | JOHNNY1 high/low | Closed by v642 for pack-only suppression: the current pack has no redundant same-frame cleanup or duplicate/no-op payloads; the useful signal is frame `2`'s large black cleanup, which needs a runtime primitive. | Reopen only after reclaiming code headroom for a smaller black-clear opcode/helper or generating code-neutral dirty-upload metadata with exact visual proof. |
