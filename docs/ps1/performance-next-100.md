...
Current all-scene rollup after the BUILDING2 low `365..381` grouped-read pass:
`-0.0670%` average over target / `100.1368%` target speed across `120`
...
Latest promoted BUILDING2 low read-group baseline: add a low-tide-only retained
...
accepted profiles. This checkpoint later moved to `-0.0670%` over target /
`100.1368%` target speed after the VISITOR3 low read-table prune, WALKSTUF1
high setup-prime cap retune, VISITOR3 high/low offscreen draw clips, BUILDING2
high restore-minus-current cleanup, and BUILDING2 high/low grouped-read passes;
