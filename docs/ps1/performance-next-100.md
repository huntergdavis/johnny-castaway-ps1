...
due `3`, and target speed `96.303%`. The accepted low third-segment canary
...
| VISITOR3 high early `34..37` direct/raw-gap cluster | Do not retry as scalar direct-stage suppression, 3-sector micro-windowing, or raw retained-gap relocation for frames `56..58`. The best forms improved visible loop/overrun but all strict-safe candidates either regressed hidden refill (`5 -> 6` or `5 -> 16`) or moved the debt into visible blocking. Reopen only with generated deadline ownership that schedules the cluster earlier than the unsafe slack slot. |
