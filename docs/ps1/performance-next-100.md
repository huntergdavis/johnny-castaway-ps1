...
or long-run heap stability. The current all-scene battle card is `2.7833%`
over target / `97.6931%` target speed across `120` timing-bearing rows after
the `activity10-johnny3-v072-prefetch-relief` optimization. The largest
remaining absolute gaps are now VISITOR3, BUILDING2, ACTIVITY9, WALKSTUF1,
JOHNNY1, and WALKSTUF3, not FISHING1.
