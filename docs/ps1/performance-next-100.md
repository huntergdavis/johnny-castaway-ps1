...
Current all-scene rollup after the VISITOR3 low scoped composite-helper pass:
`-0.0694%` average over target / `100.1388%` target speed across `120`
...
Latest promoted VISITOR3 low code-shape baseline: compile the non-FGP3
...
the VISITOR3 low scoped composite-helper pass; the current rollup is tracked
above.
