...
| 12 | Done first pass: `scripts/analyze-fg2-preprocess-plans.py` now parses FGP3 cleanup/draw payloads and emits per-frame cap/saving hotspot reports. | VISITOR3 detail shows cap-hit frames `134..136` save `0%` under blanket x-band, while frames `121..133` have large non-cap savings, so the next probe can be selective before changing runtime code. |
