...
target-hull timing promotion, the v202/v206 high re-anchor CD-pressure
promotions, the v204 low persistent setup segment, and the v205 high persistent
setup segment:
`+0.3962%` public average over target / `99.6166%` public target speed across
all `126` timing-bearing rows. The raw signed optimization matrix is
`-0.3723%` / `100.4004%`. Since the compact full-matrix baseline was about
`17.4%` over target / `87.1%` target speed, the headless methodology has
removed about `17.00` public over-target points and added about `12.52`
...
`126` re-anchor payload. The v205/v206 pair keeps VISITOR3 high loop timing
flat at `1101/1030` while cutting blocking `114 -> 107`, loop reads `22 -> 20`,
loop-read time `116 -> 107`, due misses `21 -> 20`, and active payload
`527206 -> 511690`; the setup tradeoff raises total high `scene_vb 1405 -> 1414`.
...
cutting VISITOR3 low blocking to `124`.
...
| 5 | Next VISITOR3 pass: build scheduler-owned/generated append timing for additional balanced 16/24-sector candidates without more hot hand tables, or move to generated motion/residual/precomposed pack data. | A read-group win must lower loop or blocking without increasing visible refill; scheduler counters explain why it fired. The current VISITOR3 baseline is high `1101/1030`, blocking `107`, loop-read time `107`, and low `1102/1032`, blocking `124`, loop-read time `131`; v204/v205 prove bounded setup segments can promote or cut same-loop CD pressure, and v206 proves high-only generated re-anchors can still remove CD pressure after setup, but setup debt must be justified by active-loop payoff. The wider v205 high probes `261..285` and `269..285` cut more active CD work but regressed hidden refill `0 -> 3`, so remaining VISITOR3 local read-table probes should be treated as exhausted unless generated metadata can add them size-neutrally. |
