...
`+0.1830%` public average over target / `99.8185%` public target speed across
all `126` timing-bearing rows. The raw signed optimization matrix is about
`-0.5339%` / `100.5488%`. Since the compact full-matrix baseline was
...
target-speed points. Bands are now `126` green, `0` yellow, `0` orange, and
...
shape or generated refill ownership, not another D4 placement. The follow-up
terminal scalar rows `{256..268}` and `{239..251}` are also closed on this
baseline: both phase-0 rows stayed exact-flat at `1350/1065/1041`, overrun
`24`, blocking/refill `50/0`, reads/due `18/8`, and never fired
(`group_hits=0`). Pairing either row with phase `2` preserved the tempting
speed/blocking signal (`1064/1044`, overrun `20`, blocking `46`) but still
failed the strict hidden-refill gate with `prefetch_overrun_vb=1`, again with
...
keeps VISITOR3 low exact-flat at `1339/1065/1041`, overrun `24`,
...
gain. The follow-up retest of the closed entries `76`/`77` no-decode trim
pocket is also closed on this newer phase baseline. The pair trims `6910`
bytes and drops reads `42 -> 40`, but regresses to `1588/1345/1313`, overrun
`32`, blocking/refill `52/17`. Entry `76` alone repeats the same failure.
Entry `77` alone is closer (`2909` bytes trimmed, reads `42 -> 41`), but still
regresses to `1584/1341/1313`, overrun `28`, blocking/refill `46/14`; pairing
it with phase `2` is worse at `1594/1350/1312`, overrun `38`,
blocking/refill `55/23`. Keep this pocket closed until B2-high has new
restore/render slack or generated refill ownership. The prior BUILDING2-high phase promotion adds one B2-high-only
...
this is work-volume headroom rather than a VBlank speed win. The prior W1-high
entry `53` fixed-layout screen clip shrank that entry `3893 -> 2309` bytes and
removed `8969` offscreen cleanup pixels plus `1945` offscreen draw pixels. The
broad W1-high `43..56` clip is closed as phase-negative because it saved two
...
`32 -> 31`, blocking `80 -> 78`, and loop read time `106 -> 104`.
...
`29339 -> 27643`, removing `18160` cleanup pixels with no draw-span removal,
reducing runtime restore bytes `525020 -> 510842`, max restore bytes
`12684 -> 7712`, upload bytes `17185920 -> 17182720`, and dirty rows
`26853 -> 26848` with exact-flat five-yellow timing. The prior VISITOR3-low
fixed-layout previous-visible cleanup pass keeps file size, offsets, entry
sizes, LBA, and the PS-EXE bucket fixed while changing `94` low-pack entries,
dropping logical active payload `425729 -> 405835`, cleanup restore bytes
`456786 -> 53456`, runtime restore bytes `467962 -> 64632`, max restore bytes
`293544 -> 2962`, upload bytes `18113920 -> 17494400`, and dirty rows
`28303 -> 27335` with exact-flat five-yellow timing. The prior W1-high exact-entry preserve-entry-size screen-clip subset
...
| 38 | WALKSTUF1 high | Entry `53` from the `84..108` family and entries `127..131` from the later sector family are promoted as fixed-layout screen-clip headroom, while the broad `43..56` clip is closed as phase-negative. Continue only with narrower cleanup islands or generated ownership; do not retry whole sector families as plain pack clips. | Host transform must reduce spans/bytes while preserving exact pixels and D4 gates, and any read-count win must keep blocking/refill flat. |
...
| Compact FGP3/v4 compositor scoped `O2` | Do not promote or retry as a standalone compiler toggle. The v409 probe removed `optimize("Os")` from `grCompositePacked4CompactTemporalResidualToBackground()` and kept WALKSTUF1 low's pack LBA plus the `217088` byte EXE bucket fixed, but regressed low from `1484/1431`, blocking `72`, refill `22`, reads/due `67/12` to `1486/1428`, blocking `81`, refill `27`, reads/due `69/13`. The current five-yellow retest at `scratch/ps1-perf-iterate/compact-pal4-o2-five-yellow-current/20260522-014721-1645643/summary.json` stayed flat on VISITOR3-low but regressed B2-high to `1348/1310`, blocking/refill `56/20`, W1-high to `1476/1439`, blocking/refill `47/18`, and turned W1-low/V3-high loop signals into hidden refill/blocking debt. Keep the compositor scoped to `-Os`; revisit only with a split/layout-pinned rewrite or a generated data-shape change that reduces compositor work enough to dominate codegen phase drift. |
