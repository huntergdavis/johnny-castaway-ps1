...
`+0.3697%` public average over target / `99.6402%` public target speed across
all `126` timing-bearing rows. The raw signed optimization matrix is
`-0.3988%` / `100.4240%`. Since the compact full-matrix baseline was about
...
low frame-125/frame-126 resident re-anchor, and the v234/v237 frame-118/frame-127 resident copies
...
improves `122 -> 118`. The later v237 pass owns the current low profile:
...
VISITOR3 low to `1088/1035` and blocking `95`.
