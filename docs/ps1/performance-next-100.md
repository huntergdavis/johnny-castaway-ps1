...
high `213..229` read-group/slack4 promotion, and the v327 VISITOR3 low
frame128/frame129 resident-slot swap:
`+0.3273%` public average over target / `99.6792%` public target speed across
all `126` timing-bearing rows. The raw signed optimization matrix is
`-0.4397%` / `100.4616%`. Since the compact full-matrix baseline was about
...
misses `16`; v327 then moves the raw rollup to `-0.4397%` / `100.4616%` while
cutting current-layout VISITOR3 low to `1072/1040`, overrun `32`, blocking
`64`, loop-read time `64`, and due misses `11`.
