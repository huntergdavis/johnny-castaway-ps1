...
| 91 | Toolchain | Continue testing `-O2` on currently `-Os` source files, but do not retry whole-TU `foreground_pilot.c`, `jc_reborn.c`, or `resource.c`. | `foreground_pilot.c -O2` failed structurally, while `jc_reborn.c` and `resource.c -O2` stayed exact-flat while growing ELF; remaining whole-TU candidates are `sound_ps1.c` and `events_ps1.c`, one at a time. | Free performance if loop/setup/render counters improve; otherwise log exact-flat/no-promotion and keep current `-Os`. |
...
| 3 | 91 | Continue the `-O2` sweep across currently `-Os` hot/semi-hot TUs before any new hand-rolled code; `foreground_pilot.c`, `jc_reborn.c`, and `resource.c` whole-TU probes are rejected, so move to `sound_ps1.c`. |
