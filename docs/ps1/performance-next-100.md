...
second setup segment, and the v227 low frame-125/frame-126 resident re-anchor:
`+0.3751%` public average over target / `99.6354%` public target speed across
all `126` timing-bearing rows. The raw signed optimization matrix is
`-0.3934%` / `100.4192%`. Since the compact full-matrix baseline was about
...
low frame-125/frame-126 resident re-anchor inside that accepted segment.
...
resident re-anchor moves low again to `1095/1035`, cutting overrun `64 -> 60`,
blocking `112 -> 108`, loop reads `20 -> 19`, loop-read time `119 -> 115`,
and due misses `19 -> 18`, with the accepted cumulative setup tradeoff now
`scene_vb 1408 -> 1421`.
...
improves `122 -> 118`. The later v227 pass owns the current low profile:
`scene_vb=1421`, `loop_vb=1095`, `target_vb=1035`, `overrun_vb=60`,
`blocking_vb=108`, `loop_read_vb=115`, and due misses `18`. Hidden refill stays
...
cutting VISITOR3 low to `1095/1035` and blocking `108`.
...
| 5 | Next VISITOR3 pass: build scheduler-owned/generated append timing for additional balanced 16/24-sector candidates without more hot hand tables, or move to generated motion/residual/precomposed pack data. | A read-group win must lower loop or blocking without increasing visible refill; scheduler counters explain why it fired. The current VISITOR3 baseline is high `1089/1035`, blocking `83`, loop-read time `83`, and low `1095/1035`, blocking `108`, loop-read time `115`; v204/v205/v213/v214/v216 prove bounded setup residency can promote or cut same-loop CD pressure when hidden refill stays at zero, and v206/v207/v227 prove generated re-anchors can still remove CD pressure after setup when the payload is placed inside accepted residency, but setup debt must be justified by active-loop payoff. The wider v205 high probes `261..285` and `269..285`, low setup-prime caps above `208 KiB`, and v215 unguarded low second segment either added hidden refill or regressed visible timing, so remaining VISITOR3 local read-table/setup-cap probes should be treated as exhausted unless generated metadata can add them size-neutrally and deadline-aware. |
