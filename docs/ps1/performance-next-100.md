...
setup segment, the v213/v214 high setup-prime expansions, and the v216
guarded low second setup segment:
`+0.3783%` public average over target / `99.6326%` public target speed across
all `126` timing-bearing rows. The raw signed optimization matrix is
`-0.3903%` / `100.4164%`. Since the compact full-matrix baseline was about
`17.4%` over target / `87.1%` target speed, the headless methodology has
removed about `17.02` public over-target points and added about `12.53`
...
v214 high-only setup-prime expansion from `288 KiB` to `320 KiB`, and the
v216 guarded low second setup segment for sectors `150..174`.
...
The v216 second setup segment moves low `1102/1032 -> 1098/1034`, cuts overrun
`70 -> 64`, blocking `124 -> 112`, loop reads `23 -> 20`, loop-read time
`131 -> 119`, and due misses `22 -> 19`, with the accepted cumulative setup
tradeoff `scene_vb 1408 -> 1424`.
...
improves `122 -> 118`. The later v216 pass owns the current low profile:
`scene_vb=1424`, `loop_vb=1098`, `target_vb=1034`, `overrun_vb=64`,
`blocking_vb=112`, `loop_read_vb=119`, and due misses `19`. Hidden refill stays
`prefetch_overrun_vb=0` on both tides. The broad no-regression gate is
`scratch/ps1-perf-iterate/visitor3-low-dual-segment150-174-slack4-v216-broad/20260509-103329-2754585/summary.json`;
...
`scratch/ps1-perf-iterate/visitor3-low-dual-segment150-174-slack4-v216-low/20260509-103215-2746966/summary.json`.
...
| 5 | Next VISITOR3 pass: build scheduler-owned/generated append timing for additional balanced 16/24-sector candidates without more hot hand tables, or move to generated motion/residual/precomposed pack data. | A read-group win must lower loop or blocking without increasing visible refill; scheduler counters explain why it fired. The current VISITOR3 baseline is high `1089/1035`, blocking `83`, loop-read time `83`, and low `1098/1034`, blocking `112`, loop-read time `119`; v204/v205/v213/v214/v216 prove bounded setup residency can promote or cut same-loop CD pressure when hidden refill stays at zero, and v206/v207 prove high-only generated re-anchors can still remove CD pressure after setup, but setup debt must be justified by active-loop payoff. The wider v205 high probes `261..285` and `269..285`, low setup-prime caps above `208 KiB`, and v215 unguarded low second segment either added hidden refill or regressed visible timing, so remaining VISITOR3 local read-table/setup-cap probes should be treated as exhausted unless generated metadata can add them size-neutrally and deadline-aware. |
