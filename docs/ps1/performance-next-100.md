...
| 63a | WALKSTUF1 low | Treat `141..153` as generated-owner/additive-residency-only. The `141..153` setup retarget structurally exhausted CACHE, while the `141..147` and `147..153` same-footprint swaps improved loop but regressed target, blocking, and refill because they displaced the accepted `179..185` slice. | Do not retry as a direct swap for `179..185`; preserve that slice unless a clean-byte or pack/data-shape win creates additive headroom. |
...
| 70 | VISITOR3 low | Test a targeted no-decode boundary trim immediately before sectors `248..272`, not another whole-pack reorder or append over that pocket. | Host CD-head replay must predict fewer visible reads while retaining `prefetch_overrun_vb=0` and preserving all accepted retained setup windows. |
