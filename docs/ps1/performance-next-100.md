...
Current all-scene rollup after the VISITOR3 low exit-right offscreen draw clip:
`-0.0266%` average over target / `100.0998%` target speed across `120`
...
removed about `17.43` over-target points and added about `13.00`
target-speed points.
...
rows are remaining VISITOR3 scheduler/data-shape work, WALKSTUF1 low/high,
...
accepted profiles. This checkpoint later moved to `-0.0266%` over target /
`100.0998%` target speed after the VISITOR3 low read-table prune, WALKSTUF1
high setup-prime cap retune, VISITOR3 high-only offscreen draw clip, and
VISITOR3 low exit-right offscreen draw clip; the current rollup is tracked
above.
