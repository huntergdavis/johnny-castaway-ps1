...
or long-run heap stability. The current all-scene battle card is `0.8847%`
over target / `99.4269%` target speed across `120` timing-bearing rows after
the `mary2-fgp3-padded-v081` refresh. The largest remaining absolute gaps are
now VISITOR3, WALKSTUF1, BUILDING2, BUILDING4, MARY2, BUILDING6, JOHNNY2, and
...
| MARY5 pal4 padded FGP3 | Done; keep. Both current MARY5 validated packs shrink as FGP3 temporal residuals and fit when padded back to the original `646602` byte CD footprint. High improves `loop_vb 1591 -> 1581`, `overrun_vb 9 -> 0`, `blocking_vb 8 -> 5`, `prefetch_overrun_vb 8 -> 0`, and `loop_reads 49 -> 42`; low improves `loop_vb 1592 -> 1581`, `overrun_vb 11 -> 0`, `blocking_vb 10 -> 6`, `prefetch_overrun_vb 10 -> 2`, and `loop_reads 49 -> 42`. This checkpoint later moved to `0.8847%` over target / `99.4269%` target speed after the JOHNNY2 and MARY2 stale-row refreshes; the current rollup is tracked at the top of this file. |
| JOHNNY2 pal4 padded FGP3 | Done; keep. Both current JOHNNY2 validated packs shrink as FGP3 temporal residuals and fit when padded back to the original `288637` byte CD footprint. Against the same-commit FGP2 control, high improves `loop_vb 1833 -> 1801`, `overrun_vb 82 -> 50`, `blocking_vb 401 -> 369`, and scene time `2089 -> 2080`; low improves `loop_vb 1833 -> 1800`, `overrun_vb 82 -> 49`, `blocking_vb 399 -> 377`, and scene time `2091 -> 2080`. The row refresh replaces stale v0.6.4 matrix timing; after the later MARY2 stale-row refresh, the current exact matrix rollup is `0.8847%` over target / `99.4269%` target speed. |
| MARY2 pal4 padded FGP3 | Done; keep. Both current MARY2 validated packs shrink as FGP3 temporal residuals and fit when padded back to the original `582189` byte CD footprint. Against the same-commit FGP2 control, high improves `scene_vb 2638 -> 2583`, `loop_vb 2385 -> 2330`, and `overrun_vb 135 -> 78`; low improves `scene_vb 2638 -> 2581`, `loop_vb 2384 -> 2327`, and `overrun_vb 134 -> 75`. LBAs and PS-EXE bucket stay fixed. The row refresh replaces stale v0.6.8 matrix timing, so the current exact matrix rollup is `0.8847%` over target / `99.4269%` target speed. |
