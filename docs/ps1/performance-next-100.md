...
frame128/frame129 resident-slot swap, and the v331 WALKSTUF1 low
staged-prepare scheduler fallback:
`+0.3267%` public average over target / `99.6798%` public target speed across
all `126` timing-bearing rows. The raw signed optimization matrix is
`-0.4403%` / `100.4621%`. Since the compact full-matrix baseline was about
...
public overrun to `53`, blocking to `72`, and prefetch overrun to `22`.
