...
before frame132. The promoted v501 gate improves scene `1402 -> 1400`, active
loop/target `1067/1039 -> 1065/1039`, overrun `28 -> 26`, visible blocking
`45 -> 41`, loop-read VBlanks `45 -> 41`, loop reads `8 -> 7`, and due misses
`8 -> 7`; hidden refill and prefetch overrun stay `0`, pack LBA/sectors stay
`22611/760`, the pack footprint stays `1555450` bytes, and the PS-EXE bucket
stays `217088`. VISITOR3 low, BUILDING2 high/low, and WALKSTUF1 high/low
canaries stayed exact-flat on accepted profiles. This promotion adds about
`0.0015` public over-target points removed and `0.0015` public target-speed
points gained, moving VISITOR3 high to `97.56%` target speed.
...
the refreshed read-candidate matrix now has `0` standalone, `0` guarded, and
`0` scheduler-owned open scalar rows. The remaining under-99 scenes are close
...
| --- | --- | --- | --- |
| 1 | BUILDING2 low | Turn the `218..230` near miss into a generated owned row: fire only after the hidden refill owner has already paid the cluster's first sector, then group the remaining sectors. The scalar row was one refill VBlank from promotion, so this is the most direct route to a 2-5 VBlank win. | Rebuild a fresh baseline, emit metadata only for low tide, and require raw `1614/1344`-style timing with `prefetch_overrun_vb=0`. |
| 2 | BUILDING2 low | Split `218..230` into two generated subgroups with different owners instead of one scalar slack gate. Scalar `minSlack=8` was unsafe and `9` was off; the missing state is "fire only for the second half." | Compare `218..224 + 224..230` metadata against the v526 canary with strict zero-refill. |
...
| 16 | VISITOR3 high | Rebuild the under-99 read-plan artifact root after v570 closures before trusting any more sector rows. The current scalar root was stale for `211..235` and `228..252`. | Generate fresh read plans for all nine yellow rows and diff old/new candidate classifications. |
...
| 21 | VISITOR3 low | Search for another existing setup-prime gap relocation like v477/v510, but only for already-small D4 payloads. The successful low wins moved small payloads into already paid bytes; new retained segments failed. | Host gap finder must list candidate offset, sector, and unchanged pack size; canary high/building2/walkstuf after gate. |
...
| 23 | JOHNNY1 high/low | Pack-authored black-cleanup suppression for frames whose cleanup area restores known black/background pixels. Direct retained reads are closed and JOHNNY1 now looks render/upload-bound. | Offline pixel proof against both tides, then gate both tides for reduced restore/upload bytes with flat timing accepted as work reduction. |
| 24 | JOHNNY1 high/low | Code-neutral dirty-upload coalescing for black-background rows: precompute the row mask in pack metadata and reuse existing upload path. Avoid global narrow-upload code that previously blew the EXE bucket. | Require no PS-EXE bucket change and no `grDrawBackground` growth before timing. |
| 25 | JOHNNY1 high/low | Pack-side no-op cleanup aliasing for held frames, preserving event cadence but skipping redundant restore bytes. The scene has `due_misses=0`, so render cleanup is likelier than CD. | Host diff must prove identical pixels and lower cleanup bytes; PS1 gate may promote flat timing if work-volume drops. |
...
| WALKSTUF1 high read group `178..194` current-baseline retest | Still closed after the v339 current-baseline retest. The latest probe reduced loop reads `69 -> 66`, but regressed high `scene_vb 1768 -> 1774`, active loop `1480 -> 1486`, target `1429 -> 1427`, overrun `51 -> 59`, blocking `85 -> 98`, and due misses `16 -> 17`; prefetch only improved `26 -> 23`. Do not revisit as a direct hand table; only generated deadline ownership or pack-side removal can make this neighborhood viable. |
...
| VISITOR3 motion-copy frames `119..123` plus high frame `115` | Done; keep as part of the current VISITOR3 baseline, now superseded by later sparse/re-anchor/setup/no-op/resident-slot/frame-delta/setup-segment passes. The v181 scene-specific compact motion marker moves already-composited background rows, restores exposed clean spans, and draws only residual pixels; v182 applies the same family of state-hull motion-copy payload to high-tide frame `115`. Later v188/v189/v193/v202/v204/v205/v206/v207/v213/v214/v216/v227/v234/v237/v238/v248/v249/v291/v292/v299/v302/v327/v338/v452/v460/v462/v464/v477/v501/v510 passes move the accepted profile to high `1065/1039` and low `1062/1040` while preserving the `1555450` byte footprints, current LBAs `22611/23371`, and the `217088` byte PS-EXE bucket. Next retries should expand motion-copy through generated eligibility, not hand-tail repoints. |
