...
setup-prime copy, and the v302 VISITOR3 low frame-128 resident segment copy:
`+0.3184%` public average over target / `99.6876%` public target speed across
all `126` timing-bearing rows. The raw signed optimization matrix is
`-0.4501%` / `100.4714%`. Since the compact full-matrix baseline was about
...
`9`; the v302 VISITOR3 low frame-128 resident segment copy moves the current
rollup to `-0.4501%` / `100.4714%` while cutting VISITOR3 low to `1071/1039`,
overrun `32`, blocking `63`, loop-read time `63`, and due misses `11`.
...
| WALKSTUF1 low gap1 prefix pack | Done; keep as the current WALKSTUF1 low baseline. The v289 pack-only promotion applies the same one-pixel cleanup-gap merge to low-tide frames `0..37`, changing `29` frames, saving `1126` active payload bytes, and preserving the `1535263` byte padded footprint. Focused and broad gates improve low `scene_vb 1779 -> 1770`, active loop `1487/1424 -> 1478/1428`, overrun `63 -> 50`, blocking `95 -> 75`, and due misses `15 -> 13`; hidden refill stays `25`, loop reads stay `69`, pack LBA stays `25495`, and WALKSTUF1 high plus VISITOR3 high/low, BUILDING2 high/low, ACTIVITY9 low, FISHING1 high, JOHNNY1 high, and MARY3 high/low stay flat. This is superseded in the rollup by the v291/v292/v299/v302 VISITOR3 promotions; current public rollup is `+0.3184%` over target / `99.6876%` target speed. |
| WALKSTUF1 low prefix gap6 compaction | Do not promote or retry as a larger cleanup-gap threshold. Gap6 moved frame `39` fully into paid setup-prime coverage and improved CD pressure (`blocking_vb 75 -> 70`, `loop_reads 69 -> 67`, `due_misses 13 -> 11`), but the extra cleanup geometry regressed `scene_vb 1770 -> 1774`, active loop `1478/1428 -> 1482/1426`, overrun `50 -> 56`, and hidden refill `25 -> 31`. Any next attempt to make frame `39` resident must shrink or relocate bytes without adding thousands of restored pixels. |
