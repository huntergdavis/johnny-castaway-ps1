...
setup-prime copy, the v302 VISITOR3 low frame-128 resident segment copy, and
the v305 WALKSTUF1 low gap6-prefix plus slack-guard promotion:
`+0.3156%` public average over target / `99.6902%` public target speed across
all `126` timing-bearing rows. The raw signed optimization matrix is
`-0.4529%` / `100.4740%`. Since the compact full-matrix baseline was about
...
are exact-flat. Low now applies the gap6 prefix pack with the scene-wide
3-VBlank slack guard and improves `1478/1428 -> 1475/1430`, overrun
`50 -> 45`, blocking `75 -> 67`, hidden refill `25 -> 21`, loop reads
`69 -> 66`, and loop-read time `301 -> 282`.
...
v305 WALKSTUF1 low gap6-prefix plus slack-guard promotion moves the current
raw rollup to `-0.4529%` / `100.4740%` while cutting WALKSTUF1 low to
`1475/1430`, overrun `45`, blocking `67`, hidden refill `21`, loop-read time
`282`, and due misses `12`.
...
| WALKSTUF1 high read group `178..194` current-baseline retest | Still closed after the v308 current-guard retest. The group reduced loop reads `69 -> 65` and blocking `90 -> 88`, but regressed high `scene_vb 1766 -> 1770`, active loop `1477/1431 -> 1481/1426`, overrun `46 -> 55`, and prefetch overrun `19 -> 25`. Do not revisit as a direct hand table; only generated deadline ownership or pack-side removal can make this neighborhood viable. |
