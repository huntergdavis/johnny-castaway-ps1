...
setup segment, and the v213 high setup-prime expansion:
`+0.3930%` public average over target / `99.6194%` public target speed across
all `126` timing-bearing rows. The raw signed optimization matrix is
`-0.3755%` / `100.4032%`. Since the compact full-matrix baseline was about
`17.4%` over target / `87.1%` target speed, the headless methodology has
removed about `17.01` public over-target points and added about `12.52`
...
and the v213 high-only setup-prime expansion from `232 KiB` to `288 KiB`.
The v213 high baseline moves VISITOR3 high to `1099/1032`, cuts overrun
`71 -> 67`, blocking `106 -> 96`, loop reads `20 -> 17`,
loop-read time `106 -> 96`, and due misses `20 -> 17`; the setup tradeoff
raises total high `scene_vb 1414 -> 1423`.
...
The v204 setup segment moves low `1108/1028 -> 1102/1032`, cuts overrun
`80 -> 70`, blocking `139 -> 124`, loop reads `27 -> 23`, loop-read time
`148 -> 131`, and due misses `25 -> 22`, with the accepted setup tradeoff
`scene_vb 1408 -> 1415`.
...
improves `122 -> 118`. The later v204 pass owns the current low profile:
`scene_vb=1415`, `loop_vb=1102`, `target_vb=1032`, `overrun_vb=70`,
`blocking_vb=124`, `loop_read_vb=131`, and due misses `22`. Hidden refill stays
...
`scratch/ps1-perf-iterate/visitor3-setup-prime-288-v210-high/20260509-092529-2362759/summary.json`.
...
cutting VISITOR3 low blocking to `124`. The VISITOR3 high setup-prime v213 pass
then moves the raw signed matrix to `-0.3755%` over target / `100.4032%`
target speed while cutting VISITOR3 high to `1099/1032` and blocking `96`.
...
| 5 | Next VISITOR3 pass: build scheduler-owned/generated append timing for additional balanced 16/24-sector candidates without more hot hand tables, or move to generated motion/residual/precomposed pack data. | A read-group win must lower loop or blocking without increasing visible refill; scheduler counters explain why it fired. The current VISITOR3 baseline is high `1099/1032`, blocking `96`, loop-read time `96`, and low `1102/1032`, blocking `124`, loop-read time `131`; v204/v205/v213 prove bounded setup residency can promote or cut same-loop CD pressure, and v206/v207 prove high-only generated re-anchors can still remove CD pressure after setup, but setup debt must be justified by active-loop payoff. The wider v205 high probes `261..285` and `269..285` plus low setup-prime caps above `208 KiB` either added hidden refill or regressed visible timing, so remaining VISITOR3 local read-table/setup-cap probes should be treated as exhausted unless generated metadata can add them size-neutrally. |
