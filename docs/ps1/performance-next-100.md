...
| 5 | Next VISITOR3 pass: build scheduler-owned/generated append timing for additional balanced 16/24-sector candidates without more hot hand tables. | A read-group win must lower loop or blocking without increasing visible refill; scheduler counters explain why it fired. The current cleanup-compact plus guarded-group canary is `loop_vb=1406/1405`, `blocking_vb=293/301`, and `loop_read_vb=332/341`; the last detail pass still showed secondary compose/advance pressure, but CD ownership remains first priority. |
