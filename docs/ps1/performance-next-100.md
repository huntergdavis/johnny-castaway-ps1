...
Current all-scene rollup after the BUILDING2 high `60..72` grouped-read pass:
`-0.0650%` average over target / `100.1351%` target speed across `120`
...
removed about `17.47` over-target points and added about `13.04`
target-speed points.

Latest promoted BUILDING2 high read-group baseline: add a high-tide-only
...
`loop_reads=57`. This checkpoint is now followed by the high-tide `60..72`
...
rows are WALKSTUF1 low/high, remaining VISITOR3 scheduler/data-shape work,
...
accepted profiles. This checkpoint later moved to `-0.0650%` over target /
`100.1351%` target speed after the VISITOR3 low read-table prune, WALKSTUF1
high setup-prime cap retune, VISITOR3 high/low offscreen draw clips, and the
BUILDING2 high restore-minus-current cleanup; the current rollup is tracked
above.
...
cleanup moves it to `-0.0650%` over target / `100.1351%` target speed.
...
| Compact FGP3/v4 metadata reader inline | Done; keep. `grReadCompactSpanU16` is now a default inline helper instead of a noinline `-Os` helper. VISITOR3 high/low improve to `1357/1023` and `1361/1023`, BUILDING2 high/low improve to `1394/1301` and `1385/1303`, ACTIVITY9 low remains timing-flat at `2085/2058` with an accepted `blocking_vb 28 -> 29`, and FISHING1 high remains exact-flat under target at `1068/1074`. This checkpoint later moved to `-0.0650%` over target / `100.1351%` target speed after the VISITOR3/BUILDING4/BUILDING2 restore-minus-current pack passes, WALKSTUF1 high setup-prime cap retune, and VISITOR3 high/low offscreen draw clips. |
| VISITOR3 high read-group table prune | Done; keep as code headroom. The remaining high-tide local table (`138..162`, `170..186`, `230..242`) is exhausted after compact-u16 inline and no longer changes runtime metrics. Removing it keeps VISITOR3 high/low, BUILDING2 high/low, ACTIVITY9 low, and FISHING1 high exact-flat while shrinking `foregroundPilotPlay` by `48` bytes. This checkpoint later moved to `-0.0650%` over target / `100.1351%` target speed after the VISITOR3/BUILDING4/BUILDING2 restore-minus-current pack passes, WALKSTUF1 high setup-prime cap retune, and VISITOR3 high/low offscreen draw clips. |
| VISITOR3 restore-minus-current cleanup | Done; keep as pack-side data-shape win. FGP3/v4 cleanup spans now omit intervals redrawn by the same current frame while leaving draw pixels, padded pack size, pack LBAs, and PS-EXE size fixed. VISITOR3 high/low improve to `1139/1024` and `1140/1024`, blocking drops to `191/194`, prefetch overrun drops to `0/0`, active payload drops `981514 -> 737600`, and runtime restore bytes drop `973290 -> 498676`. This checkpoint later moved to `-0.0650%` over target / `100.1351%` target speed after the BUILDING4/BUILDING2 restore-minus-current pack passes, WALKSTUF1 high setup-prime cap retune, and VISITOR3 high/low offscreen draw clips. |
| BUILDING4 restore-minus-current cleanup | Done; keep as pack-side data-shape win. FGP3/v4 cleanup spans now omit intervals redrawn by the same current frame while leaving draw pixels, padded pack size, pack LBAs, and PS-EXE size fixed. BUILDING4 high/low improve to `2844/2816` and `2855/2815`, blocking drops to `37/46`, active payload drops `1032442 -> 855284`, and runtime restore bytes drop `1229878 -> 546950`. This checkpoint later moved to `-0.0650%` over target / `100.1351%` target speed after VISITOR3 code-headroom, WALKSTUF1 high setup-prime cap, VISITOR3 high/low offscreen draw-clip, and BUILDING2 high restore-minus-current promotions. |
