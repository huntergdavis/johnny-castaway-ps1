...
| 91 | Toolchain | Continue testing `-O2` on currently `-Os` source files, but do not retry whole-TU `foreground_pilot.c`, `jc_reborn.c`, `resource.c`, or `sound_ps1.c`. | `foreground_pilot.c -O2` failed structurally, while `jc_reborn.c`, `resource.c`, and `sound_ps1.c -O2` stayed exact-flat while growing ELF; the remaining hot/semi-hot whole-TU candidate is `events_ps1.c`. | Free performance if loop/setup/render counters improve; otherwise log exact-flat/no-promotion and keep current `-Os`. |
...
| 3 | 91 | Continue the `-O2` sweep across currently `-Os` hot/semi-hot TUs before any new hand-rolled code; `foreground_pilot.c`, `jc_reborn.c`, `resource.c`, and `sound_ps1.c` whole-TU probes are rejected, so move to `events_ps1.c`. |
