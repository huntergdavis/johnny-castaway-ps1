...
or long-run heap stability. The current all-scene battle card is `0.9493%`
over target / `99.3794%` target speed across `120` timing-bearing rows after
the `activity9-window-v072c` optimization. The largest
...
| BUILDING2 high standalone visible-cost groups | Do not retry `96..104`, `66..78`, `325..331`, or adjacent one-off hand tables. `96..104` saved one nominal read but worsened visible blocking/due misses, while `66..78` was loop/blocking/read-flat with only a one-VBlank hidden-refill change. BUILDING2 high needs scheduler-owned timing or selective upload-ready/preprocessed pack data. |
