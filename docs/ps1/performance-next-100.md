...
Current all-scene rollup after the VISITOR3 high-only offscreen draw clip:
`-0.0249%` average over target / `100.0985%` target speed across `120`
...
removed about `17.42` over-target points and added about `13.00`
target-speed points.
...
`VIST3LOW.FG2` untouched. The pack remains `1555450` bytes, all entry sizes
and offsets are preserved, pack LBA stays `22472`, and the `215040` byte
PS-EXE bucket is unchanged. High improves `loop_vb 1139 -> 1137`,
`overrun_vb 115 -> 113`, and `blocking_vb 191 -> 190`; low stays exact-flat
at `1140/1024`, and BUILDING2, BUILDING4, ACTIVITY9, and FISHING1 controls
stay exact-flat. This is now the VISITOR3 high baseline.

Latest promoted WALKSTUF1 high setup-prime baseline: give high tide its own
...
ACTIVITY9 low, and FISHING1 high controls stayed exact-flat. The next true
rows are VISITOR3 low/scheduler-owned data-shape work, WALKSTUF1 low/high,
BUILDING2, BUILDING6, and selective upload-ready preprocessing.
...
The VISITOR3 offscreen-clip lane is closed as a both-tide standalone pack
transform, but the high-only data-size-preserving subset is promoted.
...
overrun and improved high `1139 -> 1137`, but low tide repeatedly regressed to
`1151/1024` with `blocking_vb 194 -> 200`. Runtime clipping already discards
these pixels visually; keep the high-only trim, but any future low-side retry
needs a scheduler-costed pack planner that preserves low-tide phase, not
another blind payload rewrite.
...
accepted profiles. This checkpoint later moved to `-0.0249%` over target /
`100.0985%` target speed after the VISITOR3 low read-table prune, WALKSTUF1
high setup-prime cap retune, and VISITOR3 high-only offscreen draw clip; the
...
moves it to `-0.0249%` over target / `100.0985%` target speed.
...
| Compact FGP3/v4 metadata reader inline | Done; keep. `grReadCompactSpanU16` is now a default inline helper instead of a noinline `-Os` helper. VISITOR3 high/low improve to `1357/1023` and `1361/1023`, BUILDING2 high/low improve to `1394/1301` and `1385/1303`, ACTIVITY9 low remains timing-flat at `2085/2058` with an accepted `blocking_vb 28 -> 29`, and FISHING1 high remains exact-flat under target at `1068/1074`. This checkpoint later moved to `-0.0249%` over target / `100.0985%` target speed after the VISITOR3/BUILDING4 restore-minus-current pack passes, WALKSTUF1 high setup-prime cap retune, and VISITOR3 high-only offscreen draw clip. |
| VISITOR3 high read-group table prune | Done; keep as code headroom. The remaining high-tide local table (`138..162`, `170..186`, `230..242`) is exhausted after compact-u16 inline and no longer changes runtime metrics. Removing it keeps VISITOR3 high/low, BUILDING2 high/low, ACTIVITY9 low, and FISHING1 high exact-flat while shrinking `foregroundPilotPlay` by `48` bytes. This checkpoint later moved to `-0.0249%` over target / `100.0985%` target speed after the VISITOR3/BUILDING4 restore-minus-current pack passes, WALKSTUF1 high setup-prime cap retune, and VISITOR3 high-only offscreen draw clip. |
| VISITOR3 restore-minus-current cleanup | Done; keep as pack-side data-shape win. FGP3/v4 cleanup spans now omit intervals redrawn by the same current frame while leaving draw pixels, padded pack size, pack LBAs, and PS-EXE size fixed. VISITOR3 high/low improve to `1139/1024` and `1140/1024`, blocking drops to `191/194`, prefetch overrun drops to `0/0`, active payload drops `981514 -> 737600`, and runtime restore bytes drop `973290 -> 498676`. This checkpoint later moved to `-0.0249%` over target / `100.0985%` target speed after the BUILDING4 restore-minus-current pack pass, WALKSTUF1 high setup-prime cap retune, and VISITOR3 high-only offscreen draw clip. |
| BUILDING4 restore-minus-current cleanup | Done; keep as pack-side data-shape win. FGP3/v4 cleanup spans now omit intervals redrawn by the same current frame while leaving draw pixels, padded pack size, pack LBAs, and PS-EXE size fixed. BUILDING4 high/low improve to `2844/2816` and `2855/2815`, blocking drops to `37/46`, active payload drops `1032442 -> 855284`, and runtime restore bytes drop `1229878 -> 546950`. This checkpoint later moved to `-0.0249%` over target / `100.0985%` target speed after VISITOR3 code-headroom, WALKSTUF1 high setup-prime cap, and VISITOR3 high-only offscreen draw-clip promotions. |
