...
second setup segment, the v227 low frame-125/frame-126 resident re-anchor, and
the v234 low frame-118 resident-copy follow-up:
`+0.3720%` public average over target / `99.6381%` public target speed across
all `126` timing-bearing rows. The raw signed optimization matrix is
`-0.3965%` / `100.4219%`. Since the compact full-matrix baseline was about
`17.4%` over target / `87.1%` target speed, the headless methodology has
removed about `17.03` public over-target points and added about `12.54`
...
v216 guarded low second setup segment for sectors `150..174`, and the v227
low frame-125/frame-126 resident re-anchor plus v234 frame-118 resident copy
inside that accepted segment.
...
resident re-anchor moves low again to `1095/1035`, and the v234 resident copy
moves low to `1091/1035`, cutting overrun `60 -> 56`, blocking `108 -> 103`,
loop reads `19 -> 18`, loop-read time `115 -> 110`, and due misses `18 -> 17`,
with the accepted cumulative setup/data tradeoff now `scene_vb 1408 -> 1417`.
...
improves `122 -> 118`. The later v234 pass owns the current low profile:
`scene_vb=1417`, `loop_vb=1091`, `target_vb=1035`, `overrun_vb=56`,
`blocking_vb=103`, `loop_read_vb=110`, and due misses `17`. Hidden refill stays
...
`scratch/ps1-perf-iterate/visitor3-setup-prime-320-v214-high/20260509-095807-2551471/summary.json`.
...
speed while cutting VISITOR3 low to `1091/1035` and blocking `103`.
...
| 5 | Next VISITOR3 pass: build scheduler-owned/generated append timing for additional balanced 16/24-sector candidates without more hot hand tables, or move to generated motion/residual/precomposed pack data. | A read-group win must lower loop or blocking without increasing visible refill; scheduler counters explain why it fired. The current VISITOR3 baseline is high `1089/1035`, blocking `83`, loop-read time `83`, and low `1091/1035`, blocking `103`, loop-read time `110`; v204/v205/v213/v214/v216 prove bounded setup residency can promote or cut same-loop CD pressure when hidden refill stays at zero, v206/v207/v227 prove generated re-anchors can still remove CD pressure after setup when the payload is placed inside accepted residency, and v234 proves unchanged payload relocation can win when semantic no-op variants fail. Setup debt must be justified by active-loop payoff. The wider v205 high probes `261..285` and `269..285`, low setup-prime caps above `208 KiB`, v215 unguarded low second segment, and low no-op/empty frame-118 variants either added hidden refill or regressed visible timing, so remaining VISITOR3 local read-table/setup-cap probes should be treated as exhausted unless generated metadata can add them size-neutrally and deadline-aware. |
