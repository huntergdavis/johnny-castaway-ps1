...
or long-run heap stability. The current all-scene battle card is `0.8228%`
over target / `99.4872%` target speed across `120` timing-bearing rows after
the `mary2-prefetch-relief-v081` refresh. The largest remaining absolute gaps are
now VISITOR3, WALKSTUF1, BUILDING2, BUILDING4, BUILDING6, JOHNNY2, and
