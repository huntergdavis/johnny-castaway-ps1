...
| BUILDING2 high read group `210..226` | Do not promote or retry as a hand-authored table. The current v896 retest in v898 saved three loop reads (`58 -> 55`) and read time (`257 -> 250`), but regressed scene/loop `1602/1351 -> 1609/1358`, overrun `40 -> 47`, blocking `54 -> 62`, and hidden refill `18 -> 26`; pack LBA and the PS-EXE bucket stayed fixed, but the extra table row shifted tracked hot symbols by `+4`. The middle cluster needs generated deadline ownership or pack-side data/work reduction, not another local read-table row. |
