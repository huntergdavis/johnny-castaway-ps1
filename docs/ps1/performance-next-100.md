...
v291 VISITOR3 high frame-140/tail setup-segment copy, and the v292 VISITOR3
low no-op payload alias:
`+0.3253%` public average over target / `99.6811%` public target speed across
all `126` timing-bearing rows. The raw signed optimization matrix is
`-0.4432%` / `100.4649%`. Since the compact full-matrix baseline was about
...
control was `1079/1039`, and v292 aliases frames `114..117` to frame `113`'s
resident no-op payload to make the current low profile `1075/1039`, overrun
`36`, blocking `67`, loop reads `12`, loop-read time `67`, and due misses `12`,
with current scene total at `scene_vb=1402`.
...
`blocking_vb=58`, `loop_read_vb=58`, and due misses `10`. The latest low alias
pass owns the current low profile:
`scene_vb=1402`, `loop_vb=1075`, `target_vb=1039`, `overrun_vb=36`,
`blocking_vb=67`, `loop_read_vb=67`, and due misses `12`. Hidden refill stays
`prefetch_overrun_vb=0` on both tides. The latest broad no-regression gate is
`scratch/ps1-perf-iterate/visitor3-low-alias-noop114117-v292-broad-norequire/20260510-023830-4043787/summary.json`;
...
`scratch/ps1-perf-iterate/visitor3-low-alias-noop114117-v292-focused/20260510-023711-4035807/summary.json`.
...
current rollup to `-0.4432%` / `100.4649%` while cutting VISITOR3 low to
