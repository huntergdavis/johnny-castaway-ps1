...
Current all-scene rollup after the missing-scene timing refresh:
`+0.6248%` public average over target / `99.4019%` public target speed across
all `126` timing-bearing rows. The raw signed optimization matrix is
`-0.1436%` / `100.1857%`. Since the compact full-matrix baseline was about
`17.4%` over target / `87.1%` target speed, the headless methodology has
removed about `16.78` public over-target points and added about `12.30`
...
FGP3/v4 pass. The missing-scene timing refresh then moves the raw signed matrix
to `-0.1436%` / `100.1857%`. The current rollup is tracked above.
...
missing-scene timing refresh moves the current raw signed matrix to
