...
target-hull timing promotion, the v202 high frame-127 re-anchor CD-pressure
promotion, the v204 low persistent setup segment, and the v205 high persistent
setup segment:
...
persistent setup segment for sectors `277..293`. The v205 pass keeps VISITOR3
high loop timing flat at `1101/1030` while cutting blocking `114 -> 108`, loop
reads `22 -> 20`, loop-read time `116 -> 108`, and due misses `21 -> 20`; the
setup tradeoff raises total high `scene_vb 1405 -> 1414`.
...
| 5 | Next VISITOR3 pass: build scheduler-owned/generated append timing for additional balanced 16/24-sector candidates without more hot hand tables, or move to generated motion/residual/precomposed pack data. | A read-group win must lower loop or blocking without increasing visible refill; scheduler counters explain why it fired. The current VISITOR3 baseline is high `1101/1030`, blocking `108`, loop-read time `108`, and low `1102/1032`, blocking `124`, loop-read time `131`; v204/v205 prove bounded setup segments can promote or cut same-loop CD pressure, but setup debt must be justified by active-loop payoff. The wider v205 high probes `261..285` and `269..285` cut more active CD work but regressed hidden refill `0 -> 3`, so remaining VISITOR3 local read-table probes should be treated as exhausted unless generated metadata can add them size-neutrally. |
