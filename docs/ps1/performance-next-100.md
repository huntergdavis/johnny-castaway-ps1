...
Latest promoted BUILDING4 baseline: keep the cleanup-metadata-only FGP3/v3
...
profiles. The current all-scene battle card is now `0.5706%` over target /
`99.6769%` target speed across `120` timing-bearing rows.
...
This checkpoint later moved to `0.5706%` over target / `99.6769%` target
speed after the ACTIVITY9 low cleanup-metadata compaction and JOHNNY2
clean-pressure relief; the current
...
`59 -> 55`, and loop reads `134 -> 132`.
Canaries stayed exact-flat for FISHING1, VISITOR3 high/low, WALKSTUF1 high,
...
| 5 | Next VISITOR3 pass: build scheduler-owned/generated append timing for additional balanced 16/24-sector candidates without more hot hand tables, or move to selective x-band/upload-ready preprocessing. | A read-group win must lower loop or blocking without increasing visible refill; scheduler counters explain why it fired. The current cleanup-compact plus guarded-group canary is `loop_vb=1406/1405`, `blocking_vb=293/301`, and `loop_read_vb=332/341`; high `111..127` is now closed, so remaining VISITOR3 local read-table probes should be treated as exhausted unless generated metadata can add them size-neutrally. |
