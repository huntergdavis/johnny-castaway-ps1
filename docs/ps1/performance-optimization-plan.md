...
| 1 | `visitor3` low/high, `walkstuf1` low/high, `building2` high/low, and `activity9` low/high | VISITOR3 remains the largest absolute gap after the prefetch recovery (`+444/+445` VBlanks) and is still the top preprocess opportunity. WALKSTUF1 remains CD-heavy (`+225/+227` VBlanks, `blocking_vb=304/297`). BUILDING2 is refreshed current-layout CD pressure (`+191/+187` VBlanks, `blocking_vb=286/279`) and one-off groups did not convert read savings into timing. ACTIVITY9 is improved by the tide-specific window retune, but still retains `+143/+136` VBlank gaps with visible blocking `146/117`. ACTIVITY1, ACTIVITY4, ACTIVITY11, ACTIVITY12, BUILDING3, FISHING2, FISHING4, MARY1, and MARY5 drop out of the top queue after relief/current refresh. | Move VISITOR3 to generated selective upload-ready or scheduler-owned CD/render work. Test BUILDING2 and ACTIVITY9 with generated metadata, not hand-authored groups. Keep WALKSTUF1/WALKSTUF3 as canaries and avoid more raw groups unless generated cost metadata predicts a visible loop/blocking win. |
