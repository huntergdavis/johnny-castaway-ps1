...
120/126 scene/tide variants carry active-loop timing and average `+3.1%` over target /
`97.4%` target speed as of `stale-zero2-v072b-current-refresh` (`3.1299%` exact over target /
`97.3807%` exact target speed). The remaining optimization target is therefore
...
Checkpoint after `stale-zero2-v072b-current-refresh`: the matrix is now
`120` timing-bearing rows at `3.1299%` exact average over target / `97.3807%`
exact target speed. The VISITOR3/WALKSTUF1/VISITOR5 recoveries prove validation-pack
...
JOHNNY3, STAND4, STAND16, ACTIVITY10, JOHNNY4, FISHING8, STAND6, STAND5, and STAND12 row
...
| 1 | `visitor3` low/high, `building2` high/low, and `walkstuf1` low/high | VISITOR3 remains the largest absolute gap after the prefetch recovery (`+444/+445` VBlanks) and is now the top preprocess opportunity. WALKSTUF1 is a still-CD-heavy mid-tier pair (`+16.0%/+16.1%`, `blocking_vb=304/297`). BUILDING2 remains visibly CD-bound and one-off groups did not convert read savings into timing. | Move VISITOR3 to generated selective upload-ready or scheduler-owned CD/render work. Test BUILDING2 with generated metadata, not hand-authored groups. Keep WALKSTUF1 as a canary and avoid more raw groups unless generated cost metadata predicts a visible loop/blocking win. |
...
| 3 | Remaining true CD-pressure rows | The latest zero-pressure refresh removed most false fixed-overhead targets. JOHNNY3 and ACTIVITY10 are now confirmed current CD-pressure rows with hundreds of blocking VBlanks/due misses, while stand/fishing/johnny fixed-overhead rows largely collapsed to target or under target. | Treat JOHNNY3 and ACTIVITY10 like VISITOR3/BUILDING2/WALKSTUF1: generated setup/segment coverage, memory-relief prefetch exceptions if clean pressure is disabling buffers, selective upload-ready preprocessing, and scheduler-owned CD budgets. Do not spend time on generic zero-CD overhead until a fresh current row proves it still exists. |
