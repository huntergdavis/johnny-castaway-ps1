...
| 1 | `walkstuf1` low/high and `visitor3` low/high | Current top outliers are WALKSTUF1 low/high at `1604/1407` and `1592/1406`, then VISITOR3 low/high at `1140/1024` and `1139/1024`. WALKSTUF1 scalar cap and direct-stage lanes are closed. VISITOR3 local C read-table rows are closed, and the default selective upload-ready plan is now footprint-closed as a same-layout append: it selects `96 / 144` frames and models `6114568` saved upload bytes, but needs `2462072` payload+rect bytes against only `814847` bytes of pack slack per tide. The budgeted same-footprint target selects `74` frames, uses `814184` bytes, and retains `3858104` modeled saved upload bytes. | Move WALKSTUF1 and VISITOR3 to scheduler-owned generated metadata, the budgeted VISITOR3 upload-ready subset, compressed selective upload payloads, or shrinking pack transforms. Avoid more raw hand-authored read groups, scalar setup caps, or same-footprint VISITOR3 default append attempts. |
