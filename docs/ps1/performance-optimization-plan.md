...
| `HP-062` | In progress: test scoped graphics helper `-O2`: confirm `graphics_ps1.c` is default `-O2`, then remove or override current helper-level `-Os` one helper at a time. | `grDrawBackground()` at default `-O2` was rejected because timing stayed exact-flat while the helper grew by `504` bytes and ELF grew `568` bytes. Continue with `grUpdateDisplay()` as the next isolated helper probe. |
...
|---|---|---|---|---|
| `-O2` compiler sweep | 0 | `HP-061`, `HP-062`, `HP-063`, `HP-064` | HP-061 is complete and HP-062 is underway. `grDrawBackground()` scoped `-O2` was rejected; continue with `grUpdateDisplay()`, then the hot/semi-hot `-Os` TUs. Each follow-up row is one isolated flag/codegen experiment with map/layout evidence and the normal visual/perf gate. | Finds free compiler wins first, or produces searchable no-promotion evidence before we spend time on riskier rewrites. |
