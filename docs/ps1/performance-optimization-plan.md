...
120/126 scene/tide variants carry active-loop timing and average `+6.0%` over target /
`94.8%` target speed as of `stale-top-v072b-current-refresh` (`5.9984%` exact over target /
`94.8106%` exact target speed). The remaining optimization target is therefore
...
Checkpoint after `stale-top-v072b-current-refresh`: the matrix is now
`120` timing-bearing rows at `5.9984%` exact average over target / `94.8106%`
...
and FISHING6 row refreshes also prove stale matrix rows must be replaced before
treating a scene as a fixed-overhead target. The next loop should prioritize
generated pack metadata, selective preprocessing, and scheduler-owned CD/render
budgets before more raw read-count groups.
...
| 3 | Zero-CD fixed overhead rows | WALKSTUF2 and STAND3 also dropped under target after current-pack refreshes, leaving the remaining zero-pressure percent gaps mostly on older rows such as JOHNNY5, FISHING4, ACTIVITY6, MISCGAG1, and STAND15. These may still be stale or real render/present/upload overhead, but they are no longer the top absolute gap. | Refresh any stale validated-pack rows before optimizing. If a current zero-pressure row remains high, treat it as a render/present/upload/loop overhead problem: pack-time frame coalescing, upload-ready/direct16 bands, resident-pack fast paths, or a real present/input scheduler. CD policies cannot explain these rows. |
