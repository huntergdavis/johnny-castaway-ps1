...
| 1 | `visitor3` low/high, `walkstuf1` low/high, `building4` high/low, and `building2` high/low | VISITOR3 remains the largest absolute gap after the setup-prime cap, low-tide `170..186` group, and cleanup compaction (`+390/+387` VBlanks) and is still the top preprocess opportunity. The FGP3 detail analyzer shows VISITOR3's blanket x-band problem is localized: frames `134..136` hit the rect cap and save `0%`, while nearby frames `121..133` have large non-cap byte savings. The first tracked CSV threshold plan selects `96 / 144` frames and estimates `6114568` upload bytes saved in that subset. WALKSTUF1 setup-prime helped but remains CD-heavy (`+217/+192` VBlanks, `blocking_vb=276/278`). BUILDING4 is now the largest non-VISITOR building gap (`+211/+197`), while BUILDING2 cleanup compaction drops to `+141/+143` with blocking `212/193` but still has visible CD pressure. | Move VISITOR3, BUILDING4, and remaining BUILDING2 pressure to generated selective upload-ready or scheduler-owned CD/render work. For VISITOR3, start with the CSV-selected non-cap frames and leave cap-hit crash frames on the current path. Keep WALKSTUF1 as a canary and avoid more raw groups unless generated cost metadata predicts a visible loop/blocking win and runtime ownership is proven. |
| 2 | BUILDING-family CD pressure | Clean-snapshot relief cut the fresh BUILDING4/6 failure baselines sharply, and BUILDING2 cleanup compaction removes another `36..38` loop VBlanks per tide without code or LBA movement. BUILDING4 high/low still retain `+211/+197` VBlank gaps, BUILDING6 high/low retain `+78/+78`, and BUILDING2 still has post-compaction visible CD pressure. Raw BUILDING4/6 setup-prime and family-wide window sweeps regressed. | Continue per-tide generated setup/read policy, but treat every BUILDING win as layout- and tide-sensitive. Move to generated read groups or host-side preprocessing that changes read placement/format; do not assume high/low pairs share a safe setup-prime or window knee. |
| 3 | Remaining true CD-pressure rows | The latest prefetch-relief pass removed ACTIVITY4 and FISHING4 from the due-miss-collapse queue. The largest current pressure set is VISITOR3, WALKSTUF1, BUILDING2, ACTIVITY9, BUILDING4, BUILDING6, and JOHNNY6. WALKSTUF3 refreshed to near-target and should mainly serve as a stability canary unless a generated opportunity appears. | Continue with generated setup/segment coverage, selective upload-ready preprocessing, and scheduler-owned CD/render budgets for the remaining top rows. Do not spend time on generic zero-CD overhead until a fresh current row proves it still exists. |
