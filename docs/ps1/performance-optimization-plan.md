...
120/126 scene/tide variants carry active-loop timing and average `+7.8%` over target /
`93.3%` target speed as of `mismatch-top-v072-current-refresh` (`7.8267%` exact over target /
`93.3111%` exact target speed). The remaining optimization target is therefore
...
Checkpoint after `mismatch-top-v072-current-refresh`: the matrix is now
`120` timing-bearing rows at `7.8267%` exact average over target / `93.3111%`
exact target speed. The VISITOR3/WALKSTUF1 recoveries prove validation-pack
