...
VISITOR3 is no longer blocked by obvious scalar knobs. The accepted timing baseline remains v193: high `1101/1030`, overrun `71`, blocking `116`, active payload `542729`; low remains the v189 pack at `1108/1028`, overrun `80`, blocking `139`, active payload `595110`. The current source baseline is v197, which removes unused noncompact motion dispatch and buys code-layout headroom while keeping all VISITOR3 and broad canary metrics exact-flat. Frame `117` proves cleanup geometry matters as much as byte count, v187/v192 prove generic runtime narrow upload is too large, and v196 proves a naive compact motion-marker dispatch is not code-size/cadence safe. The next ideas should either move ownership to generated pack data or change scheduling without expanding `grDrawBackground`.

| Priority | Experiment | Why It Could Move VISITOR3 | First Gate |
|---:|---|---|---|
| 1 | Precomposed direct upload strip for frame `133`, public high/low matrix state only. | Frame `133` has the biggest x-band signal (`341760 -> 85856` modeled upload bytes). Baking restored-background plus foreground pixels for the exact public island state avoids strided scratch-copy upload. | Build a side pack or opcode for frame `133`; require fixed LBA/PS-EXE bucket and no visual residue. |
