...
### VISITOR3 White-Whale Candidate Deck After v214

VISITOR3 is no longer blocked by obvious scalar knobs. The accepted timing baseline is now v214 high plus v204 low: high `1089/1035`, overrun `54`, blocking `83`, loop-read time `83`, active payload `496661`; low `1102/1032`, overrun `70`, blocking `124`, loop-read time `131`, active payload `595110`, with accepted setup tradeoffs from high `scene_vb 1405 -> 1420` and low `1408 -> 1415`. The v197 source-size pass removed unused noncompact motion dispatch and bought code-layout headroom; v202/v206/v207 prove split high-only re-anchoring can still remove CD pressure after low rejects the same family; v204/v205/v213/v214 prove carefully bounded reusable setup residency can move or de-risk active-loop CD pressure when the read-plan cluster is large enough. Frame `117` proves cleanup geometry matters as much as byte count, v187/v192/v198 prove runtime scratch-copy narrow upload loses even when bytes drop, v199/v200 prove runtime pre-staging breaks perf completion whether the buffer is static or heap-backed, v196 proves a naive compact motion-marker dispatch is not code-size/cadence safe, v201/v203 prove wider-anchor late-yacht byte cuts can regress low-tide cadence, v205 proves wider high setup segments can regress hidden refill even when active CD counters improve, v208/v209 prove frames `116` and `114` are still cadence-negative under no-cleanup opcode-1 hull even after removing cleanup rows, v210-v212 prove low setup-prime caps above `208 KiB` add hidden refill or visible cadence debt, and v214 suggests high setup residency still has a little room but should now be tested in smaller, generated/deadline-aware increments. The next ideas should either make upload pixels contiguous in generated pack/sidecar data before runtime, move ownership to generated pack metadata, change scheduling without expanding `grDrawBackground`, or continue high-only custom compression/precomposed candidates with low isolated.
...
| 16 | Deadline-owned read schedule for VISITOR3 low tail reads. | Low still has `27` loop reads and `139` blocking VBlanks; generated deadline metadata may shift reads without pack changes. | No code change until read-plan predicts a blocking drop. |
...
| 18 | Setup-prime microsegment instead of scalar prime size. | Scalar `200/216/256 KiB` primes are closed, but tiny side segments can cover late active reads without moving the whole window. | One high segment with fixed bucket; require `blocking_vb` drop. |
