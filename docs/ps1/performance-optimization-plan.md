...
VISITOR3 is no longer blocked by obvious scalar knobs. The accepted timing baseline remains v193: high `1101/1030`, overrun `71`, blocking `116`, active payload `542729`; low remains the v189 pack at `1108/1028`, overrun `80`, blocking `139`, active payload `595110`. The current source baseline is v197, which removes unused noncompact motion dispatch and buys code-layout headroom while keeping all VISITOR3 and broad canary metrics exact-flat. Frame `117` proves cleanup geometry matters as much as byte count, v187/v192/v198 prove runtime scratch-copy narrow upload loses even when bytes drop, v199/v200 prove runtime pre-staging breaks perf completion whether the buffer is static or heap-backed, v196 proves a naive compact motion-marker dispatch is not code-size/cadence safe, and v201 proves wider-anchor late-yacht byte cuts can still regress low-tide cadence. The next ideas should either make upload pixels contiguous in generated pack/sidecar data before runtime, move ownership to generated pack metadata, or change scheduling without expanding `grDrawBackground`.
