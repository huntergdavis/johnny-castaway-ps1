...
120/126 scene/tide variants carry active-loop timing and average `+7.6%` over target /
`93.5%` target speed as of `visitor5-v072-prefetch-relief` (`7.5934%` exact over target /
`93.5083%` exact target speed). The remaining optimization target is therefore
...
Checkpoint after `visitor5-v072-prefetch-relief`: the matrix is now
`120` timing-bearing rows at `7.5934%` exact average over target / `93.5083%`
...
STAND, ACTIVITY7, VISITOR1, FISHING7, and ACTIVITY8 row refreshes also prove
stale matrix rows must be replaced before treating a scene as a fixed-overhead
target. The next loop should prioritize generated pack metadata, selective
preprocessing, and scheduler-owned CD/render budgets before more raw read-count
groups.
...
| 3 | Zero-CD fixed overhead rows | `walkstuf2`, `stand3`, and several smaller older rows remain high percent-over-target with `blocking_vb=0`, `prefetch_overrun_vb=0`, and `due_misses=0`; STAND1/2/7/9/10/11, VISITOR4, ACTIVITY7, VISITOR1, and FISHING7 dropped under or near target after current-pack refreshes. | Refresh stale validated-pack rows first, then treat remaining zero-pressure rows as render/present/upload/loop overhead problems: pack-time frame coalescing, upload-ready/direct16 bands, resident-pack fast paths, or a real present/input scheduler. CD policies cannot explain these rows. |
