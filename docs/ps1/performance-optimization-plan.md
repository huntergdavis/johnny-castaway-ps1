...
### VISITOR3 White-Whale Candidate Deck After v209

VISITOR3 is no longer blocked by obvious scalar knobs. The accepted timing baseline is now v207 high plus v204 low: high `1101/1030`, overrun `71`, blocking `106`, active payload `496661`; low `1102/1032`, overrun `70`, blocking `124`, active payload `595110`, with accepted setup tradeoffs from high `scene_vb 1405 -> 1414` and low `1408 -> 1415`. The v197 source-size pass removed unused noncompact motion dispatch and bought code-layout headroom; v202/v206/v207 prove split high-only re-anchoring can still remove CD pressure after low rejects the same family; v204/v205 prove carefully bounded reusable setup segments can move or de-risk active-loop CD pressure when the read-plan cluster is large enough. Frame `117` proves cleanup geometry matters as much as byte count, v187/v192/v198 prove runtime scratch-copy narrow upload loses even when bytes drop, v199/v200 prove runtime pre-staging breaks perf completion whether the buffer is static or heap-backed, v196 proves a naive compact motion-marker dispatch is not code-size/cadence safe, v201/v203 prove wider-anchor late-yacht byte cuts can regress low-tide cadence, v205 proves wider setup coverage can regress hidden refill even when active CD counters improve, and v208/v209 prove frames `116` and `114` are still cadence-negative under no-cleanup opcode-1 hull even after removing cleanup rows. The next ideas should either make upload pixels contiguous in generated pack/sidecar data before runtime, move ownership to generated pack metadata, change scheduling without expanding `grDrawBackground`, or continue high-only re-anchor/custom compression candidates with low isolated.
