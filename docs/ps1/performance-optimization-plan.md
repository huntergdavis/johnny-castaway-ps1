...
VISITOR3 is no longer blocked by obvious scalar knobs. The accepted timing baseline remains v193/v202: high `1101/1030`, overrun `71`, blocking `114`, active payload `527206`; low remains the v189/v197 pack at `1108/1028`, overrun `80`, blocking `139`, active payload `595110`. The current source baseline is v197, which removes unused noncompact motion dispatch and buys code-layout headroom while keeping all VISITOR3 and broad canary metrics exact-flat; the current high-pack baseline is v202, which proves split high-only re-anchoring can still remove CD pressure after low rejects the same family. Frame `117` proves cleanup geometry matters as much as byte count, v187/v192/v198 prove runtime scratch-copy narrow upload loses even when bytes drop, v199/v200 prove runtime pre-staging breaks perf completion whether the buffer is static or heap-backed, v196 proves a naive compact motion-marker dispatch is not code-size/cadence safe, v201/v203 prove wider-anchor late-yacht byte cuts can regress low-tide cadence, and v202 proves the same re-anchor lane can promote when split high-only. The next ideas should either make upload pixels contiguous in generated pack/sidecar data before runtime, move ownership to generated pack metadata, change scheduling without expanding `grDrawBackground`, or continue high-only re-anchor/custom compression candidates with low isolated.
...
| 1 | `visitor3` low/high and `walkstuf1` low/high | Current top outliers are VISITOR3 low/high at `1108/1028` and `1104/1030`, then WALKSTUF1 low/high on their accepted compact baseline. WALKSTUF1 scalar setup caps and scalar direct-stage caps are closed through the `6/7/8 KiB` knee; remaining WALKSTUF1 wins need scheduler-owned or data-shape changes. VISITOR3 local C read-table rows are closed, the default selective upload-ready plan is footprint-closed as a same-layout append, the raw foreground-only upload payload is unsafe because selected x-bands are not draw-covered, the no-op empty-hold recast found `0` eligible zero-visual-work payload entries, the scoped composite-helper code-shape win is promoted, the v4 draw-tail trim plus stage guard is superseded, and the v181/v182 motion-copy payloads are now the baseline. The refreshed preprocessing matrix still ranks VISITOR3 first, but the next pass should start from generated motion eligibility, residual dictionaries, and safe pixel ownership rather than stale upload-ready assumptions. | Keep VISITOR3 as the white-whale target. Expand motion-copy with generated eligibility and visual-state fuzzing, try scene-local residual compression/precomposed strips, or add scheduler-owned deadline metadata for the remaining residual reads. Avoid more raw hand-authored read groups, scalar setup caps, scalar direct-stage caps, no-op entry pruning, or same-footprint raw VISITOR3 upload-ready appends. |
