...
120/126 scene/tide variants carry active-loop timing and average `+13.6%` over target /
`88.8%` target speed as of `walkstuf1-v072-prefetch-relief` (`13.6273%` exact over target /
`88.7828%` exact target speed). The remaining optimization target is therefore
...
Checkpoint after `walkstuf1-v072-prefetch-relief`: the matrix is now
`120` timing-bearing rows at `13.6273%` exact average over target / `88.7828%`
exact target speed. The WALKSTUF1 recovery proves validation-pack refreshes can
change memory pressure enough to reopen old prefetch paths; broad timing fixes
and data-shape changes still beat one-off read tuning. The next loop should
prioritize validated-pack memory policy, generated pack metadata, selective
preprocessing, and scheduler-owned CD/render budgets before more raw
read-count groups.
...
| 1 | `visitor3` low/high, `building2` high/low, and `walkstuf1` low/high | WALKSTUF1 moved from the worst row pair to a still-CD-heavy mid-tier pair (`+16.0%/+16.1%`, `blocking_vb=304/297`). VISITOR3 is now the largest absolute gap (`+408/+378` VBlanks) and current canaries show high tide can fall back to `policy=none` after validation-pack changes. BUILDING2 remains visibly CD-bound and one-off groups did not convert read savings into timing. | First retry the validated-pack memory-policy pattern on VISITOR3, then generated read metadata / selective upload-ready pack data for VISITOR3 and BUILDING2. Keep WALKSTUF1 as a canary and avoid more hand-coded groups unless generated cost metadata predicts a visible loop/blocking win. |
