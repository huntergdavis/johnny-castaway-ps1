...
`99.3%` target speed as of `building4-6-johnny6-v072c-prefetch-relief` (`1.0270%` exact over target /
`99.3047%` exact target speed). The remaining optimization target is therefore
...
Checkpoint after `building4-6-johnny6-v072c-prefetch-relief`: the matrix is now
`120` timing-bearing rows at `1.0270%` exact average over target / `99.3047%`
exact target speed. The VISITOR3/WALKSTUF1/VISITOR5/ACTIVITY10/JOHNNY3/JOHNNY1/ACTIVITY9/MARY1/ACTIVITY11/ACTIVITY12/BUILDING4/BUILDING6/JOHNNY6 recoveries prove validation-pack
...
| 3 | Remaining true CD-pressure rows | The latest prefetch-relief pass removed BUILDING4/6/JOHNNY6 from the due-miss-collapse queue but did not make them zero-pressure rows. The largest current pressure set is VISITOR3, WALKSTUF1, BUILDING2, ACTIVITY9, BUILDING4, FISHING4 low, BUILDING6, ACTIVITY4 low, and JOHNNY6. WALKSTUF3 refreshed to near-target and should mainly serve as a stability canary unless a generated opportunity appears. | Continue with generated setup/segment coverage, selective upload-ready preprocessing, and scheduler-owned CD/render budgets for the remaining top rows. Do not spend time on generic zero-CD overhead until a fresh current row proves it still exists. |
