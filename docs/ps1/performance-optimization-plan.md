...
120/126 scene/tide variants carry active-loop timing and average `+4.6%` over target /
`96.1%` target speed as of `stale-zero-v072b-current-refresh` (`4.5708%` exact over target /
`96.0653%` exact target speed). The remaining optimization target is therefore
...
Checkpoint after `stale-zero-v072b-current-refresh`: the matrix is now
`120` timing-bearing rows at `4.5708%` exact average over target / `96.0653%`
...
FISHING6, JOHNNY5, FISHING4, ACTIVITY6, MISCGAG1, STAND15, and ACTIVITY4 row
...
| 3 | Zero-CD fixed overhead rows | WALKSTUF2, STAND3, JOHNNY5, ACTIVITY6, MISCGAG1, and STAND15 all dropped to target or under target after current-pack refreshes. The remaining zero-pressure percent gaps are lower-priority rows such as JOHNNY3, STAND4, STAND16, JOHNNY4, and STAND6. | Refresh any stale validated-pack rows before optimizing. If a current zero-pressure row remains high, treat it as a render/present/upload/loop overhead problem: pack-time frame coalescing, upload-ready/direct16 bands, resident-pack fast paths, or a real present/input scheduler. CD policies cannot explain these rows. |
