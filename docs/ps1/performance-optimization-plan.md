...
| `P4-194` | Queued: add a map/EXE-bucket gate to `ps1-perf-iterate`. | Several semantically valid cleanups fail only after crossing `149504 -> 147456`; promote no code-size cleanup that changes the PS-EXE bucket unless the run also improves cadence. |
| `P4-195` | Queued: add a foreground-pack LBA comparison gate. | The harness should flag `FG\\FISHING1.FG2 LBA 399 -> 398` as a first-class warning/failure so CD-phase misses are obvious before deeper metric triage. |
| `P4-196` | Queued: parse build map deltas for hot functions. | Before retrying helper/cache refactors, record whether `foreground_pilot.c` function addresses or section order moved; code-layout phase is now a measured risk. |
...
| `P4-200` | Queued: host-side group planner for fishing1 read sequence. | Use the accepted summary plus entry table to propose append groups that reduce `68` loop reads and `5` backward seeks without increasing sectors. |
...
| `P4-205` | Queued: host-side DuckStation log read-sequence extractor. | Existing emulator logs include CD seeks; post-process logs outside the PS1 binary to infer read cadence without hot-path instrumentation. |
