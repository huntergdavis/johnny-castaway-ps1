...
|---|---|---|---|---|
| `-O2` compiler sweep | 0 | `HP-061`, `HP-062`, `HP-063`, `HP-064` | HP-061 is complete; continue with HP-062 next. Each follow-up row is one isolated flag/codegen experiment with map/layout evidence and the normal visual/perf gate. | Finds free compiler wins first, or produces searchable no-promotion evidence before we spend time on riskier rewrites. |
