...
| 1 | `visitor3` low/high and `walkstuf1` low/high | Current top outliers are VISITOR3 low/high at `1108/1028` and `1105/1031`, then WALKSTUF1 low/high on their accepted compact baseline. WALKSTUF1 scalar setup caps and scalar direct-stage caps are closed through the `6/7/8 KiB` knee; remaining WALKSTUF1 wins need scheduler-owned or data-shape changes. VISITOR3 local C read-table rows are closed, the default selective upload-ready plan is footprint-closed as a same-layout append, the raw foreground-only upload payload is unsafe because selected x-bands are not draw-covered, the no-op empty-hold recast found `0` eligible zero-visual-work payload entries, the scoped composite-helper code-shape win is promoted, the v4 draw-tail trim plus stage guard is superseded, and the v181 motion-copy payload is now the baseline. The refreshed preprocessing matrix still ranks VISITOR3 first, but the next pass should start from motion eligibility, residual dictionaries, and generated safe pixel ownership rather than stale upload-ready assumptions. | Keep VISITOR3 as the white-whale target. Expand motion-copy with generated eligibility and visual-state fuzzing, try scene-local residual compression/precomposed strips, or add scheduler-owned deadline metadata for the remaining residual reads. Avoid more raw hand-authored read groups, scalar setup caps, scalar direct-stage caps, no-op entry pruning, or same-footprint raw VISITOR3 upload-ready appends. |
