...
`99.5%` target speed as of `activity9-lowgroup-v072c` (`0.8692%` exact over target /
`99.4529%` exact target speed). The remaining optimization target is therefore
...
Checkpoint after `activity9-lowgroup-v072c`: the matrix is now
`120` timing-bearing rows at `0.8692%` exact average over target / `99.4529%`
...
| 1 | `visitor3` low/high, `walkstuf1` low/high, `building2` high/low, and `activity9` low/high | VISITOR3 remains the largest absolute gap after the prefetch recovery (`+444/+445` VBlanks) and is still the top preprocess opportunity. The FGP3 detail analyzer shows VISITOR3's blanket x-band problem is localized: frames `134..136` hit the rect cap and save `0%`, while nearby frames `121..133` have large non-cap byte savings. The first tracked CSV threshold plan selects `96 / 144` frames and estimates `6114568` upload bytes saved in that subset. WALKSTUF1 remains CD-heavy (`+225/+227` VBlanks, `blocking_vb=304/297`). BUILDING2 is refreshed current-layout CD pressure (`+191/+187` VBlanks, `blocking_vb=286/279`), and the latest visible-cost high-side groups still failed to convert local read/refill movement into active-loop timing. ACTIVITY9 is improved by the tide-specific window retune, but still retains `+143/+136` VBlank gaps with visible blocking `146/117`. | Move VISITOR3 and BUILDING2 to generated selective upload-ready or scheduler-owned CD/render work. For VISITOR3, start with the CSV-selected non-cap frames and leave cap-hit crash frames on the current path. Test ACTIVITY9 residuals with generated metadata, not hand-authored groups. Keep WALKSTUF1/WALKSTUF3 as canaries and avoid more raw groups unless generated cost metadata predicts a visible loop/blocking win and runtime ownership is proven. |
