...
| 15 | Negative test: frame `116` sparse-hull with upload-cost veto. | Adjacent precursor hull frames are suspect, but a vetoed dry-run can validate whether all remaining opcode `1` candidates are closed by upload cost. | Dry-run only unless predicted upload decreases. |
