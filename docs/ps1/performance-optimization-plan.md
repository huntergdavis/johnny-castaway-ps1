...
120/126 scene/tide variants carry active-loop timing and average `+9.1%` over target /
`92.2%` target speed as of `stand-family-v072-current-refresh` (`9.0942%` exact over target /
`92.2322%` exact target speed). The remaining optimization target is therefore
...
Checkpoint after `stand-family-v072-current-refresh`: the matrix is now
`120` timing-bearing rows at `9.0942%` exact average over target / `92.2322%`
...
VISITOR3 row refresh also proves stale matrix rows must be replaced even when
the aggregate gets less flattering. The next loop should prioritize generated
pack metadata, selective preprocessing, and scheduler-owned CD/render budgets
before more raw read-count groups.
