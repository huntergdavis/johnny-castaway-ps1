...
| `P4-230` | Failed/no promotion: adjacent fishing1 tail group `384..396`. | Adding `{384,396}` plus 12-sector retained capacity kept timing/read counts flat but did not fire (`group_hits=0`) and raised the retained buffer `29712 -> 31760`; keep the accepted 11-sector capacity until runtime append-start evidence or generated group metadata identifies a real next group. |
