...
VISITOR3 is no longer blocked by obvious scalar knobs. The accepted timing baseline is now v204: high `1101/1030`, overrun `71`, blocking `114`, active payload `527206`; low `1102/1032`, overrun `70`, blocking `124`, active payload `595110`, with an accepted setup tradeoff from `scene_vb 1408 -> 1415`. The v197 source-size pass removed unused noncompact motion dispatch and bought code-layout headroom; v202 proves split high-only re-anchoring can still remove CD pressure after low rejects the same family; v204 proves a carefully bounded reusable setup segment can move low active-loop timing when the read-plan cluster is large enough. Frame `117` proves cleanup geometry matters as much as byte count, v187/v192/v198 prove runtime scratch-copy narrow upload loses even when bytes drop, v199/v200 prove runtime pre-staging breaks perf completion whether the buffer is static or heap-backed, v196 proves a naive compact motion-marker dispatch is not code-size/cadence safe, and v201/v203 prove wider-anchor late-yacht byte cuts can regress low-tide cadence. The next ideas should either make upload pixels contiguous in generated pack/sidecar data before runtime, move ownership to generated pack metadata, change scheduling without expanding `grDrawBackground`, or continue high-only re-anchor/custom compression candidates with low isolated.
