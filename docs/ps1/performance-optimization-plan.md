...
| 16 | Deadline-owned read schedule for VISITOR3 low tail reads. | Low is down to `20` loop reads and `112` blocking VBlanks after v216, but the remaining `64` VBlank overrun is still a scheduler/data-ownership problem. Generated deadline metadata may shift the last reads without pack changes. | No code change until read-plan predicts a blocking drop with `prefetch_overrun_vb=0`. |
