...
| `P5-197` | Done: complete the compact-layout full matrix. | `docs/ps1/performance-scene-matrix.csv` now has `126 / 126` routed scene/tide rows under `compact-fgp3-v2-fullmatrix`; `120 / 126` carry active-loop timing. Timing-bearing average is `+17.4%` over target / `87.1%` target speed, latest row `2026-04-29T18:13:13`. `mary3`, `suzy1`, and `suzy2` high/low are metadata-only routes with no active-loop timing and are excluded from averages. |
