...
### VISITOR3 White-Whale Candidate Deck After v227

VISITOR3 is no longer blocked by obvious scalar knobs. The accepted timing baseline is now v214 high plus v227 low: high `1089/1035`, overrun `54`, blocking `83`, loop-read time `83`, active payload `496661`; low `1095/1035`, overrun `60`, blocking `108`, loop-read time `115`, active payload `564565`, with accepted setup tradeoffs from high `scene_vb 1405 -> 1420` and low `1408 -> 1421`. The v197 source-size pass removed unused noncompact motion dispatch and bought code-layout headroom; v202/v206/v207 prove split high-only re-anchoring can still remove CD pressure after low rejects the same family; v227 proves low can accept the same re-anchor family when the repacked payload lands inside already accepted residency. v204/v205/v213/v214/v216 prove carefully bounded reusable setup residency can move or de-risk active-loop CD pressure when the read-plan cluster is large enough and deadline ownership is explicit. Frame `117` proves cleanup geometry matters as much as byte count, v187/v192/v198 prove runtime scratch-copy narrow upload loses even when bytes drop, v199/v200 prove runtime pre-staging breaks perf completion whether the buffer is static or heap-backed, v196 proves a naive compact motion-marker dispatch is not code-size/cadence safe, v201/v203 prove wider-anchor late-yacht byte cuts can regress low-tide cadence, v205 proves wider high setup segments can regress hidden refill even when active CD counters improve, v208/v209 prove frames `116` and `114` are still cadence-negative under no-cleanup opcode-1 hull even after removing cleanup rows, v210-v212 prove low setup-prime caps above `208 KiB` add hidden refill or visible cadence debt, v215 proves an unguarded second low setup segment is unsafe, and v216 proves the same segment can promote once refill slack is guarded. The next ideas should either make upload pixels contiguous in generated pack/sidecar data before runtime, move ownership to generated pack metadata, change scheduling without expanding `grDrawBackground`, or continue custom compression/precomposed candidates with high and low isolated.
...
| 16 | Deadline-owned read schedule for VISITOR3 low tail reads. | Low is down to `19` loop reads and `108` blocking VBlanks after v227, but the remaining `60` VBlank overrun is still a scheduler/data-ownership problem. Generated deadline metadata may shift the last reads without pack changes. | No code change until read-plan predicts a blocking drop with `prefetch_overrun_vb=0`. |
...
| 1 | `visitor3` low/high and `walkstuf1` low/high | Current top outliers are VISITOR3 low/high at `1095/1035` and `1089/1035`, then WALKSTUF1 low/high on their accepted compact baseline. WALKSTUF1 scalar setup caps and scalar direct-stage caps are closed through the `6/7/8 KiB` knee; remaining WALKSTUF1 wins need scheduler-owned or data-shape changes. VISITOR3 local C read-table rows are closed, the default selective upload-ready plan is footprint-closed as a same-layout append, the raw foreground-only upload payload is unsafe because selected x-bands are not draw-covered, the no-op empty-hold recast found `0` eligible zero-visual-work payload entries, the scoped composite-helper code-shape win is promoted, the v4 draw-tail trim plus stage guard is superseded, the v181/v182/v188/v189/v193/v202/v206/v207/v227 motion-copy/re-anchor payloads are baseline, and the v204/v205/v213/v214/v216 setup-residency passes prove scheduler-owned setup debt can pay or de-risk CD pressure when it removes enough active-loop debt and keeps hidden refill at zero. The refreshed preprocessing matrix still ranks VISITOR3 first, but the next pass should start from generated motion eligibility, residual dictionaries, safe pixel ownership, and explicit setup-vs-loop accounting rather than stale upload-ready assumptions. | Keep VISITOR3 as the white-whale target. Expand motion-copy with generated eligibility and visual-state fuzzing, try scene-local residual compression/precomposed strips, or add scheduler-owned deadline metadata for the remaining residual reads. Avoid more raw hand-authored read groups, scalar low setup caps, scalar direct-stage caps, no-op entry pruning, or same-footprint raw VISITOR3 upload-ready appends. |
