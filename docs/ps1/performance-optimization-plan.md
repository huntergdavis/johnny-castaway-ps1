...
120/126 scene/tide variants carry active-loop timing and average `+1.1%` over target /
`99.2%` target speed as of `activity11-12-v072c-prefetch-relief` (`1.1131%` exact over target /
`99.2199%` exact target speed). The remaining optimization target is therefore
...
Checkpoint after `activity11-12-v072c-prefetch-relief`: the matrix is now
`120` timing-bearing rows at `1.1131%` exact average over target / `99.2199%`
exact target speed. The VISITOR3/WALKSTUF1/VISITOR5/ACTIVITY10/JOHNNY3/JOHNNY1/ACTIVITY9/MARY1/ACTIVITY11/ACTIVITY12 recoveries prove validation-pack
...
JOHNNY3, STAND4, STAND16, ACTIVITY10, JOHNNY4, FISHING8, STAND6, STAND5, STAND12, BUILDING2, WALKSTUF3, ACTIVITY5, BUILDING5, BUILDING7, VISITOR6, VISITOR7, BUILDING3, FISHING2, and MARY5 row
...
| 3 | Remaining true CD-pressure rows | The latest prefetch-relief pass removed ACTIVITY11/ACTIVITY12 from the due-miss collapse queue, and the stale-next refresh removed BUILDING3/FISHING2/MARY5 from the high-percent target list. The largest current pressure set is VISITOR3, WALKSTUF1, BUILDING2, ACTIVITY9, BUILDING4/6, FISHING4 low, ACTIVITY4 low, and JOHNNY6. WALKSTUF3 refreshed to near-target and should mainly serve as a stability canary unless a generated opportunity appears. | Continue with generated setup/segment coverage, selective upload-ready preprocessing, and scheduler-owned CD/render budgets for the remaining top rows. Do not spend time on generic zero-CD overhead until a fresh current row proves it still exists. |
