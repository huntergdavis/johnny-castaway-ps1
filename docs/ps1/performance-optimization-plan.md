...
| 1 | `walkstuf1` low/high and `visitor3` low/high | Current top outliers are WALKSTUF1 low/high at `1604/1407` and `1592/1406`, then VISITOR3 low/high at `1135/1024` and `1137/1024`. WALKSTUF1 scalar cap and direct-stage lanes are closed. VISITOR3 local C read-table rows are closed, the default selective upload-ready plan is footprint-closed as a same-layout append, the raw foreground-only upload payload is unsafe because selected x-bands are not draw-covered, the no-op empty-hold recast found `0` eligible zero-visual-work payload entries, and the scoped composite-helper code-shape win is now promoted. The budgeted same-footprint target still selects `74` frames, uses `814184` bytes, and retains `3858104` modeled saved upload bytes. | Move WALKSTUF1 and VISITOR3 to scheduler-owned generated metadata, the budgeted VISITOR3 upload-ready subset with a safe pixel source, compressed selective upload payloads, or shrinking pack transforms. Avoid more raw hand-authored read groups, scalar setup caps, no-op entry pruning, or same-footprint VISITOR3 default append attempts. |
