...
- **Public performance is effectively at native speed.** The public-capped
  rollup is `+0.3284%` over target / `99.6783%` target speed.
- **Raw optimization headroom remains visible.** The uncapped signed rollup is
  `-0.4401%` over target / `100.4621%` target speed, so future optimization
...
- **Public rollup:** `+0.3284%` over target / `99.6783%` target speed.
- **Raw signed rollup:** `-0.4401%` over target / `100.4621%` target speed.
- **Bands:** `111` green, `15` orange, `0` yellow, `0` red.
- **Under-99 focus set:** VISITOR3 low/high, WALKSTUF1 low/high, BUILDING2
...
- **VISITOR3 baseline:** high `1074/1038` with `58` blocking VBlanks, `10`
  ...
  copy; low is the paired current-layout control at `1079/1039` with `70`
  blocking VBlanks, `16` loop reads, and `12` due misses.
