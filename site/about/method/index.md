...
- **Host capture.** The desktop build (the same code, with the SDL2
  ...
  pan / volume. This is run by `scripts/capture-host-scene.sh`.
- **Pack compile.** `scripts/build-scene-foreground-pack.py` turns a
...
- **CD packaging.** `mkpsxiso` consumes `config/ps1/cd_layout.xml`
  ...
  the title raw.
- **PS1 replay.** `src/foreground_pilot.c` opens the matching pack
  ...
  sound events fire through `src/sound_ps1.c` on a per-pack event
...
it directly: timer-2 plus SIO0 IRQ at 250&nbsp;Hz, in `src/spi.c`.
...
`src/graphics_ps1.c` tracks per-frame dirty regions in `currDirty`
