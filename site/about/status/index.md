...
allocation. The public headless battle card is `+0.2657%` over
target / `99.7386%` target speed; the raw signed optimization matrix is
about `-0.4512%` / `100.4689%`. The allocator validation branch records a R34
...
clean-rect chunking and measures `1475/1443`, blocking/refill/due `48/12/6`,
with loop reads/read time at `39/200`. Both paths keep pack LBA/sectors and the PS-EXE bucket fixed while
...
and `1475/1443`; the latest high setup-segment retarget cuts high blocking
