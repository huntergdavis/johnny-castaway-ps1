...
<dd>A small text file the runtime reads at boot to override default state for a deterministic run. Carries one or more tokens — <code>night 1</code>, <code>lowtide 1</code>, <code>holiday N</code>, <code>raft-stage N</code>, <code>fgpilot &lt;slug&gt;</code>, <code>seed N</code>, <code>fgpilot freeplay</code>, etc. — that fix variant flags and entry points the screensaver loop would otherwise pick at random. The capture pipeline writes one before each host run; the regtest harness writes one before each headless DuckStation pass; the menu help auto-builder writes one before each pad-script run. End users never see it; release discs ship without it and let the screensaver pick its own state. The token surface is documented in [Scripted input]({{ '/docs/scripted-input/' | relative_url }}) and [Regression testing]({{ '/docs/regtest/' | relative_url }}).</dd>
</dl>
