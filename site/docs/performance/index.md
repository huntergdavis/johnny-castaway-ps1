...
the public average is **+0.2% over target / 99.8% target speed** (`0.1983%`
exact public over target / `99.8035%` exact public target speed); the raw
signed optimization matrix is `-0.5186%` / `100.5339%`.
...
- **Timing compression before throughput work.** The timing-bearing matrix
  public average is now +0.1983% over target / 99.8035% target speed, with three
