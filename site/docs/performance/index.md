...
the public average is **+0.3% over target / 99.7% target speed** (`0.3225%`
exact public over target / `99.6839%` exact public target speed); the raw
signed optimization matrix is `-0.4447%` / `100.4662%`.
...
- **Timing compression before throughput work.** The timing-bearing matrix
  public average is now +0.3225% over target / 99.6839% target speed, with several
