...
the public average is **+0.3% over target / 99.7% target speed** (`0.3414%`
exact public over target / `99.6691%` exact public target speed); the raw
signed optimization matrix is about `-0.3755%` / `100.3994%`.
...
- **Timing compression before throughput work.** The timing-bearing matrix
  public average is now +0.3414% over target / 99.6691% target speed, with several
