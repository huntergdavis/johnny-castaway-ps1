...
the public average is **+0.3% over target / 99.7% target speed** (`0.3156%`
exact public over target / `99.6902%` exact public target speed); the raw
signed optimization matrix is `-0.4529%` / `100.4740%`.
...
- **Timing compression before throughput work.** The timing-bearing matrix
  public average is now +0.3156% over target / 99.6902% target speed, with several
