...
the public average is **+0.2% over target / 99.8% target speed** (`0.2028%`
exact public over target / `99.7992%` exact public target speed); the raw
signed optimization matrix is about `-0.5141%` / `100.5295%`.
...
- **Timing compression before throughput work.** The timing-bearing matrix
  public average is now +0.2028% over target / 99.7992% target speed, with three
