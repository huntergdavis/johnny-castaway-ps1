...
the public average is **+0.3% over target / 99.7% target speed** (`0.2640%`
exact public over target / `99.7402%` exact public target speed); the raw
signed optimization matrix is about `-0.4529%` / `100.4705%`.
...
- **Timing compression before throughput work.** The timing-bearing matrix
  public average is now +0.2640% over target / 99.7402% target speed, with several
