...
the public average is **+0.2% over target / 99.8% target speed** (`0.2006%`
exact public over target / `99.8013%` exact public target speed); the raw
signed optimization matrix is `-0.5163%` / `100.5317%`.
...
- **Timing compression before throughput work.** The timing-bearing matrix
  public average is now +0.2006% over target / 99.8013% target speed, with three
