...
same no-shift payload lane through `walkstuf1-low-frame109-inplace-v875`.
...
`879801 -> 790381` without moving pack offsets, LBA/sectors, or PS-EXE size.
...
5. **Generated scheduler ownership for the remaining under-99 rows.** MARY3 is
   ...
   WALKSTUF1 low v747..v875 pass keeps the row exact-flat except for the v859
   one-VBlank target/overrun win while shrinking the no-shift frame list
   through frame `109` in-place (`879801 -> 790381` active payload), and v760
