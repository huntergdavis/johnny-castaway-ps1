...
the public average is **+0.3% over target / 99.7% target speed** (`0.2697%`
exact public over target / `99.7347%` exact public target speed); the raw
signed optimization matrix is `-0.4975%` / `100.5171%`.
...
same no-shift payload lane through `walkstuf1-low-frame140-inplace-v873`.
...
`879801 -> 790538` without moving pack offsets, LBA/sectors, or PS-EXE size.
...
5. **Generated scheduler ownership for the remaining under-99 rows.** MARY3 is
   ...
   WALKSTUF1 low v747..v873 pass keeps the row exact-flat except for the v859
   one-VBlank target/overrun win while shrinking the no-shift frame list
   through frame `140` in-place (`879801 -> 790538` active payload), and v760
...
- **Timing compression before throughput work.** The timing-bearing matrix
  public average is now +0.2697% over target / 99.7347% target speed, with several
