...
the public average is **+0.3% over target / 99.7% target speed** (`0.3426%`
exact public over target / `99.6651%` exact public target speed); the raw
signed optimization matrix is `-0.4259%` / `100.4489%`.
...
- **Timing compression before throughput work.** The timing-bearing matrix
  public average is now +0.3426% over target / 99.6651% target speed, with several
