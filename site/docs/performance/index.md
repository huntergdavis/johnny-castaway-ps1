...
the public average is **+0.2% over target / 99.8% target speed** (`0.2073%`
exact public over target / `99.7948%` exact public target speed); the raw
signed optimization matrix is about `-0.5096%` / `100.5251%`.
...
- **Timing compression before throughput work.** The timing-bearing matrix
  public average is now +0.2073% over target / 99.7948% target speed, with three
