...
the public average is **+0.2% over target / 99.8% target speed** (`0.2334%`
exact public over target / `99.7696%` exact public target speed); the raw
signed optimization matrix is about `-0.4835%` / `100.4999%`.
...
- **Timing compression before throughput work.** The timing-bearing matrix
  public average is now +0.2334% over target / 99.7696% target speed, with several
