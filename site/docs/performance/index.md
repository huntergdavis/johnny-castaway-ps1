...
the public average is **+0.2% over target / 99.8% target speed** (`0.1977%`
exact public over target / `99.8041%` exact public target speed); the raw
signed optimization matrix is `-0.5192%` / `100.5344%`.
...
- **Timing compression before throughput work.** The timing-bearing matrix
  public average is now +0.1977% over target / 99.8041% target speed, with three
