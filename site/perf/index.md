...
| Stats version | mixed across rows; latest refreshed row is `walkstuf1-high-frame49-inplace-v841`; per-row version is in the [`Stats Version` column below](#reading-the-table). |
| FISHING 1 canary | `1068 / 1072 VBlanks`, `0.0%` public over target, `100.0%` public target speed, `blocking_vb=5` |

Current W1 work-volume track: `walkstuf1-high-frame49-inplace-v841`
shrinks high entry `49` / source frame `59` in place after the v839 frame51
shrink, preserving all payload offsets while cutting that entry
`5269 -> 770` bytes. High is exact-flat at `1764`, `1476/1434`,
blocking/refill `81/23`, loop reads/read time `63/276`, and due `16`, while
runtime rows/spans/pixels stay at `16859/129919/731016`; v657/v839/v841 now provide the active
payload drop `918345 -> 872690`, CD sectors `605 -> 586`, and loop
...
<table class="scene-perf-table">
  ...
  <tbody>
    ...
    <tr id="perf-walkstuf1-high">
      ...
      <td>high</td>
      <td>measured</td>
      <td>2026-05-14T21:06:22</td>
      <td>walkstuf1-high-frame49-inplace-v841</td>
      <td>+2.9%</td>
      ...
      <td>23</td>
      <td>16</td>
      <td>same-speed preserve-offset frame49 payload shrink on top of v839; entry 49/source frame 59 shrinks 5269 -&gt; 770 bytes, active payload 877189 -&gt; 872690, file size/LBA/sectors/PS-EXE bucket stay fixed, scene/loop/target remain 1764/1476/1434, overrun 42, blocking/refill 81/23, reads/read time 63/276, and due 16</td>
    </tr>
