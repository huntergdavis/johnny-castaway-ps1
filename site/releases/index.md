...
- **Battle card is now public-capped at native speed.** After the follow-up
  ...
  v213, all 126 timing-bearing rows average `+0.3930%` public over target /
  `99.6194%` public target speed; the raw signed CSV is `-0.3755%` /
  `100.4032%` for
  optimization work.
- **VISITOR3 motion-copy payloads are promoted.** Frames `119..123` in both
  ...
  the high-only `288 KiB` setup-prime cap now move already-composited
