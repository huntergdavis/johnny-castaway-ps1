 ...
 * NOTE (post-1.0 candidate, do not re-land without console soak): a
 * verified-continuous (CdlReadN raw) variant of these entry reads was
 * byte-exact in emulator but hung a console boot at ~80% (0.9.14-bt2,
 * reverted). Boot-time ReadN interplay is unproven; previews use it
 * safely because a failed read there degrades to a repaint, while a
 * wedged boot read parks the ship. Returns the number of effects
 * loaded; 0 -> caller runs the per-file fallback loop. */
